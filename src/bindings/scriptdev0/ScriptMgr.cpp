...
std::map<uint32, Script*> mAuraScripts;
std::map<uint32, Script*> mLootScripts;
std::map<uint32, Script*> mSpellScripts;
...
void Script::RegisterAsAuraScript(bool)
{
	for(auto spellId : SpellIds)
		mAuraScripts[spellId] = this;
	++num_sc_scripts;
}

void Script::RegisterAsSpellScript(bool)
{
	for(auto spellId : SpellIds)
		mSpellScripts[spellId] = this;
	++num_sc_scripts;
}

void Script::RegisterAsLootScript(bool)
{
	mLootScripts[LootId] = this;
	++num_sc_scripts;
}
...
MANGOS_DLL_EXPORT
bool AuraApply(Aura const* pAura, bool apply, uint32 type) {
	auto itr = mAuraScripts.find(pAura->GetId());
	if(itr == mAuraScripts.end())
		return false;

	if(itr->second == nullptr)
		return false;

	if(itr->second->pEffectAuraApply == nullptr)
		return false;

	return itr->second->pEffectAuraApply(pAura, apply, type);
}

MANGOS_DLL_EXPORT
void FillLoot(Loot& loot, Player* owner, uint32 id) {
	auto itr = mLootScripts.find(id);
	if(itr == mLootScripts.end())
		return;

	if(itr->second == nullptr)
		return;

	if(itr->second->pFillLoot == nullptr)
		return;

	itr->second->pFillLoot(loot, owner);
}

MANGOS_DLL_EXPORT
bool EffectDummy(Object* pCaster, uint32 spellId, SpellEffectIndex effIndex, Object* pTarget) {
	auto itr = mSpellScripts.find(spellId);
	if(itr == mSpellScripts.end())
		return false;

	if(itr->second == nullptr)
		return false;

	if(itr->second->pEffectDummy == nullptr)
		return false;

	return itr->second->pEffectDummy(pCaster, spellId, effIndex, pTarget);
}