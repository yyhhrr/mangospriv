...
const float MAX_PLAYER_DISTANCE_DUNGEON = 500.0f;
...
bool npc_escortAI::IsPlayerOrGroupInRange()
{
    if (Player* pPlayer = GetPlayerForEscort())
    {
		...
		else if (m_creature->IsWithinDistInMap(pPlayer, MAX_PLAYER_DISTANCE))
		{
            return true;
		}
    }
...
void npc_escortAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !m_creature->getVictim() && m_uiWPWaitTimer && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiWPWaitTimer <= uiDiff)
        {
            ...
            if (CurrentWP == WaypointList.end())
            {
				if(mPreventReset == false) {
					debug_log("SD0: EscortAI reached end of waypoints");
					...
					if (m_bCanReturnToStart)
					{
						...
						debug_log("SD0: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
						return;
					}
            ...
            {
                m_creature->GetMotionMaster()->MovePoint(CurrentWP->uiId, CurrentWP->fX, CurrentWP->fY, CurrentWP->fZ);
                debug_log("SD0: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->uiId, CurrentWP->fX, CurrentWP->fY, CurrentWP->fZ);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_playerGuid && !m_creature->getVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer < uiDiff)
        {
            ...
            {
				mPreventReset = false;
                debug_log("SD0: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::UpdateEscortAI(const uint32)
{
...
void npc_escortAI::MovementInform(uint32 uiMoveType, uint32 uiPointId)
{
	...
	else
	{
		...
		{
			error_log("EscortAI for Npc %u reached waypoint out of order %u, expected %u.", m_creature->GetEntry(), uiPointId, CurrentWP->uiId);
			return;
		}
...
void npc_escortAI::SetCurrentWaypoint(uint32 uiPointId)
{
    ...
    debug_log("SD0: EscortAI current waypoint set to id %u", CurrentWP->uiId);
}
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("SD0: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("SD0: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
...
void npc_escortAI::Start(bool bRun, const Player* pPlayer, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    if (m_creature->getVictim())
    {
        debug_log("SD0: EscortAI attempt to Start while in combat.");
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
		debug_log("SD0: EscortAI attempt to Start while already escorting.");
        return;
    }
    ...
    if (WaypointList.empty())
    {
        error_db_log("SD0: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint).");
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        debug_log("SD0: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        m_creature->GetMotionMaster()->MovementExpired();
        m_creature->GetMotionMaster()->MoveIdle();
        debug_log("SD0: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }

    //disable npcflags
    m_creature->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);

    debug_log("SD0: EscortAI started with " SIZEFMTD " waypoints. Run = %d, PlayerGuid = %s", WaypointList.size(), m_bIsRunning, m_playerGuid.GetString().c_str());
