...
struct MANGOS_DLL_DECL npc_escortAI : public ScriptedAI
{
    public:
		...
		// If set to true the next time the NPC reaches the final waypoint it will neither despawn nor run back to its origin but
		// stay at the waypoint. Once you set it back to false it will go back again. Will be reset to false if the player(s) go
		// out of range and the escort fails
		void SetPreventReset(bool prevented) { mPreventReset = prevented; }
    ...
    private:
        ...
        bool DespawnAtFar;
		bool mPreventReset;									//creature does not reset when it reaches the last waypoint.
};
#endif
