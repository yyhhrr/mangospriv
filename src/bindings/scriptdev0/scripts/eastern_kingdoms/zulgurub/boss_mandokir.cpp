...
struct MANGOS_DLL_DECL boss_mandokirAI : public ScriptedAI
{
	...
	void JustDied(Unit* killer)
    {
		...
		if (killer->GetTypeId() == TYPEID_PLAYER)
			Track(static_cast<Player*>(killer), Raids::ZulGurub, Bosses::Mandokir);
    }
    ...
    void JustReachedHome()
    {
        if (m_pInstance)
		{
			...
			m_pInstance->SetData(TYPE_MANDOKIR, FAIL);
		}
		...
		if(auto* ohgan = m_creature->GetMap()->GetUnit(m_ohganGuid))
			ohgan->RemoveFromWorld();
