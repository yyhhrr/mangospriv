...
struct MANGOS_DLL_DECL boss_jindoAI : public ScriptedAI
{
    ...
    void SummonedCreatureJustDied(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_BRAINWASH_TOTEM)
        {
            m_uiBrainWashTotemTimer = 20000;    // how long delay?
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiTeleportTimer < uiDiff)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
            {
                ...
                // summon 9 skeletons in the pit at random pointsx
                float fX, fY, fZ;
...
{
    mob_brainwash_totemAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        Unit* pTarget = m_creature->SelectRandomUnfriendlyTarget();
        if (!pTarget)
        {
            pTarget = m_creature->SelectRandomUnfriendlyTarget();
        }
        if (m_uiBWTimer < uiDiff)
        {
            Unit* target = m_creature->SelectRandomUnfriendlyTarget();
            
            if (!target || (target->GetTypeId() == TYPEID_UNIT && target->GetCreatureType() == CREATURE_TYPE_CRITTER)
                || (m_creature->GetOwner() && m_creature->GetOwner()->getVictim() == target))
            {
                m_uiBWTimer = 3000;
                return;
            }

            DoCastSpellIfCan(target, SPELL_BW_MC);
            m_uiBWTimer = 240000;
        }
        else
        {
...
CreatureAI* GetAI_mob_brainwash_totem(Creature* pCreature)
{
