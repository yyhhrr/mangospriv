...
struct MANGOS_DLL_DECL boss_thekalAI : public boss_thekalBaseAI
{
    ...
    void JustDied(Unit* killer)
    {
		...
		if (killer->GetTypeId() == TYPEID_PLAYER)
			Track(static_cast<Player*>(killer), Raids::ZulGurub, Bosses::Thekal);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        switch (m_uiPhase)
        {
            ...
            case PHASE_TIGER:
                ...
                {
                    if (Unit* pTopTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0))
                    {
                        if (DoCastSpellIfCan(m_creature, SPELL_FORCE_PUNCH) == CAST_OK)
                        {
                            m_uiForcePunchTimer = urand(16000, 21000);
                            m_creature->getThreatManager().modifyThreatPercent(pTopTarget, -30);
                        }
                    }
                }
                else
...
//                        Creature* pThekal = m_pInstance->GetSingleCreatureFromStorage(NPC_THEKAL);
                ...
                if (m_uiGreaterHealTimer < uiDiff)
                {
					if (m_pInstance)
					{
						...
						if (pTarget && m_creature->IsWithinDistInMap(pTarget, 18.0f))
            ...
            case PHASE_NORMAL:
                ...
                if (m_uiGougeTimer < uiDiff)
                {
                    if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_GOUGE) == CAST_OK)
                    {
                        ...
                        m_uiGougeTimer = urand(21000, 25000);
                    }
                }
                else
                ...
                if (m_uiKickTimer < uiDiff)
                {
                    if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_KICK) == CAST_OK)
                        m_uiKickTimer = urand(14000, 25000);
                }
                else
                ...
                if (m_uiBlindTimer < uiDiff)
                {
                    if (DoCastSpellIfCan(m_creature->getVictim(), SPELL_BLIND) == CAST_OK)
                        m_uiBlindTimer = urand(12000, 20000);
                }
                else
...
bool EffectDummyCreature_thekal_resurrection(Unit*, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
