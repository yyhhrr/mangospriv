...
struct MANGOS_DLL_DECL boss_garrAI : public ScriptedAI
{
    ...
    {
        if (pInstance)
            pInstance->SetData(TYPE_GARR, IN_PROGRESS);
    }

    void JustReachedHome()
    {
        if (pInstance)
            pInstance->SetData(TYPE_GARR, FAIL);
    ...
    {
        if (pInstance)
			pInstance->SetData(TYPE_GARR, DONE);

		using namespace ProgressMonitor;
		if (killer->GetTypeId() == TYPEID_PLAYER)
			Track(static_cast<Player*>(killer), Raids::MoltenCore, Bosses::Garr);
    }
...
struct MANGOS_DLL_DECL mob_fireswornAI : public ScriptedAI
{
    ...
    void Reset()
    {
        m_bExplosion = m_bIsErruption = false;
	...
	{
        if (pInstance)
            if (Creature* pGarr = pInstance->GetSingleCreatureFromStorage(NPC_GARR))
    ...
    void JustDied(Unit* /*pKiller*/)
    {
        ...
        {
            // Garr's Frenzy
            Creature* pGarr = pInstance->GetSingleCreatureFromStorage(NPC_GARR);
   ...
   // }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
			m_creature->RemoveAurasDueToSpell(SPELL_SEPARATION_ANXIETY);
            m_creature->CastSpell(m_creature, SPELL_MASSIVE_ERUPTION, false);
			m_bIsErruption = true;
        }
        ...
        if (m_uiSeparationAnxietyTimer <= uiDiff)
        {
            ...
            {
				Creature* pGarr = pInstance->GetSingleCreatureFromStorage(NPC_GARR);
                if (pGarr && pGarr->isAlive() && pGarr->GetDistance(m_creature) > 40.0f)
