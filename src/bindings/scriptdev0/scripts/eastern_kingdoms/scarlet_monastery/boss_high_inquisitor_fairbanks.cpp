...
struct MANGOS_DLL_DECL boss_high_inquisitor_fairbanksAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		if(instance && instance->IsAshbringerEventRunning())
		{
			...
			if(m_creature->GetDistance(instance->GetAshbringer()) < 5.f && m_creature->GetUInt32Value(UNIT_FIELD_DISPLAYID) != 16179)
			{
				m_creature->SetFacingToObject(instance->GetAshbringer());
				m_creature->SetUInt32Value(UNIT_FIELD_DISPLAYID, 16179);
			}
			return;
		}
...
bool GossipHello_boss_fairbanks(Player* pPlayer, Creature* pCreature)
{
    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, -3000200, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
    pPlayer->SEND_GOSSIP_MENU(90010, pCreature->GetGUID());

	return true;
}

bool GossipSelect_boss_fairbanks(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
	auto a = uiAction - (GOSSIP_ACTION_INFO_DEF+1);
	int gossip_id = -3000201 - a;
	int menu_id = 90011 + uiAction - (GOSSIP_ACTION_INFO_DEF+1);

	if(gossip_id > -3000216)
...
void AddSC_boss_high_inquisitor_fairbanks()
{
    ...
    newscript->pGossipHello =  &GossipHello_boss_fairbanks;
    newscript->pGossipSelect = &GossipSelect_boss_fairbanks;
