...
void instance_stratholme::OnObjectCreate(GameObject* pGo)
{
    switch(pGo->GetEntry())
    {
        ...
        case GO_ZIGGURAT_DOOR_5:
			...
			if (m_auiEncounter[TYPE_BARON] == IN_PROGRESS)
				pGo->SetGoState(GO_STATE_READY);

            if (m_auiEncounter[TYPE_BARON] == FAIL)
                pGo->SetGoState(GO_STATE_ACTIVE);
            break;
        ...
        case TYPE_BARTHILAS_RUN:
            if (uiData == IN_PROGRESS)
            {
                ...
                if (pBarthilas && pBarthilas->isAlive() && !pBarthilas->isInCombat())
                {
					...
					pBarthilas->m_movementInfo.RemoveMovementFlag(MOVEFLAG_WALK_MODE);
...
void instance_stratholme::Update(uint32 uiDiff)
{
    if (m_bBaronWarn)
    {
        ...
        {
			GameObject* pGauntlet = GetSingleGameObjectFromStorage(GO_GAUNTLET_GATE1);
			Creature* pMagistrate = GetSingleCreatureFromStorage(NPC_BARTHILAS);
