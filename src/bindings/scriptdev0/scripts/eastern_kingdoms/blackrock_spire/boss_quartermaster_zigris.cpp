...
struct MANGOS_DLL_DECL boss_quatermasterzigrisAI : public ScriptedAI
{
    ...
    uint32 m_uiHelingPotion_Timer;
	uint32 m_uiNetzTimer;

    void Reset()
    {
        m_uiShootTimer    = 1000;
        m_uiStunBombTimer = 16000;
        m_uiHelingPotion_Timer = 25000;
		m_uiNetzTimer	= 8000;
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        // HelingPotion
        if (m_uiHelingPotion_Timer < uiDiff)
        {
            DoCastSpellIfCan(m_creature, SPELL_HEALING_POTION);
            m_uiHelingPotion_Timer = 22000;
        }
        else
            m_uiHelingPotion_Timer -= uiDiff;

        // Netz Timer
        if (m_uiNetzTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature->getVictim(), SPELL_HOOKEDNET);
            m_uiNetzTimer = 9000;
        }
        else
            m_uiNetzTimer -= uiDiff;
