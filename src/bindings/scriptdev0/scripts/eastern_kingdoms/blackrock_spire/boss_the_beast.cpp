...
{
    boss_the_beastAI(Creature* pCreature) : ScriptedAI(pCreature)
	{
		Reset();
	}
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiFireballTimer <= uiDiff)
        {
            ...
            m_uiFireballTimer = urand(6000, 10000);
        }
        else
        ...
        if (m_uiFlamebreakTimer <= uiDiff)
        {
            ...
            m_uiFlamebreakTimer = urand(5000, 10000);
        }
        else
