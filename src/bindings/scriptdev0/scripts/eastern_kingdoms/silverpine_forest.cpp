...
struct MANGOS_DLL_DECL npc_pyrewood_citizen : public ScriptedAI
{
	uint32 timer;

    npc_pyrewood_citizen(Creature* pCreature) : 
	...
	{
		return new npc_pyrewood_citizen(creature);
	}
	...
	{
		using namespace std::chrono;
		const int32 timeToChangeAppearing = 20; //20:00 o'clock (8 pm)

		//not sure if this 'optimisation' is necessary, but whatever... 
		if(timer < 10000)
		{
			timer += timeDelta;
			return;
		...
		auto sinceMidnight = duration_since_midnight();
		auto hoursSinceMidnight = duration_cast<hours>(sinceMidnight).count();

		auto currentDisplayId = m_creature->GetDisplayId();
		auto nativeDisplayId = m_creature->GetNativeDisplayId();

		if(timeToChangeAppearing <= hoursSinceMidnight)
		{
			if(currentDisplayId == nativeDisplayId || currentDisplayId == 0)
				m_creature->SetDisplayId(729);
		}
		else
		{
			if(currentDisplayId != nativeDisplayId)
				m_creature->SetDisplayId(nativeDisplayId);
		}
...
void AddSC_silverpine_forest()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_pyrewood_citizen";
    pNewScript->GetAI = npc_pyrewood_citizen::GetAI;
    pNewScript->RegisterSelf();
}
