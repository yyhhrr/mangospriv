...
enum
{
    ...
    SPELL_CHROMATIC_MUT_1       = 23174,                    // Spell cast on player if they get 5 debuffs
...
struct MANGOS_DLL_DECL boss_chromaggusAI : public ScriptedAI
{
    boss_chromaggusAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
		...
		if(m_pInstance)
		{
			m_uiBreathOneSpell = aPossibleBreaths[m_pInstance->GetData(VALUE_BREATH1)];
			m_uiBreathTwoSpell = aPossibleBreaths[m_pInstance->GetData(VALUE_BREATH2)];
		}
    ...
    void Reset()
    {
        ...
        m_uiAfflictionTimer = 5000;                        // This is special - 5 seconds means that we cast this on 1 pPlayer every 5 sconds
        m_uiFrenzyTimer     = urand(8000, 12000);

        m_switchBreath		= false;
        m_bEnraged          = false;
		m_aggroResetted		= false;

		m_creature->SetBoundingValue(6, 8);
    }
    ...
    void JustDied(Unit* killer)
    {
		...
		if (killer->GetTypeId() == TYPEID_PLAYER)
			Track(static_cast<Player*>(killer), Raids::BlackWingLair, Bosses::Chromaggus);
    }
	...
	{
		if (pSpell->Id == SPELL_TIME_LAPSE && !m_aggroResetted)
		{
			m_aggroResetted = true;
			DoResetThreatIf([&](Unit* unit) -> bool
			{
				return unit->HasAura(SPELL_TIME_LAPSE);
			});
		}
	}
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiAfflictionTimer < uiDiff)
        {
			...
			uint32 maxCount = urand(10, 15);
			std::random_shuffle(std::begin(vGuids), std::end(vGuids));

			while(vGuids.size() > maxCount)
				vGuids.erase(std::begin(vGuids));
            ...
            for (std::vector<ObjectGuid>::const_iterator i = vGuids.begin();i != vGuids.end(); ++i)
            {
                ...
                if (pUnit)
                {
					pUnit->CastSpell(pUnit, m_uiSpellAfflict, true);
                    // Chromatic mutation if target is effected by 5 afflictions
...
bool lever_used(Player*, GameObject* go)
{
	instance_blackwing_lair* instance = static_cast<instance_blackwing_lair*>(go->GetInstanceData());
	if(instance == nullptr)
		return true;

	instance->SetData(TYPE_CHROMAGGUS, SPECIAL);
...
void AddSC_boss_chromaggus()
{
	...
	pNewScript = new Script;
	pNewScript->Name = "chromaggus_lever";
	pNewScript->pGOUse = lever_used;
    pNewScript->RegisterSelf();
}
