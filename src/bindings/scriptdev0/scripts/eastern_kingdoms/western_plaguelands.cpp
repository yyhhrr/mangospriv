...
bool GossipHello_npcs_dithers_and_arbington(Player* pPlayer, Creature* pCreature)
{
	...
	bool displayOptions = pPlayer->GetQuestRewardStatus(5216) || pPlayer->GetQuestStatus(5216) == QUEST_STATUS_COMPLETE || pPlayer->GetQuestStatus(5216) == QUEST_STATUS_INCOMPLETE;
	displayOptions = displayOptions || pPlayer->GetQuestRewardStatus(5229) || pPlayer->GetQuestStatus(5229) == QUEST_STATUS_COMPLETE || pPlayer->GetQuestStatus(5229) == QUEST_STATUS_INCOMPLETE;
...
bool GossipSelect_npcs_dithers_and_arbington(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
#define GOSSIP_ITEM_ILLUSION    "Ich bin bereit f\303\274r die Illusion, Myranda."
...
bool GossipHello_npc_myranda_the_hag(Player* pPlayer, Creature* pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(QUEST_SUBTERFUGE) == QUEST_STATUS_COMPLETE &&
        !pPlayer->GetQuestStatus(QUEST_IN_DREAMS) == QUEST_STATUS_COMPLETE && !pPlayer->HasAura(SPELL_SCARLET_ILLUSION))
    {
...
bool GossipSelect_npc_myranda_the_hag(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
struct MANGOS_DLL_DECL caer_darrowAI : public ScriptedAI
{
    caer_darrowAI(Creature* pCreature) : ScriptedAI(pCreature)
	{
        Reset();	
	}
...
CreatureAI* GetAI_caer_darrow(Creature *_Creature)
{
    return new caer_darrowAI (_Creature);
}
...
void AddSC_western_plaguelands()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "mob_fliegen";
    pNewScript->GetAI = &GetAI_caer_darrow;
    pNewScript->RegisterSelf();
}
