...
bool GossipHello_BG_AV_TeamSmith(Player* pPlayer, Creature* pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, "R\303\274ste uns auf!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        //wrong textid
...
bool ProcessEventId_event_spell_BG_AV_BOSS(uint32 uiEventId, Object* pSource, Object* pTarget, bool bIsStart)
{
    if (pSource->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (m_pInstance)
        {
            ...
            {
                if (Creature* myIvus = ((Player*)pSource)->SummonCreature(NPC_IVUS, -259.0f, -342.0f, 6.6f, 2.9f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000))
    ...
    void KilledUnit(Unit* pVictim)
    {
        ...
        if (!id)
        {
            if (pVictim->GetTypeId() == TYPEID_PLAYER)
                m_creature->MonsterYell("Ich labe mich an deinem Leid, sterbliches Wesen! Vermische deine Essenz mit LOKHOLAR!", LANG_UNIVERSAL);
...
struct MANGOS_DLL_DECL mob_av_bowman : public ScriptedAI
{
	...
	void getShootTarget()
	{
        ...
        if (pTarget && !m_creature->IsHostileTo(pTarget))
            pTarget = 0;
	}
