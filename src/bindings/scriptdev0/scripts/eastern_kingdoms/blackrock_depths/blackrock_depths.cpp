...
bool GOUse_go_shadowforge_brazier(Player*, GameObject* pGo)
{
...
bool GOUse_go_relic_coffer_door(Player*, GameObject* pGo)
{
    if (ScriptedInstance* pInstance = (ScriptedInstance*)pGo->GetInstanceData())
    {
...
struct MANGOS_DLL_DECL npc_grimstoneAI : public npc_escortAI
{
    npc_grimstoneAI(Creature* pCreature) : npc_escortAI(pCreature)
    {
        ...
        m_uiMobSpawnId = static_cast<uint8>(urand(0, 5));
    ...
    void SummonedCreatureJustDied(Creature*)
    {
        ++m_uiMobDeadCount;
...
bool EffectDummyCreature_spell_banner_of_provocation(Unit*, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
...
bool GossipSelect_npc_kharan_mighthammer(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
bool GossipSelect_npc_lokhtos_darkbargainer(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
struct MANGOS_DLL_DECL npc_marshal_windsorAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId)
    {
        ...
        m_uiWP = static_cast<uint8>(uiPointId);
        switch(uiPointId)
        {
            case 1:
...
bool GOUse_go_cell_door(Player*, GameObject* pGo)
{
    ...
    void WaypointReached(uint32 uiPointId)
    {
        ...
        switch(uiPointId)
        {
            case 1:
				...
				m_creature->MonsterSay("Bier!", LANG_UNIVERSAL, NULL);
                break;
            case 2:
                m_creature->HandleEmote(EMOTE_ONESHOT_ATTACKUNARMED);
				m_creature->MonsterSay("Leer?... Ich...", LANG_UNIVERSAL, NULL);
                break;
            case 3:
                m_creature->HandleEmote(EMOTE_ONESHOT_ATTACKUNARMED);
				m_creature->MonsterYell("Mehr BIER!", LANG_UNIVERSAL, NULL);
                break;
            case 4:
                m_creature->HandleEmote(EMOTE_ONESHOT_KICK);
                break;
            case 5:
                m_creature->HandleEmote(EMOTE_ONESHOT_KICK);
				m_creature->MonsterYell("BIER... BIER! hicks...", LANG_UNIVERSAL, NULL);
...
bool QuestRewarded_npc_rocknot(Player*, Creature* pCreature, Quest const* pQuest)
{
...
struct MANGOS_DLL_DECL mob_doomgripAI : public ScriptedAI
{
    mob_doomgripAI(Creature* pCreature) : ScriptedAI(pCreature) { }
	...
	void JustDied(Unit*)
	{
        auto instance = static_cast<ScriptedInstance*>(m_creature->GetInstanceData());
		if(instance == nullptr)
			return;

		instance->SetData(TYPE_DOOMGRIP, DONE);
	}
};

bool GO_portrait_klicked(Player* player, GameObject* go)
{
    if (instance_blackrock_depths* instance = (instance_blackrock_depths*)go->GetInstanceData())
        instance->SpawnDarkKeeper(player, go);

    return false;
}

bool GO_dark_coffer_klicked(Player*, GameObject* go)
{
	//GO_DARK_COFFER
    if (instance_blackrock_depths* instance = (instance_blackrock_depths*)go->GetInstanceData())
	{
		auto coffer = instance->GetSingleGameObjectFromStorage(GO_DARK_COFFER);
		if(coffer == nullptr)
			return false;

		coffer->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NO_INTERACT);
		//TODO: for some reason the coffer sometimes gets the state GO_JUST_DEACTIVATED and becomes not lootable. no idea why. 
		//this does not fix it!
		coffer->SetLootState(GO_READY);
	}
...
void AddSC_blackrock_depths()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_doomgrip";
    pNewScript->GetAI = [](Creature* creature) -> CreatureAI*
	{
		return new mob_doomgripAI(creature);
	};
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "go_portrait";
    pNewScript->pGOUse = &GO_portrait_klicked;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "go_dark_coffer";
    pNewScript->pGOUse = &GO_dark_coffer_klicked;
    pNewScript->RegisterSelf();
}
