...
bool EffectDummyCreature_spell_boss_thermaplugg(Unit*, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    ...
    if (instance_gnomeregan* pInstance = (instance_gnomeregan*)pCreatureTarget->GetInstanceData())
        pInstance->DoActivateBombFace(static_cast<uint8>(urand(0, MAX_GNOME_FACES - 1)));
