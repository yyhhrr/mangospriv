...
void outdoor_pvp_eastern_plaguelands::OnObjectCreate(GameObject* pGo)
{
    switch(pGo->GetEntry())
    {
...
void outdoor_pvp_eastern_plaguelands::OnPlayerEnterZone(Player* pPlayer, uint32 uiZoneId)
{
...
void outdoor_pvp_eastern_plaguelands::SetData(uint32 uiType, uint32 uiData)
{
    ...
    for (uint8 i = 0; i < 4; i++)
    {
        // buff alliance
        if (m_uiTowersAlly == m_aPlaguelandsTowerBuffs[i].uiTowers)
            DoProcessTeamBuff(sPlaguelandsPlayers, ALLIANCE, m_aPlaguelandsTowerBuffs[i].uiSpellIdAlly);
        // buff horde
        if (m_uiTowersHorde == m_aPlaguelandsTowerBuffs[i].uiTowers)
            DoProcessTeamBuff(sPlaguelandsPlayers, HORDE, m_aPlaguelandsTowerBuffs[i].uiSpellIdHorde);
    }

    // debuff players if towers == 0; spell to remove will be always the first
    if (m_uiTowersHorde == 0)
        DoProcessTeamBuff(sPlaguelandsPlayers, HORDE, m_aPlaguelandsTowerBuffs[0].uiSpellIdHorde, true);
    if (m_uiTowersAlly == 0)
        DoProcessTeamBuff(sPlaguelandsPlayers, ALLIANCE, m_aPlaguelandsTowerBuffs[0].uiSpellIdAlly, true);
...
void outdoor_pvp_eastern_plaguelands::DoSummonFlightMasterIfCan()
{
    Player* pPlayer = GetPlayerInMap(sPlaguelandsPlayers);
    if (!pPlayer)
        return;
...
void outdoor_pvp_eastern_plaguelands::DoUnsummonFlightMaster()
{
    if (!m_uiFlightMasterGUID)
        return;
...
void outdoor_pvp_eastern_plaguelands::DoSummonSoldiersIfCan()
{
}

void outdoor_pvp_eastern_plaguelands::DoUnsummonSoldiers()
{
}

void outdoor_pvp_eastern_plaguelands::UpdateZoneWorldState(uint32 uiStateId, uint32 uiStateValue)
{
    DoUpdateWorldState(sPlaguelandsPlayers, uiStateId, uiStateValue);
}
...
void outdoor_pvp_eastern_plaguelands::SendPlayerWorldState(Player* pPlayer)
{
    ...
    // towers
    pPlayer->SendUpdateWorldState(m_uiCrownguardState, 1);
    pPlayer->SendUpdateWorldState(m_uiEastwallState, 1);
   pPlayer->SendUpdateWorldState(m_uiNorthpassState, 1);
    pPlayer->SendUpdateWorldState(m_uiPlaguewoodState, 1);
}

void outdoor_pvp_eastern_plaguelands::ProcessCaptureEvent(uint32 uiCaptureType, uint32 uiTeam, uint32 uiTower)
{
    switch(uiTower)
    {
...
bool ProcessEventId_event_crownguard_tower(uint32 uiEventId, Object* pSource, Object* pTarget, bool bIsStart)
{
    ...
    {
        if (outdoor_pvp_eastern_plaguelands* pOutdoorPvp = (outdoor_pvp_eastern_plaguelands*)((Player*)pSource)->GetInstanceData())
        {
            switch(uiEventId)
            {
...
bool ProcessEventId_event_plaguewood_tower(uint32 uiEventId, Object* pSource, Object* pTarget, bool bIsStart)
{
    if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER)
    {
      ((Player*)pSource)->MonsterSay("ohje1",LANG_UNIVERSAL);
        if(outdoor_pvp_eastern_plaguelands* pOutdoorPvp = (outdoor_pvp_eastern_plaguelands*)((Player*)pSource)->GetInstanceData())
	{
	  ((Player*)pSource)->MonsterSay("ohje2",LANG_UNIVERSAL);
            switch(uiEventId)
	    {
                ...
                case EVENT_PLAGUEWOOD_CONTEST_HORDE:
                    ...
                    pOutdoorPvp->ProcessCaptureEvent(CONTENT, ((Player*)pSource)->GetTeam(), TOWER_ID_PLAGUEWOOD);
                    break;
...
: OPvPCapturePoint(pvp), m_TowerState(EP_TS_N), m_FlightMasterSpawned(0)
{
    ...
    AddObject(EP_PWT_FAGS,EPTowerFlags[EP_PWT].entry,EPTowerFlags[EP_PWT].map,EPTowerFlags[EP_PWT].x,EPTowerFlags[EP_PWT].y,EPTowerFlags[EP_PWT].z,EPTowerFlags[EP_PWT].o,EPTowerFlagsEP_PWT].rot0,EPTowerFlags[EP_PWT].rot1,EPTowerFlags[EP_PWT].rot2,EPTowerFlags[EP_PWT].rot3);
}
