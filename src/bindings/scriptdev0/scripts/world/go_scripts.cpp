...
bool GOUse_go_cat_figurine(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_barov_journal(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_field_repair_bot_74A(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_orb_of_command(Player* pPlayer, GameObject*)
{
	if (pPlayer->GetQuestStatus(7761) == QUEST_STATUS_COMPLETE && pPlayer->GetQuestRewardStatus(7761))
...
bool GOUse_go_resonite_cask(Player*, GameObject* pGO)
{
    if (pGO->GetGoType() == GAMEOBJECT_TYPE_GOOBER)
...
bool GOUse_go_tablet_of_madness(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_demon_portal(Player* pPlayer, GameObject*)
{
    ...
    switch(uiStoneType)
    {
        ...
        case TYPE_GREATER_STONE:
        {
            ...
            else
            {
                ...
                uint8 uiRandom = static_cast<uint8>(urand(0, 3));
                switch(uiRandom)
                {
...
bool GOUse_go_hiveashi_pod(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_panther_cage(Player* pPlayer, GameObject*)
{
...
bool GOUse_go_silithyst_geyser(Player* pPlayer, GameObject* pObject)
{
...
bool GOUse_go_Cuergos_Gold(Player* pPlayer, GameObject*)
{
    if (pPlayer->GetQuestStatus(QUEST_GOLD) == QUEST_STATUS_INCOMPLETE)
	{
        pPlayer->SummonCreature(NPC_PIRAT, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), pPlayer->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 300000);
		pPlayer->SummonCreature(NPC_PIRAT, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), pPlayer->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 300000);
		pPlayer->SummonCreature(NPC_PIRAT, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), pPlayer->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 300000);
	}
	return false;
...
bool GOUse_go_snake_statue(Player* pPlayer, GameObject* pGO)
{
	if (pPlayer->GetQuestStatus(6027) == QUEST_STATUS_INCOMPLETE)
	{
		if (Creature* pCreature = pPlayer->SummonCreature(12369, 230.044f, 2994.25f, -0.866118f, 5.571f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000)) {

			pCreature->MonsterSay("Wer wagt es meine Ruhe zu st\303\226ren?", LANG_UNIVERSAL, 0);
			pCreature->AI()->AttackStart(pPlayer);
		}
	}
...
void AddSC_go_scripts()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "go_cuergos_gold";
    pNewScript->pGOUse = &GOUse_go_Cuergos_Gold;
    pNewScript->RegisterSelf();

	pNewScript = new Script;
	pNewScript->Name = "go_snake_statue";
	pNewScript->pGOUse = &GOUse_go_snake_statue;
	pNewScript->RegisterSelf();
}
