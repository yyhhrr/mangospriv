...
enum Spells
{
    ...
    SPELL_SPIRIT_HEAL1            = 22012,                // Spirit Heal
    SPELL_WAITING_TO_RESURRECT1   = 2584                  // players who cancel this aura don't want a resurrection
};
...
struct MANGOS_DLL_DECL npc_spirit_guideAI : public ScriptedAI
{
	...
	uint32 m_uiCastSpellResurrectTimer;

    void Reset()
    {
        m_uiCastSpellResurrectTimer = 1000;
	...
	void UpdateAI(const uint32 uiDiff)
    {
		...
		if (m_uiCastSpellResurrectTimer < uiDiff)
		{
			...
			for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
			{
				...
				if (pPlayer && pPlayer->IsWithinDistInMap(m_creature, 25.0f) && !pPlayer->HasAura(SPELL_WAITING_TO_RESURRECT1))
					m_creature->CastSpell(pPlayer, SPELL_WAITING_TO_RESURRECT1, true);
			}
				m_uiCastSpellResurrectTimer = 5000;
		}
		else
			m_uiCastSpellResurrectTimer -= uiDiff;
    }
...
bool GossipHello_npc_spirit_guide(Player* pPlayer, Creature*)
{
    pPlayer->CastSpell(pPlayer, SPELL_WAITING_TO_RESURRECT1, true);
    return true;
}
