...
void instance_temple_of_ahnqiraj::OnCreatureDeath(Creature* pCreature)
{
    ...
    {
        case NPC_ANUBUSATH_SENTINEL:
        case NPC_OBSIDAN:
            cnt = urand(0, 2);
            for (uint32 i = 0; i < cnt; i++)
            {
                float x, y, z;
                pCreature->GetRandomPoint(pCreature->GetPositionX(), pCreature->GetPositionY(), pCreature->GetPositionZ(), pCreature->GetObjectBoundingRadius(), x, y, z);
                GameObject* obj = pCreature->SummonGameObject(GO_OBSIDAN_SMALL, x, y, z, rand_norm_f() * 2 * M_PI_F, 3600 * IN_MILLISECONDS);
                if (obj)
                    obj->Respawn();
            }
            return;
