...
enum
{
    ...
    SPELL_SUMMON_WORM_3     = 25832, //Worms should cast 26662

    NPC_SPAWN_FANKRISS      = 15630,
    NPC_VEKNISS_HATCHLING   = 15962,
};

static const uint32 aEntangleSounds[3] = { SOUND_SENTENCE_YOU, SOUND_LAWS, SOUND_TRESPASS };
...
static const float aHatchlingSpawns[3][3] = { 
    { -8043.01f, 1254.21f, -84.19f },
    { -8003.f, 1222.9f, -82.1f },
...
struct MANGOS_DLL_DECL boss_fankrissAI : public ScriptedAI
{
    ...
    void Aggro(Unit*)
    {
        ...
        //m_creature->PlayDirectSound(SOUND_WILL_BE);
    }
    ...
    void JustDied(Unit* /*pKiller*/)
    {
        ...
        //m_creature->PlayDirectSound(SOUND_SERVE_TO);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiMortalWoundTimer < uiDiff)
        {
            ...
            for (auto& player : players)
            {
                ...
                if (pPlayer && pPlayer->isAlive() && m_creature->isInFront(pPlayer, m_creature->GetCombatDistance(pPlayer, true)))
                    DoCastSpellIfCan(pPlayer, SPELL_MORTAL_WOUND, CAST_TRIGGERED);
            }
        ...
        {
            uint8 uiSpawnIndex = static_cast<uint8>(urand(0, 2));
            if (DoCastSpellIfCan(m_creature, aSummonWormSpells[uiSpawnIndex]) == CAST_OK)
                m_uiSummonWormTimer = urand(15000, 30000);
        }
        else
        ...
        if (m_uiEntangleTimer < uiDiff)
        {
            ...
            {
                uint8 uiEntangleIndex = static_cast<uint8>(urand(0, 2));
                if (DoCastSpellIfCan(pTarget, aEntangleSpells[uiEntangleIndex]) == CAST_OK)
                {
                    //m_creature->PlayDirectSound(aEntangleSounds[urand(0, 2)]);
            ...
            if (m_uiEntangleSummonTimer <= uiDiff)
            {
                if (Player* pTarget = m_creature->GetMap()->GetPlayer(m_EntangleTargetGuid))
                {
                    ...
                    for (uint8 i = 0; i < 3; ++i)
                    {
                        ...
                        for (uint8 j = 0; j < 2; ++j)
                        {
                            m_creature->GetRandomPoint(aHatchlingSpawns[i][0], aHatchlingSpawns[i][1], aHatchlingSpawns[i][2], 3.0f, fX, fY, fZ);
                            m_creature->SummonCreature(NPC_VEKNISS_HATCHLING, fX, fY, fZ, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
                        }
                    }
                }
...
struct npc_spawn_of_fankrissAI : public ScriptedAI
{
    npc_spawn_of_fankrissAI(Creature* pCreature) : ScriptedAI(pCreature) { Reset(); }

    uint32 m_uiEnrageTimer;

    void Reset()
    {
        m_uiEnrageTimer = 30 * IN_MILLISECONDS;
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiEnrageTimer)
        {
            ...
            {
                if (DoCastSpellIfCan(m_creature, 0) == SPELL_CAST_OK)
                    m_uiEnrageTimer = 0;
            }
            else
...
CreatureAI* GetAI_npc_spawn_of_fankriss(Creature* pCreature)
{
    return new npc_spawn_of_fankrissAI(pCreature);
}
...
void AddSC_boss_fankriss()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "npc_spawn_of_fankriss";
    pNewScript->GetAI = &GetAI_npc_spawn_of_fankriss;
    pNewScript->RegisterSelf();
}
