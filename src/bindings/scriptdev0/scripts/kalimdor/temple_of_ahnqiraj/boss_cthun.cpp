...
struct MANGOS_DLL_DECL eye_of_cthunAI : public ScriptedAI
{
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_CTHUN, PHASE_EYE_NORMAL);
    }

    void EnterEvadeMode()
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_CTHUN, PHASE_NOT_STARTED);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        switch (m_pInstance->GetData(TYPE_CTHUN))
        {
            case PHASE_EYE_NORMAL:
            {
                ...
                {
                    //Switch to Dark Beam
                    m_pInstance->SetData(TYPE_CTHUN, PHASE_EYE_DARK_GLARE);
    ...
    void DamageTaken(Unit* pDealer, uint32& uiDamage)
    {
        ...
        switch (m_pInstance->GetData(TYPE_CTHUN))
        {
            ...
            case PHASE_EYE_DARK_GLARE:
            {
                ...
                m_pInstance->SetData(TYPE_CTHUN, PHASE_TRANSITION);
...
struct MANGOS_DLL_DECL cthunAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        switch (m_pInstance->GetData(TYPE_CTHUN))
        {
            ...
            case PHASE_TRANSITION:
            {
                ...
                {
                    // Switch
                    m_pInstance->SetData(TYPE_CTHUN, PHASE_CTHUN);
            ...
            case PHASE_CTHUN:
            {
                ...
                {
                    m_pInstance->SetData(TYPE_CTHUN, PHASE_CTHUN_WEAKENED);
    ...
    void JustDied(Unit*)
    {
        // Switch
        if (m_pInstance)
            m_pInstance->SetData(TYPE_CTHUN, PHASE_FINISH);
    }
...
void flesh_tentacleAI::JustDied(Unit*)
{
    if (!m_cThunGuid)
    {
