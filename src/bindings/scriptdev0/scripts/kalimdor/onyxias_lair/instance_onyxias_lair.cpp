...
void instance_onyxias_lair::Initialize()
{
	...
	m_lWelpenEggsGuids.clear();
	m_lWelpenGuids.clear();
}
...
void instance_onyxias_lair::OnCreatureCreate(Creature* pCreature)
{
    switch(pCreature->GetEntry())
    {
		...
		case NPC_ONYXIA_WHELP:
			m_lWelpenGuids.push_back(pCreature->GetObjectGuid());
			break;
    }
}
...
void instance_onyxias_lair::OnObjectCreate(GameObject* pGo)
{
	...
	{
		case GO_ONYXIA_EGG:
			m_lWelpenEggsGuids.push_back(pGo->GetObjectGuid());
			return;
	}
}
...
void instance_onyxias_lair::SetData(uint32 uiType, uint32 uiData)
{
	...
	{
		for (GuidList::const_iterator itr = m_lWelpenEggsGuids.begin(); itr != m_lWelpenEggsGuids.end(); ++itr)
		{
		...
		for (GuidList::const_iterator itr = m_lWelpenGuids.begin(); itr != m_lWelpenGuids.end(); ++itr)
		{
			if (Creature* pWelpen = instance->GetCreature(*itr))
			{
				if (pWelpen->isAlive())
					pWelpen->ForcedDespawn();
			}
		}
	}
