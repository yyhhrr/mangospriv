...
#include "..\eastern_kingdoms\blackrock_spire\blackrock_spire.h"
...
bool GossipSelect_npc_neeru_fireblade(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
enum
{
	...
	THRALL_YELL_REND_DIED		 = 0,
	THRALL_YELL_REND_DIED_1		 = 0,
	SPELL_SEGEN_VON_THRALL		 = 16609,
};
...
struct MANGOS_DLL_DECL npc_thrall_warchiefAI : public ScriptedAI
{
	...
	uint32 GossipStepThrall;
	uint32 Text_Thrall_Timer;
    ...
    void Aggro(Unit*)
    {
		m_creature->MonsterYell("F\303\274r die HORDE!",LANG_UNIVERSAL,NULL);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
		...
		if (TYPE_GYTH == DONE && !RendEventDone)
		{
			if (Text_Thrall_Timer < uiDiff)
			{
				switch (GossipStepThrall)
				{
					case 1:
					DoScriptText(THRALL_YELL_REND_DIED, m_creature);
					Text_Thrall_Timer = 7000;
					break;

					case 2:
					DoScriptText(THRALL_YELL_REND_DIED_1, m_creature);
					Text_Thrall_Timer = 2000;
					break;
					...
					for(Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
					{
						...
						if (pPlayer->GetTeam() == HORDE && pPlayer->GetAreaId() == 1637)
							m_creature->CastSpell(pPlayer, SPELL_SEGEN_VON_THRALL, false);				
					}
...
bool GossipHello_npc_thrall_warchief(Player* pPlayer, Creature* pCreature)
{
	...
	if (pPlayer->isGameMaster())
	    pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, "[DEV] Segen Buff", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+10);
...
bool GossipSelect_npc_eitrigg(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
