...
enum
{
    ...
    // North
    TYPE_KING_GORDOK            = 10,
	TYPE_GUARD_MOLDAR			= 11,
	TYPE_GUARD_SLIPKIK			= 12,
	TYPE_GUARD_FENGUS			= 13,
	TYPE_CAPTAIN_KROMCRUSH		= 14,
	TYPE_KNOT_CHEST				= 15,
	TYPE_GORDOK_CHEST			= 16,
...
class MANGOS_DLL_DECL instance_dire_maul : public ScriptedInstance
{
    ...
    protected:
		...
		void GordokTributeEvent(uint32 diff);
		...
		uint32 GossipStepGordok;
		uint32 Text_Timer_Event;

		uint64 m_uiGordokTrubuteGUID;
		uint64 m_uiKnotsKisteGUID;
        uint64 m_uiGordokGUID;
        uint64 m_uiChoRushGUID;
		uint64 m_uiSlipKikGUID;
		uint64 m_uiFengusGUID;
		uint64 m_uiMoldarGUID;
        uint64 m_uiMizzleGUID;
		uint64 m_uiKromcrushGUID;
		uint64 m_uiTributeTriggerGUID;
};
