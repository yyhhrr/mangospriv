...
static Loc Larva[]=
{
    {-9695.0f, 1585.0f, 25.0f},
    {-9627.0f, 1538.0f, 21.44f}
};
...
struct MANGOS_DLL_DECL boss_ayamissAI : public ScriptedAI
{
    ...
    {
        if (m_pInstance)
            m_pInstance->SetData(TYPE_AYAMISS, FAIL);
    }
    ...
    void Aggro(Unit* pWho)
    {
		...
		m_creature->GetMotionMaster()->MovePoint(1, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ() + 20.0f, false);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
		...
		if (!m_bPhaseTwo && m_creature->GetHealthPercent() <= 70.0f)
        {
