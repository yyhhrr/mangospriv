...
struct MANGOS_DLL_DECL boss_kurinnaxxAI : public ScriptedAI
{
	...
	void checkPlayerRange()
	{
		if (m_pInstance)
		{
			...
			{
				if (Creature* pCreature = m_pInstance->GetSingleCreatureFromStorage(NPC_KURINNAXX))
				{
	...
	void UpdateAI(const uint32 uiDiff)
	{
		if (!m_creature->SelectHostileTarget() || !m_creature->getVictim())
		{
			...
			for (auto& player : players) {
                ...
                if (plr && plr->GetDistance2d(m_creature) < 51.f && !plr->isGameMaster() && plr->isGMVisible() && plr->isAlive())
				{
                    AttackStart(plr);
...
#if AQ_PATCH >= 15
		if (!m_bHasEnraged && m_creature->GetHealthPercent() <= 30.0f)
#else
        if (!m_bHasEnraged && m_creature->GetHealthPercent() <= 20.0f)
#endif
		{
		...
		if (m_uiSummonPlayerTimer <= uiDiff)
		{
			if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1)) {
				...
				float fX, fY, fZ;
				m_creature->GetNearPoint2D(fX, fY, m_creature->GetObjectBoundingRadius(), m_creature->GetOrientation());
                pTarget->UpdateAllowedPositionZ(fX, fY, fZ);
				DoTeleportPlayer(pTarget, fX, fY, fZ + 2, pTarget->GetOrientation());
			}
		...
		if (m_uiTrashTimer <= uiDiff)
		{
			...
			if (pTarget && pTarget->isAlive() && m_creature->CanReachWithMeleeAttack(pTarget))
