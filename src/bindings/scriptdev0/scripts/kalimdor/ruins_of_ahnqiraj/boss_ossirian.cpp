...
struct MANGOS_DLL_DECL boss_ossirianAI : public ScriptedAI
{
    ...
    void Aggro(Unit* pWho)
    {
		...
		m_creature->SummonObject(m_creature->GetMap(), GO_OSSIRIAN_CRYSTAL, Crystal[0].x, Crystal[0].y, Crystal[0].z, 0.0f);
    }
...
bool GOUse_go_ossirian_crystal(Player* pPlayer, GameObject* pGo)
{
    ...
    while(RollAgain)
    {
        ...
        {
			CoordX = Crystal[random].x;
			CoordY = Crystal[random].y;
			CoordZ = Crystal[random].z;

			GameObject* Crystal = pGo->SummonObject(pGo->GetMap(), GO_OSSIRIAN_CRYSTAL, CoordX, CoordY, CoordZ, 0.0f);
            RollAgain = false;
        }
    }
