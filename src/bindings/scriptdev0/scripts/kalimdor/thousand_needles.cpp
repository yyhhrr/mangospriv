...
bool GossipSelect_npc_plucky_johnson(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
struct MANGOS_DLL_DECL mob_pantherAI : public ScriptedAI
{
    mob_pantherAI(Creature* pCreature) : ScriptedAI(pCreature)
	{
        Reset();
	}
...
CreatureAI* GetAI_mob_panther(Creature *_Creature)
{
    return new mob_pantherAI (_Creature);
}
...
void AddSC_thousand_needles()
{
    ...
    pNewScript = new Script;
    pNewScript->Name="mob_panther";
    pNewScript->GetAI = &GetAI_mob_panther;
    pNewScript->RegisterSelf();
}
