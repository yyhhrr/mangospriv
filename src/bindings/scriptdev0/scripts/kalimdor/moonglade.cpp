...
#define GOSSIP_ITEM_THUNDER     "Ein Flug nach Thunder Bluff, bitte."
#define GOSSIP_ITEM_AQ_END      "Wisst Ihr, wo ich die andere H\303\244lfte des Anh\303\244nger der Wasserbelastbarkeit finden kann?"
...
bool GossipSelect_npc_bunthen_plainswind(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
#define GOSSIP_BEAR1 "Was wollt Ihr darstellen, Geist?"
#define GOSSIP_BEAR2 "Ich versuche die Bedeutung der St\303\244rke des K\303\266rpers zu verstehen."
#define GOSSIP_BEAR3 "Ich versuche die Bedeutung der Kraft des Herzens zu verstehen."
#define GOSSIP_BEAR4 "Ich habe deine Worte verstanden, Gro\303\237er B\303\244rengeist. Nun versuche ich deinen Segen zu lernen."
...
bool GossipSelect_npc_great_bear_spirit(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
#define GOSSIP_ITEM_RUTHERAN    "Ich m\303\266chte einen Flug nach Rut'theran Village."
#define GOSSIP_ITEM_AQ_AGI      "Wisst Ihr, wo ich die andere H\303\244lfte des Anh\303\244nger der Wasserbelastbarkeit finden kann?"
...
bool GossipSelect_npc_silva_filnaveth(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF + 1:
...
enum
{
    ...
    EMOTE_ERANIKUS_REDEEM       = -1000743,        // world emote before WotLK
    ...
    void WaypointReached(uint32 uiPointId)
    {
        switch(uiPointId)
        {
            ...
            case 17:
                if (Creature* pEranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid))
                {
					BroadcastMessage("Eranikus, Tyrant of the Dream has entered our world.", BroadcastColor::Emote);
            ...
            if (m_uiShadesummonTimer <= uiDiff)
            {
                ...
                if (m_uiSummonCount == MAX_SUMMON_TURNS)
                {
                    ...
                    if (Creature* pEranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid))
                    {
                        ...
                        pEranikus->m_movementInfo.RemoveMovementFlag(MOVEFLAG_FLYING);
...
bool EffectDummyCreature_conjure_rift(Unit* pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature*)
{
    //always check spellid and effectindex
    if (uiSpellId == SPELL_CONJURE_RIFT && uiEffIndex == EFFECT_INDEX_0)
    {
        auto* creature = pCaster->SummonCreature(NPC_ERANIKUS_TYRANT, aEranikusLocations[0].m_fX, aEranikusLocations[0].m_fY, aEranikusLocations[0].m_fZ, aEranikusLocations[0].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
		if (!creature)
			return true;
            ...
            if (m_uiEventTimer <= uiDiff)
            {
                switch(m_uiEventPhase)
                {
                    case 0:
						...
						BroadcastMessage("Eranikus, Tyrant of the Dream is wholly consumed by the Light of Elune. Tranquility sets in over the Moonglade.", BroadcastColor::Emote);
