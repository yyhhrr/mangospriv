...
SDName: Felwood
SD%Complete: 95
SDComment: Quest support: related to 4101/4102 (To obtain Cenarion Beacon), 4506, 7603 (Summon Pollo Grande), 5203, 4261
SDCategory: Felwood
EndScriptData */
...
bool EffectDummyCreature_npc_kitten(Unit*, uint32 uiSpellId, SpellEffectIndex effIndex, Creature* pCreatureTarget)
{
...
bool GossipSelect_npc_corrupt_saber(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
bool GossipSelect_npcs_riverbreeze_and_silversky(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
    {
        pPlayer->CLOSE_GOSSIP_MENU();
...
bool QuestRewarded_npc_niby_the_almighty(Player*, Creature* pCreature, Quest const* pQuest)
{
...
bool ProcessEventId_npc_kroshius(uint32 uiEventId, Object* pSource, Object*, bool)
{
...
/*####
# go_felwood_corrupted_plants
####*/

enum eFelwoodCorruptedAndCleansedPlants
{
    GO_CORRUPTED_NIGHT_DRAGON         = 174684,
    GO_CORRUPTED_SONGFLOWER           = 174713,
    GO_CORRUPTED_WINDBLOSSOM          = 174601,
    GO_CORRUPTED_WHIPPER_ROOT         = 173284,

    GO_CLEANSED_NIGHT_DRAGON          = 173325,
    GO_CLEANSED_SONGFLOWER            = 174715,
    GO_CLEANSED_WHIPPER_ROOT          = 174622,
    GO_CLEANSED_WINDBLOSSOM           = 174618,
...
bool QuestRewarded_go_felwood_corrupted_plants(Player* pPlayer, GameObject* pGo, const Quest* pQuest)
{
    ...
    switch(pGo->GetEntry())
    {
		case GO_CORRUPTED_NIGHT_DRAGON:
			if (pGo->GetGUID())
			{
				...
				pGo->SummonObject(pMap, GO_CLEANSED_NIGHT_DRAGON, CoordX, CoordY, CoordZ, CoordO);
			}
			break;
		case GO_CORRUPTED_SONGFLOWER:
			if (pGo->GetGUID())
			{
				CoordX = pGo->GetPositionX();
				CoordY = pGo->GetPositionY();
				...
				pGo->SummonObject(pMap, GO_CLEANSED_SONGFLOWER, CoordX, CoordY, CoordZ, CoordO);
			}
			break;
		case GO_CORRUPTED_WINDBLOSSOM:
			if (pGo->GetGUID())
			{
				CoordX = pGo->GetPositionX();
				CoordY = pGo->GetPositionY();
				...
				pGo->SummonObject(pMap, GO_CLEANSED_WINDBLOSSOM, CoordX, CoordY, CoordZ, CoordO);
			}
			break;
		case GO_CORRUPTED_WHIPPER_ROOT:
			if (pGo->GetGUID())
			{
				CoordX = pGo->GetPositionX();
				CoordY = pGo->GetPositionY();
				...
				pGo->SummonObject(pMap, GO_CLEANSED_WHIPPER_ROOT, CoordX, CoordY, CoordZ, CoordO);
			}
			break;
    }
...
/*######
## go_cleaned_felwood_plants
######*/

bool GOUse_go_cleansed_felwood_plants(Player* pPlayer, GameObject* pGo)
{
    ...
    switch(pGo->GetEntry())
    {
        case GO_CLEANSED_NIGHT_DRAGON:
			if (pPlayer->isAlive())
			{
				...
				/*pGo->DestroyForPlayer(pPlayer);
				pGo->SetRespawnTime(604800); //1 Woche - total dirty, da RemoveFromWorld() nicht geht.
				pGo->SaveRespawnTime();*/
				pGo->Delete();
			}
            break;
...
struct MANGOS_DLL_DECL npc_captured_arkonarinAI : public npc_escortAI
{
	...
	void WaypointReached(uint32 uiPointId) override
	{
		...
		case 41:
			...
			m_creature->SummonCreature(NPC_JAEDENAR_LEGIONNAIRE, 5082.068f, -490.084f, 296.856f, 5.15f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			m_creature->SummonCreature(NPC_JAEDENAR_LEGIONNAIRE, 5084.135f, -489.187f, 296.832f, 5.15f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			m_creature->SummonCreature(NPC_JAEDENAR_LEGIONNAIRE, 5085.676f, -488.518f, 296.824f, 5.15f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			break;
		case 43:
			SetRun(false);
			break;
		case 104:
			...
			break;
		case 105:
			m_creature->SummonCreature(NPC_SPIRT_TREY, 4844.839f, -395.763f, 350.603f, 6.25f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			break;
		case 106:
...
struct MANGOS_DLL_DECL npc_areiAI : public npc_escortAI, private DialogueHelper
{
	...
	void WaypointReached(uint32 uiPointId) override
	{
		if (uiPointId == 36)
		{
			...
			m_creature->SummonCreature(NPC_IRONTREE_STOMPER, 6573.321f, -1195.213f, 442.489f, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			m_creature->SummonCreature(NPC_IRONTREE_WANDERER, 6573.240f, -1213.475f, 443.643f, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
			m_creature->SummonCreature(NPC_IRONTREE_WANDERER, 6583.354f, -1209.811f, 444.769f, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
		}
	}
...
void AddSC_felwood()
{
    ...
    pNewScript = new Script;
    pNewScript->Name = "go_felwood_corrupted_plants";
    pNewScript->pQuestRewardedGO = &QuestRewarded_go_felwood_corrupted_plants;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "go_cleansed_felwood_plants";
    pNewScript->pGOUse = &GOUse_go_cleansed_felwood_plants;
    pNewScript->RegisterSelf();
