...
struct MANGOS_DLL_DECL npc_aged_dying_ancient_kodoAI : public ScriptedAI
{
    ...
    void SpellHit(Unit*, SpellEntry const* pSpell)
    {
...
bool pariah_manifest_spirit(Unit* pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget) {
	if(pCaster->GetTypeId() != TYPEID_PLAYER || uiSpellId != 21960) {
		return false;
	}

	if(pCaster->GetDistance(pCreatureTarget) > 20.0f) {
		Spell::SendCastResult((Player*)pCaster, uiSpellId, SpellCastResult::SPELL_FAILED_OUT_OF_RANGE);
		return false;
	}
	...
	case 13739:
	case 13738:
		pCreatureTarget->SetFactionTemporary(14);
		return true;
...
void AddSC_desolace()
{
	...
	pNewScript = new Script();
	pNewScript->Name = "quest_pariah_dummy_spirit";
	pNewScript->pEffectDummyNPC = pariah_manifest_spirit;
	pNewScript->RegisterSelf();
}
