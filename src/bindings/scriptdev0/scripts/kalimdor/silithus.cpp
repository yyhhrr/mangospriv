...
bool GossipSelect_npc_highlord_demitrian(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
bool GossipSelect_npcs_rutgar_and_frankal(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF:
...
enum
{
	...
	QUEST_LONG_FORGOTTEN_MEMORIES		= 8305,
    ...
    SPELL_CAELESTRASZ_MOLTEN_RAIN       = 25150,			// 25170, 25169
	...
	GO_AQ_DOOR_GLOW						= 66336, //invisible wall: 180322
	GO_AQ_DOOR							= 66334,
	GO_AQ_DOOR_ROOTS					= 66335,
};
...
struct MANGOS_DLL_DECL npc_anachronos_the_ancientAI : public ScriptedAI
{
    npc_anachronos_the_ancientAI(Creature* pCreature) : ScriptedAI(pCreature)
	{
		...
		GetGameObjectListWithEntryInGrid(tmplist, pCreature, 176146, 200.f);
		gos.insert(std::begin(gos), std::begin(tmplist), std::end(tmplist));

		GetGameObjectListWithEntryInGrid(tmplist, pCreature, 176147, 200.f);
		gos.insert(std::begin(gos), std::begin(tmplist), std::end(tmplist));

		GetGameObjectListWithEntryInGrid(tmplist, pCreature, 176148, 200.f);
		gos.insert(std::begin(gos), std::begin(tmplist), std::end(tmplist));
    ...
    void DoSummonWarriors(bool kaldorei)
    {
        float fX, fY, fZ;
        // Summon kaldorei warriors
		for (uint8 i = 0; i < MAX_KALDOREI && kaldorei; ++i)
        {
    ...
    void JustSummoned(Creature* pSummoned)
    {
        ...
        switch(pSummoned->GetEntry())
        {
            ...
            case NPC_QIRAJI_TANK:
				pSummoned->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_UNK_16);
                m_lQirajiWarriorsList.push_back(pSummoned->GetObjectGuid());
				pSummoned->SetCanAttackPlayer(false);
                break;
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId)
    {
        ...
        switch(uiPointId)
        {
			case POINT_ID_GATE:
				...
				for (auto& guid : m_lQirajiWarriorsList)
				{
					if (Creature* creature = m_creature->GetMap()->GetCreature(guid))
					{
						creature->CastSpell(creature, 25171, true);
					}
				}
	...
	void castSpellOnMobs(uint32 spell)
	{
		for (auto& guid : m_lQirajiWarriorsList)
		{
			if (Creature* creature = m_creature->GetMap()->GetCreature(guid))
			{
				if (creature->GetEntry() != NPC_KALDOREI_INFANTRY)
                ...
                switch(m_uiEventStage)
                {
                    ...
                    case 10:
						if (Creature* pMerithra = m_creature->GetMap()->GetCreature(m_uiMerithraGUID))
						{
							...
							pMerithra->m_movementInfo.AddMovementFlag(MOVEFLAG_FLYING);
                    ...
                    case 15:
                        if (Creature* pArygos = m_creature->GetMap()->GetCreature(m_uiArygosGUID))
						{
							...
							pArygos->m_movementInfo.AddMovementFlag(MOVEFLAG_FLYING);
						}
                        m_uiEventTimer = 4000;
                        break;
                    case 16:
                    ...
                    case 21:
						if (Creature* pCaelestrasz = m_creature->GetMap()->GetCreature(m_uiCaelestraszGUID))
						{
							...
							pCaelestrasz->m_movementInfo.AddMovementFlag(MOVEFLAG_FLYING);
                    ...
                    case 23:
                        if (Creature* pCaelestrasz = m_creature->GetMap()->GetCreature(m_uiCaelestraszGUID))
						{
							...
							castSpellOnMobs(25170);
							castSpellOnMobs(25169);
						}
                        m_uiEventTimer = 6000;
                        break;
                    case 24:
					...
					case 29:
						OnTimeStop();
						m_creature->CastSpell(m_creature, SPELL_PRISMATIC_BARRIER, true);
                    ...
                    case 42:
                        if (Creature* pFandral = m_creature->GetMap()->GetCreature(m_uiFandralGUID))
                        {
							pFandral->m_movementInfo.AddMovementFlag(MOVEFLAG_WALK_MODE);
	...
	void OnTimeStop()
	{
		for (auto& guid : m_lQirajiWarriorsList)
		{
			if (Creature* creature = m_creature->GetMap()->GetCreature(guid))
			{
				creature->CastSpell(creature, 23973, true);
				creature->CastSpell(creature, 22579, true);
				creature->setAttackEnabled(false);
			}
		}
	}
};
...
bool QuestAcceptGO_crystalline_tear(Player* pPlayer, GameObject* pGo, const Quest* pQuest)
{
	...
	if (pQuest->GetQuestId() == QUEST_A_PAWN_ON_THE_ETERNAL_BOARD && globalEventRunning == false)
	{
...
bool GoUse_crystalline_tear(Player* plr, GameObject*)
{
	if (!plr->GetQuestRewardStatus(QUEST_LONG_FORGOTTEN_MEMORIES))
		plr->CompleteQuest(QUEST_LONG_FORGOTTEN_MEMORIES);
...
/*
    ...
    SPELL_PRISMATIC_BARRIER             = 25159,            // Sends event 9425 - should activate Go 176146
    SPELL_CALL_ANCIENTS                 = 25167,            // Sends event 9426 - should activate Go 176147
	GO_AQ_DOOR_GLOW						= 329946, //invisible wall: 180322
	GO_AQ_DOOR							= 66334,
	GO_AQ_DOOR_ROOTS					= 66335,
*/

bool OnEvent(uint32 eventId, Object* pSource, Object* pTarget, bool isStart)
{
...
//	void GetGameObjectListWithEntryInGrid(std::list<GameObject*>& lList, WorldObject* pSource, uint32 uiEntry, float fMaxSearchRange)
	...
	auto unit = static_cast<Unit*>(pSource);
	ObjectGuid guid;

	uint32 entry = 0;
	if (eventId == 9425)
		entry = 176146;
	else if (eventId == 9426)
		entry = 176147;
	else if (eventId == 9427)
		entry = 176148;

	std::list<GameObject*> gos;
	GetGameObjectListWithEntryInGrid(gos, unit, entry, 50.f);
