...
bool GossipSelect_npc_lorax(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
    switch(uiAction)
    {
...
bool GossipSelect_npc_rivern_frostwind(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
bool GossipSelect_npc_witch_doctor_mauari(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
static const DialogueEntry aIntroDialogue[] =
{
    ...
    {SAY_QUEST_END_2,        NPC_RANSHALLA,        30000},
	{DATA_ESCORT_END,		 0,					   0},
    {0, 0, 0},
};
...
bool QuestAccept_npc_ranshalla(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_GUARDIANS_ALTAR)
    {
        ...
        if (npc_ranshallaAI* pEscortAI = dynamic_cast<npc_ranshallaAI*>(pCreature->AI())) {
			...
			pEscortAI->SetPreventReset(true);
		}
...
bool GOUse_go_elune_fire(Player* , GameObject* pGo)
{
...
enum
{
	...
	NPC_DOCTOR_WEAVILS_FLYING_MACHINE = 15553,
	NPC_NUMBER_TWO = 15554,

	QUEST_DECOY = 8606,

	GO_BAG_OF_GOLD = 180660,
...
const float macheineSpawnX = 5086.10f, macheineSpawnY = -5160.f, macheineSpawnZ = 940.f;
const float landingX = 5089.84f, landingY = -5085.35f, landingZ = 922.63f;
const float hillTopX = 5086.10f, hillTopY = -5115.40f, hillTopZ = 931.17f;
const float deSpawnX = 5086.21f, deSpawnY = -5051.95f, deSpawnZ = 930.0f;
const float numberTwoSpawnX = 5103.16f, numberTwoSpawnY = -5088.73f, numberTwoSpawnZ = 926.20f;

struct MANGOS_DLL_DECL npc_flying_machine : public ScriptedAI
{
	Creature* mWeavil = nullptr;
	GameObject* mBagOfGold = nullptr;
	Player* mOwner = nullptr;

	uint32 mStepTimer = 0;
	uint32 mStep = 0;

	npc_flying_machine(Creature* pCreature) : ScriptedAI(pCreature)	{}
	...
	void MovementInform(uint32, uint32 point) override
	{
		if(point == 1)
		{
			mWeavil = m_creature->SummonCreature(NPC_DOCTOR_WEAVIL, landingX, landingY, landingZ, 4.5f, TEMPSUMMON_MANUAL_DESPAWN, 0);
			mWeavil->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
			mStepTimer = 1000;
		} else if (point == 2)
			static_cast<TemporarySummon*>(m_creature)->UnSummon();
	}

	void SummonedMovementInform(Creature* creature, uint32, uint32 point) override
	{
		if (creature->GetEntry() != NPC_DOCTOR_WEAVIL)
			return;

		if (point == 0)
		{
			creature->SetFacingTo(1.39f);

			std::list<GameObject*> bags;
			GetGameObjectListWithEntryInGrid(bags, creature, GO_BAG_OF_GOLD, 10.f);
			for (auto bag : bags)
			{
				bag->RemoveFromWorld();
				break;
		...
		{
			static_cast<TemporarySummon*>(mWeavil)->UnSummon();
			m_creature->GetMotionMaster()->MovePoint(2, deSpawnX, deSpawnY, deSpawnZ, false);
		}
	}
	...
	void UpdateAI(uint32 timeDelta) override
	{
		...
		if (mStepTimer > timeDelta)
		{
			mStepTimer -= timeDelta;
			return;
		}

		if(mStep == 0)
		{
			mWeavil->GetMotionMaster()->MovePoint(0, hillTopX, hillTopY, hillTopZ);
			mStepTimer = 0;
		} else if (mStep == 1)
		{
			DoScriptText(SAY_WEAVIL1, mWeavil);
			mStepTimer = 8000;
		} else if (mStep == 2)
		{
			DoScriptText(SAY_WEAVIL2, mWeavil);
			mStepTimer = 10000;
		} else if (mStep == 3)
		{
			DoScriptText(SAY_WEAVIL3, mWeavil);
			mStepTimer = 5000;
		} else if (mStep == 4)
		{
			mWeavil->m_movementInfo.RemoveMovementFlag(MOVEFLAG_WALK_MODE);
			mWeavil->GetMotionMaster()->MovePoint(1, landingX, landingY, landingZ);
			auto numberTwo = mWeavil->SummonCreature(NPC_NUMBER_TWO, numberTwoSpawnX, numberTwoSpawnY, numberTwoSpawnZ, 3.4f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
			numberTwo->AI()->AttackStart(mOwner);
			mOwner->CompleteQuest(QUEST_DECOY);
			mOwner->SendQuestCompleteEvent(QUEST_DECOY);
			mStepTimer = 0;
		}
...
bool OnEvent_PlaceBagOfGold(uint32 eventId, Object* pSource, Object* pTarget, bool isStart)
{
	...
	if (!plr->HasAura(25688))
		return false;

	auto summon = plr->SummonCreature(NPC_DOCTOR_WEAVILS_FLYING_MACHINE, macheineSpawnX, macheineSpawnY, macheineSpawnZ, 0.f, TEMPSUMMON_MANUAL_DESPAWN, 0);
	if (summon == nullptr)
		return true;

	summon->m_movementInfo.AddMovementFlag(MOVEFLAG_FLYING);
	summon->m_movementInfo.RemoveMovementFlag(MOVEFLAG_FALLING);
	summon->m_movementInfo.RemoveMovementFlag(MOVEFLAG_WALK_MODE);
	summon->addUnitState(UNIT_STAT_IGNORE_PATHFINDING);
	summon->GetMotionMaster()->MovePoint(1, landingX, landingY, landingZ, false);

	auto ai = static_cast<npc_flying_machine*>(summon->AI());
	if (ai)
	{
		ai->mBagOfGold = static_cast<GameObject*>(pTarget);
		ai->mOwner = plr;
	}
...
void AddSC_winterspring()
{
	...
	pNewScript = new Script;
	pNewScript->Name = "AQ_PLACE_LOOT";
	pNewScript->pProcessEventId = &OnEvent_PlaceBagOfGold;
	pNewScript->RegisterSelf();


	pNewScript = new Script;
	pNewScript->Name = "npc_aq_weavil_flying_machine";
	pNewScript->GetAI = [](Creature* cr) -> CreatureAI*
	{
		return new npc_flying_machine(cr);
	};
	pNewScript->RegisterSelf();
}
