...
bool GossipSelect_npc_lady_jaina_proudmoore(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
struct MANGOS_DLL_DECL npc_morokkAI : public npc_escortAI
{
    ...
    void UpdateEscortAI(const uint32)
    {
...
bool GossipSelect_npc_nat_pagle(Player* pPlayer, Creature* pCreature, uint32, uint32 uiAction)
{
...
bool QuestAccept_npc_private_hendel(Player*, Creature* pCreature, const Quest* pQuest)
{
...
enum
{
    SPELL_FLAMMENATEM   = 9573,
    SPELL_SPALTEN       = 20677,
	SPELL_ENRAGE		= 8269,
	SPELL_QUEST_CAST	= 16054
};

struct MANGOS_DLL_DECL mob_aschenschwingeAI : public ScriptedAI
{
    mob_aschenschwingeAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }

    uint32 m_uiFlammenatem_Timer;
    uint32 m_uiSpalten_Timer;
	//uint32 m_uiEnrage_Timer;
	uint32 m_uiQuestcast_Timer;

    void Reset()
    {
        m_uiFlammenatem_Timer	= 5000;
        m_uiSpalten_Timer		= 7000;
		//m_uiEnrage_Timer		= 1000;
		m_uiQuestcast_Timer		= 2000;
    }

    void Aggro(Unit*)
    {
		m_creature->MonsterYell("Oh, etwas zu fressen!",LANG_UNIVERSAL,NULL);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        // Flammenatem
        if (m_uiFlammenatem_Timer < uiDiff)
        {
            DoCastSpellIfCan(m_creature->getVictim(), SPELL_FLAMMENATEM);
            m_uiFlammenatem_Timer = 8000;
        }
        else
            m_uiFlammenatem_Timer -= uiDiff;

        // Spalten
        if (m_uiSpalten_Timer < uiDiff)
        {
            DoCastSpellIfCan(m_creature->getVictim(), SPELL_SPALTEN);
            m_uiSpalten_Timer = 6000;
        }
        else
            m_uiSpalten_Timer -= uiDiff;

        //Quest Cast bei 10%
        if (m_creature->GetHealthPercent() < 10.0f)
        {
            if (m_uiQuestcast_Timer < uiDiff)
            {
                DoCastSpellIfCan(m_creature, SPELL_QUEST_CAST);
                m_uiQuestcast_Timer = 5000;
            }
            else
                m_uiQuestcast_Timer -= uiDiff;
        }
...
CreatureAI* GetAI_mob_aschenschwinge(Creature* pCreature)
{
    return new mob_aschenschwingeAI(pCreature);
}
...
struct MANGOS_DLL_DECL npc_stinki_ignatzAI : public npc_escortAI
{
    npc_stinki_ignatzAI(Creature* c) : npc_escortAI(c) {}
...
bool QuestAccept_npc_stinki_ignatz(Player* pPlayer, Creature* pCreature, Quest const* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_ESCAPE_H || pQuest->GetQuestId() == QUEST_ESCAPE_A)
    {
		DoScriptText(SAY_BEGIN, pCreature, pPlayer);

        if (npc_stinki_ignatzAI* pEscortAI = dynamic_cast<npc_stinki_ignatzAI*>(pCreature->AI()))
...
CreatureAI* GetAI_npc_stinki_ignatzAI(Creature* pCreature)
{
    return new npc_stinki_ignatzAI(pCreature);
}  
...
enum 
{
	SPELL_MENTAL_DOMINATION = 25772,
	SPELL_WILL_OF_WEAVIL = 25773,
	SPELL_MIND_SHATTER = 25774,
};


struct MANGOS_DLL_DECL npc_dr_weavil : public ScriptedAI
{
	uint32 mMentalDominationTimer = 30 * IN_MILLISECONDS;
	uint32 mMindShatterTimer = 20 * IN_MILLISECONDS;

	npc_dr_weavil(Creature* pCreature) : ScriptedAI(pCreature)	{}

	void Reset() override
	{
		mMentalDominationTimer = 30 * IN_MILLISECONDS;
		mMindShatterTimer = 20 * IN_MILLISECONDS;
	}

	void UpdateAI(uint32 delta) override
	{
		auto* victim = m_creature->getVictim();
		if (!m_creature->SelectHostileTarget() || !victim)
			return;


		if(mMentalDominationTimer < delta)
		{
			mMentalDominationTimer = 2 * MINUTE * IN_MILLISECONDS;
			m_creature->CastSpell(victim, SPELL_MENTAL_DOMINATION, true);
			m_creature->CastSpell(victim, SPELL_WILL_OF_WEAVIL, true);

		} else
			mMentalDominationTimer -= delta;


		if (mMindShatterTimer < delta)
		{
			mMindShatterTimer = 20 * IN_MILLISECONDS;
			m_creature->CastSpell(m_creature, SPELL_MIND_SHATTER, true);
		} else
			mMindShatterTimer -= delta;
	}
};
...
void AddSC_dustwallow_marsh()
{
	...
	pNewScript = new Script;
    pNewScript->Name = "mob_aschenschwinge";
    pNewScript->GetAI = &GetAI_mob_aschenschwinge;
	pNewScript->RegisterSelf();

	pNewScript = new Script;
	pNewScript->Name = "npc_stinki_ignatz";
	pNewScript->GetAI = &GetAI_npc_stinki_ignatzAI;
	pNewScript->pQuestAcceptNPC = &QuestAccept_npc_stinki_ignatz;
	pNewScript->RegisterSelf();

	pNewScript = new Script;
	pNewScript->Name = "NPC_DR_WEAVIL";
	pNewScript->GetAI = [](Creature* c) -> CreatureAI* { return new npc_dr_weavil(c); };
	pNewScript->RegisterSelf();
}
