...
SpellEntry const* ScriptedAI::SelectSpell(Unit* pTarget, int32, int32 uiMechanic, SelectTarget selectTargets, uint32 uiPowerCostMin, uint32 uiPowerCostMax, float fRangeMin, float fRangeMax, SelectEffect selectEffects)
{
...
//            continue;
        ...
        if (uiMechanic >= 0 && pTempSpell->Mechanic != static_cast<uint32>(uiMechanic))
            continue;
...
{
	return m_creature->CanCast(pTarget, pSpellEntry->Id, bTriggered);
}
...
void ScriptedAI::SetEquipmentSlots(bool bLoadDefault, uint32 iMainHand, uint32 iOffHand, uint32 iRanged)
{
    if (bLoadDefault)
    {
