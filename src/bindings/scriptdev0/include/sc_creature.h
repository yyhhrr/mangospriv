...
struct MANGOS_DLL_DECL ScriptedAI : public CreatureAI
{
	...
	//Called before entering evade mode. Return wheter or not you wish to enter the evade mode
	bool PreEnterEvadeMode() { return true; }
	...
	void DoResetThreatIf(FUN&& f)
	{
		...
		for (ThreatList::const_iterator itr = tList.begin(); itr != tList.end(); ++itr)
		{
			...
			if (pUnit && m_creature->getThreatManager().getThreat(pUnit) && std::forward<FUN>(f)(pUnit))
    ...
    void SetEquipmentSlots(bool bLoadDefault, uint32 iMainHand = EQUIP_NO_CHANGE, uint32 iOffHand = EQUIP_NO_CHANGE, uint32 iRanged = EQUIP_NO_CHANGE);
