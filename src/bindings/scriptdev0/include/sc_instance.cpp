...
void DialogueHelper::StartNextDialogueText(int32 iTextEntry)
{
    ...
    if (!bFound)
    {
        error_log("SD0: Script call DialogueHelper::StartNextDialogueText, but textEntry %i is not in provided dialogue (on map id %u)", iTextEntry, m_pInstance ? m_pInstance->instance->GetId() : 0);
        return;
    }
