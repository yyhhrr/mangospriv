        ...
        if (entry->IsRaid())
        {
            ...
            if(!player->isGameMaster() && !sWorld.getConfig(CONFIG_BOOL_INSTANCE_IGNORE_RAID))
            {
                ...
                if (!group || !group->isRaidGroup())
                {
                    ...
                    if (player->GetSession()->GetSessionDbcLocale() == LOCALE_deDE)
                        player->GetSession()->SendAreaTriggerMessage("Ihr m\303\274sst einem Schlachtzug angeh\303\266ren, um diese Instanz zu betreten.");
                    else
...
MapManager::Update(uint32 diff)
{
    ...
    while(iter != i_maps.end())
    {
        ...
        if(pMap && pMap->CanUnload((uint32)i_timer.GetCurrent()))
        {
            pMap->UnloadAll(true);
            delete pMap;
