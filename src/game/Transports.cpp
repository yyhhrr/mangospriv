...
bool Transport::Create(uint32 guidlow, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress)
{
    ...
    SetGoState(GO_STATE_READY, false);
...
bool Transport::AddPassenger(Player* passenger)
{
	if (m_passengers.find(passenger) == m_passengers.end())
    {
		...
		if (passenger->IsMounted() && GetEntry() == 20808 || GetEntry() == 176231 || GetEntry() == 176244 || GetEntry() == 176310 || GetEntry() == 177233)
			passenger->RemoveSpellsCausingAura(SPELL_AURA_MOUNTED);
