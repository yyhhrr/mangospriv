...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (!pItem->IsTargetValidForItemUse(targets.getUnitTarget()))
    {
        ...
        for(int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
        {
			if (proto->Spells[i].SpellId == 25720 && !pUser->HasAura(25688))
			{
				if (SpellEntry const* spellInfo = sSpellStore.LookupEntry(proto->Spells[i].SpellId))
					Spell::SendCastResult(_player, spellInfo, SPELL_FAILED_NOT_READY);
				return;
			}
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    else if(mover->GetTypeId() == TYPEID_UNIT)
    {
        // not have spell in spellbook or spell passive and not casted by client
		...
		sLog.outError("World: HandleCastSpellOpcode - weird mover type id received from %s. %u", _player ? _player->GetName() : "unk", mover->GetTypeId());
    ...
    // Specify Target for Arcane Missiles
    if (spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && spellInfo->SpellFamilyFlags & UI64LIT(0x00000800))
        if (Unit *target = ObjectAccessor::Instance().GetUnit(*_player, _player->GetTargetGuid()))
            targets.setUnitTarget(target);

	auto spell = std::make_shared<Spell>(_player, spellInfo, false);
...
void WorldSession::HandleCancelAuraOpcode( WorldPacket& recvPacket)
{
    ...
    {
        if (std::shared_ptr<Spell> curSpell = _player->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
...
void WorldSession::HandleCancelAutoRepeatSpellOpcode( WorldPacket& /*recvPacket*/)
{
    ...
    _player->GetMover()->InterruptSpell(CURRENT_AUTOREPEAT_SPELL, true, true);
}
