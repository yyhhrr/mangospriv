...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADFROM,            "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags,"
        ...
        "health, power1, power2, power3, power4, power5, exploredZones, equipmentCache, ammoId, actionBars, honor_current_ranking, isPvpFlagged FROM characters WHERE guid = '%u'", m_guid.GetCounter());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
	...
	if (pCurrChar->GetMap()->IsDungeon() && GetSecurity() == SEC_PLAYER)
		pCurrChar->CastCustomSpell(pCurrChar, 5727, 0, 0, 0, true);
	...
	/*QueryResult *result = WorldDatabase.Query("SELECT progress FROM world_events WHERE id = '1';");
	Field *fields = result->Fetch();
	...
	chH.PSendSysMessage("Willkommen auf dem Classic-WoW Blizzlike Server!");
	chH.PSendSysMessage("Alle Neuigkeiten findest du immer auf: www.classic-wow.org");
	if (pCurrChar->GetSession()->IsPremium())
	{
		chH.PSendSysMessage("Deine Erfahrungsrate kannst du mit .rate \303\244ndern.");
		chH.PSendSysMessage("Die Schnellreise-Funktion kannst du mit .schnellreise \303\244ndern.");
		pCurrChar->setXPRate(2);
		pCurrChar->setInstantFlight(1);
		pCurrChar->SetTaxiCheater(true);
		pCurrChar->UpdateSkillsToMaxSkillsForLevel();
	}

	//chH.PSendSysMessage("Das Zul'Gurub Opening Event ist zu %u Prozent abgeschlossen.", progress);

	chH.PSendSysMessage("Last Core Update: [31.03.2015] Version: [1.0.6]");
	if (pCurrChar->GetSession()->IsPremium())
		chH.PSendSysMessage("Account Stufe: [Premium]");
	else
		chH.PSendSysMessage("Account Stufe: [Normal]");
	...
	if (pCurrChar->isGameMaster() && !pCurrChar->isGMVisible())
