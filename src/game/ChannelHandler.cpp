...
void WorldSession::HandleJoinChannelOpcode(WorldPacket& recvPacket)
{
	...
	if(channelname == "world" && GetSecurity() >= SEC_MODERATOR)
	{
		ChannelMgr* cMgr = channelMgr(ALLIANCE);
		if(!cMgr)
			return;

		if(auto* channel = cMgr->GetJoinChannel(channelname, 5))
			channel->Join(_player->GetObjectGuid(), pass.c_str());

		cMgr = channelMgr(HORDE);
		if(!cMgr)
			return;
