...
#include "Warden-System/WardenDataStorage.h"
...
World::World()
{
	...
	m_PremiumAccountCheckTimer = 360000;
...
void World::LoadConfigSettings(bool reload)
{
	...
	setConfig(CONFIG_UINT32_SKILL_GAIN_CRAFTING_PREMIUM, "SkillGain.Crafting.Premium", 1);
	setConfig(CONFIG_UINT32_SKILL_GAIN_GATHERING_PREMIUM, "SkillGain.Gathering.Premium", 1);
	setConfig(CONFIG_UINT32_INSTANT_FLIGHT_PREMIUM, "InstantFlightPaths", 0);
    ...
    // DosProtection
    setConfig(CONFIG_UINT32_PACKET_SPOOF_POLICY, "PacketSpoof.Policy", (uint32)WorldSession::DosProtection::POLICY_KICK);
    setConfig(CONFIG_UINT32_PACKET_SPOOF_BANMODE, "PacketSpoof.BanMode", (uint32)BAN_ACCOUNT);
    setConfig(CONFIG_UINT32_PACKET_SPOOF_BANDURATION, "PacketSpoof.BanDuration", 86400);
}
...
void World::SetInitialWorldSettings()
{
	...
	if (!MapManager::ExistMapAndVMap(0,-6240.32f, 331.033f) ||
        //!MapManager::ExistMapAndVMap(0,-8949.95f,-132.493f) ||		//TODO: same as below. bug?
    ...
    sLog.outString("Loading Player Hints...");
    sObjectMgr.LoadHintText();
    ...
    sLog.outString("Loading Spell Priority Data...");
    sSpellMgr.LoadSpellPriorities();
    ...
    sLog.outString("Loading Creature Resilience Data...");
    sObjectMgr.LoadCreatureResilience();
    ...
    m_timers[WUPDATE_AQ_STATE].SetInterval(MINUTE*IN_MILLISECONDS);
	...
	sLog.outString("Cleaning up tracking tables...");
	CharacterDatabase.PExecute("DELETE FROM character_bosskill WHERE DATEDIFF(CURRENT_TIMESTAMP, date) > 7;");
	CharacterDatabase.PExecute("DELETE FROM character_receiveditem WHERE DATEDIFF(CURRENT_TIMESTAMP, date) > 3;");
	CharacterDatabase.PExecute("DELETE FROM character_destroyeditem WHERE DATEDIFF(CURRENT_TIMESTAMP, date) > 3;");

	sLog.outString("Cleaning up warden tables...");
	CharacterDatabase.PExecute("DELETE FROM character_warden WHERE DATEDIFF(CURRENT_TIMESTAMP, timestamp) > 30;");
...
{
    if (diff > 500 && uint32(m_gameTime - m_startTime) > 36000)
    {
        m_lagCounter += diff - 500;
        // detected more than 60 seconds lag within 20 minutes
        if (!m_ShutdownTimer && m_lagCounter >= 60000)
    ...
    if (m_timers[WUPDATE_AQ_STATE].Passed())
    {
        m_timers[WUPDATE_AQ_STATE].Reset();
        UpdateWorldState();
    }
	...
	if (m_MaintenanceTimeChecker < diff)
	{
		...
		m_MaintenanceTimeChecker = 60000; // alle 10 minuten
	}
	else
		m_MaintenanceTimeChecker -= diff;*/

	// Premium Account: alle 15 Minuten prüfen.
	if (m_PremiumAccountCheckTimer < diff)
	{
		LoginDatabase.Execute("UPDATE account_premium SET active = 0 WHERE unsetdate<=UNIX_TIMESTAMP() AND unsetdate<>setdate;");
		m_PremiumAccountCheckTimer = 15000;
	}
	else
		m_PremiumAccountCheckTimer -= diff;
...
void World::ServerMaintenanceStart()
{
	...
	for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
	{
		if (itr->second->GetPlayer() && itr->second->GetPlayer()->IsInWorld())
		{
			...
			ChatHandler(itr->second->GetPlayer()).SendSysMessage("[SERVER] Die Berechnung der Ehre ist abgeschlossen!");
		}
	}
...
uint32 const AQStateFields[30][4] =
{
    //stateID, targetID, maxQuestCnt, stackSize
    { 2018, 1998, 4500, 20 },   //H COPPER BAR
    { 1997, 0, 4500, 20 },      //A COPPER BAR
    { 2053, 2055, 1300, 20 },   //A PURPLE LOTUS
    { 2054, 0, 1300, 20 },      //H PURPLE LOTUS
    { 2069, 2071, 8000, 10 },   //A THICK LEATHER
    { 2070, 0, 8000, 10 },      //H THICK LEATHER
    { 2101, 2103, 850, 20 },    //A SPOTTED YELLOWTAIL
    { 2102, 0, 850, 20 },       //H SPOTTED YELLOWTAIL
    { 2088, 2090, 20000, 20 },  //A RUNECLOTH BANDAGE
    { 2089, 0, 20000, 20 },     //H RUNECLOTH BANDAGE
    { 2002, 2003, 1400, 20 },   //A IRON BAR
    { 2011, 2012, 1200, 20 },   //A THORIUM BAR
    { 2057, 2058, 1000, 20 },   //A ARTHAS TEARS
    { 2047, 2048, 1650, 20 },   //A STRANGLEKELP
    { 2060, 2061, 18000, 10 },  //A LIGHT LEATHER
    { 2063, 2064, 11000, 10 },  //A MEDIUM LEATHER
    { 2098, 2099, 1000, 20 },   //A ROAST RAPTOR
    { 2092, 2093, 700, 20 },    //A RAINBOW FIN ALBACORE
    { 2076, 2077, 40000, 20 },  //A LINEN BANDAGE
    { 2082, 2083, 30000, 20 },  //A SILK BANDAGE
    { 2005, 2006, 1100, 20 },   //H TIN BAR
    { 2008, 2009, 900, 20 },    //H MITHRIL BAR
    { 2021, 2020, 4800, 20 },   //H PEACEBLOOM
    { 2050, 2051, 950, 20 },    //H FIREBLOOM
    { 2066, 2067, 6000, 10 },   //H HEAVY LEATHER
    { 2073, 2074, 6000, 10 },   //H RUGGED LEATHER
    { 2095, 2096, 500, 20 },    //H LEAN WOLF STEAK
    { 2105, 2106, 500, 20 },    //H BAKED SALMON
    { 2079, 2080, 12500, 20 },  //H WOOL BANDAGE
    { 2085, 2086, 12500, 20 },  //H MAGEWEAVE BANDAGE
};

void World::SendWorldState(WorldSession *plr)
{
    for (int i = 0; i < 30; i++)
    {
        WorldPacket data;
        if (AQStateFields[i][1])
        {
            sBattleGroundMgr.BuildUpdateWorldStatePacket(&data, AQStateFields[i][1], AQStateFields[i][2] * AQStateFields[i][3]);
            plr->SendPacket(&data);
        }

        sBattleGroundMgr.BuildUpdateWorldStatePacket(&data, AQStateFields[i][0], m_WorldState[i] * AQStateFields[i][3]);
        plr->SendPacket(&data);
    }
}
...
{
	QueryResult *result = WorldDatabase.Query("SELECT progress, id FROM world_events WHERE id >= 3 AND id < 33");
    ...
    while (result->NextRow())
    {
        ...
        uint32 index = fields[1].GetUInt32() - 3;

        uint32 progress = fields[0].GetUInt32();

        if (m_WorldState[index] != progress)
        {
            m_WorldState[index] = progress;

            if (m_WorldState[index] > AQStateFields[index][2])
                m_WorldState[index] = AQStateFields[index][2];
        }
	}
	delete result;

    result = WorldDatabase.Query("SELECT progress, id FROM world_events WHERE id >= 42 AND id < 45");
    ...
    while (result->NextRow())
    {
        ...
        uint32 index = fields[1].GetUInt32() - 11;
