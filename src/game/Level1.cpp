...
bool ChatHandler::HandleHarveyAccountAdd(char* args) {
	...
	if(!(strm >> plrName) || plrName.length() == 0) {
		return false;
	}

	std::transform(plrName.begin(), plrName.end(), plrName.begin(), tolower);
	plrName[0] = toupper(plrName[0]);

	strm >> duration;

	bool temp = duration != 0;
	auto player = sObjectAccessor.FindPlayerByName(plrName.c_str());
	if(player == nullptr) {
		std::stringstream errStrm;
		errStrm << "Player '" << plrName << "' not found.";
		SendSysMessage(errStrm.str().c_str());
		return true;
	}

	sHarvey.addAccount(player->GetSession()->GetAccountId(), temp, duration);
	std::stringstream notify;
	notify << "Added Harvey (account) to player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}

bool ChatHandler::HandleHarveyAccountRemove(char* args) {
	if(args == nullptr || strlen(args) == 0) {
		return false;
	}
	...
	sHarvey.removeAccount(player->GetSession()->GetAccountId());
	std::stringstream notify;
	notify << "Removed Harvey (account) from player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}

bool ChatHandler::HandleHarveyCharacterAdd(char* args) {
	if(args == nullptr || strlen(args) == 0) {
		return false;
	}
	...
	sHarvey.addCharacter(player->GetGUID(), temp, duration);
	std::stringstream notify;
	notify << "Added Harvey (character only) to player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}

bool ChatHandler::HandleHarveyCharacterRemove(char* args) {
	if(args == nullptr || strlen(args) == 0) {
		return false;
	}
	...
	sHarvey.removeCharacter(player->GetGUID());
	std::stringstream notify;
	notify << "Removed Harvey (character only) from player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}

bool ChatHandler::HandleHarveyIPAdd(char* args) {
	if(args == nullptr || strlen(args) == 0) {
		return false;
	}
	...
	sHarvey.addIP(player->GetSession()->GetRemoteAddress(), temp, duration);
	std::stringstream notify;
	notify << "Added Harvey (IP) to player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}

bool ChatHandler::HandleHarveyIPRemove(char* args) {
	if(args == nullptr || strlen(args) == 0) {
		return false;
	}
	...
	sHarvey.removeIP(player->GetSession()->GetRemoteAddress());
	std::stringstream notify;
	notify << "Removed Harvey (IP) from player '" << plrName << "'.";
	SendSysMessage(notify.str().c_str());
	return true;
}
...
bool ChatHandler::HandleGPSCommand(char* args)
{
	...
	std::ofstream logFile("gps.txt", std::ios::app);
	if (logFile)
	{
		logFile << obj->GetPositionX() << "f, " << obj->GetPositionY() << "f, " << obj->GetPositionZ() << "f" << "\n";
	}
...
bool ChatHandler::HandleModifyMountCommand(char* args)
{
    ...
    chr->SendMessageToSet( &data, true );
	*/
	chr->SetSpeedRate(UnitMoveType::MOVE_RUN, 2);
    return true;
}
...
bool ChatHandler::HandleGoHelper( Player* player, uint32 mapid, float x, float y, float const* zPtr, float const* ortPtr)
{
    ...
    player->TeleportTo(mapid, x, y, z + 3.0f, ort);
...
bool ChatHandler::HandleLookupDestroyedItemCommand(char* args)
{
	auto* plr = getSelectedPlayer();
	if(!plr)
	{
		SendSysMessage("You have to select a player!");
		SetSentErrorMessage(true);
		return false;
	}

	uint32 id = 0;
	if (!ExtractOptUInt32(&args, id, 0))
	{
		SendSysMessage(".tracking listdestroyed <optional item id>");
		SetSentErrorMessage(true);
		return false;
	}
	...
	QueryResult* result;
	if(id)
		result = CharacterDatabase.PQuery("SELECT * FROM character_destroyeditem WHERE itemId = %u AND guid = %I64u ORDER BY date", id, plr->GetGUID());
	else
		result = CharacterDatabase.PQuery("SELECT * FROM character_destroyeditem WHERE guid = %I64u ORDER BY date", plr->GetGUID());

	if(!result)
	{
		SendSysMessage("The player did not destroy that item");
		SetSentErrorMessage(true);
		return true;
	}

	SendSysMessage("Destroyed Items:");
	do
	{
		...
		{
			delete result;
			SendSysMessage("Item does not exist");
			SetSentErrorMessage(true);
			return true;
		}
		...
		if (plr)
		{
			if (plr->CanUseItem(itemProto))
				usableStr = GetMangosString(LANG_COMMAND_ITEM_USABLE);
		}

		char* action = "";
		switch (fields[3].GetUInt32())
		{
		case 0: action = "Traded"; break;
		case 1: action = "Mail"; break;
		case 2: action = "Sold"; break;
		case 3: action = "Destroyed"; break;
		}

		PSendSysMessage("%d - |cffffffff|Hitem:%d:0:0:0:0:0:0:0|h[%s]|h|r %s Amount: %u Action: %s Date: %s", id, id, itemProto->Name1, usableStr, fields[2].GetUInt32(), action, fields[4].GetString());
...
bool ChatHandler::HandleLookupReceivedItemCommand(char* args)
{
	auto* plr = getSelectedPlayer();
	if (!plr)
	{
	...
	if (!ExtractOptUInt32(&args, id, 0))
	{
		SendSysMessage(".tracking listreceived <optional item id>");
		SetSentErrorMessage(true);
		return false;
	}
	...
	QueryResult* result;
	if (id)
		result = CharacterDatabase.PQuery("SELECT * FROM character_receiveditem WHERE itemId = %u AND guid = %I64u ORDER BY date", id, plr->GetGUID());
	else
		result = CharacterDatabase.PQuery("SELECT * FROM character_receiveditem WHERE guid = %I64u ORDER BY date", plr->GetGUID());

	if (!result)
	{
		SendSysMessage("The player did not receive that item");
		SetSentErrorMessage(true);
		return true;
	}

	SendSysMessage("Received Items:");
	do
	{
		auto* fields = result->Fetch();
		...
		PSendSysMessage("%d - |cffffffff|Hitem:%d:0:0:0:0:0:0:0|h[%s]|h|r %s Amount: %u Date: %s", id, id, itemProto->Name1, usableStr, fields[2].GetUInt32(), fields[3].GetString());
...
std::vector<std::vector<std::string>> bosses =
{
	/*Molten Core*/
	{ "Lucifron", "Magmadar", "Gehennas", "Garr", "Geddon", "Shazzrah", "Sulfuronherold", "Golemagg", "Majordomus", "Ragnaros" },

	{ "Onyxia" },

	{ "Razorgore", "Vaelstrasz", "Lashlayer", "Firemaw", "Ebonroc", "Flamegor", "Chromaggus", "Nefarian" },

	{ "Jeklik", "Venoxis", "Mandokir", "Marli", "EdgeOfMadness", "Thekal", "Gahzranka", "Arlokk", "Jindo", "Hakkar" },

	{ "Kurinaxx", "Rajaxx", "Moam", "Buru", "Ayamiss", "Ossiran" },

	{ "Skeram", "Yauj", "Sartura", "Fankriss", "Huhuran", "Viscidus", "Imperators", "Ouro", "CThun" },

	{ "AnubRekhan", "Faerlina", "Maexxna", "Patchwerk", "Grobbulus", "Gluth", "Thaddius", "Noth", "Heigan", "Loatheb", "Razuvious", "Gothik", "FourHorsemen", "Sapphiron", "KelThuzad" },

	{ "Azuregos", "LordKazzak", "Emeriss", "Lethon", "Ysondre", "Taerar" },
};

bool ChatHandler::HandleLookupKilledBossesCommand(char* args)
{
	auto* plr = getSelectedPlayer();
	if (!plr)
	{
	...
	if (!ExtractOptUInt32(&args, id, 0))
	{
		SendSysMessage(".tracking listbosses <optional raid id>");
		SetSentErrorMessage(true);
		return false;
	}
	...
	QueryResult* result;
	if (id)
		result = CharacterDatabase.PQuery("SELECT * FROM character_bosskill WHERE raid = %u AND guid = %I64u ORDER BY date", id, plr->GetGUID());
	else
		result = CharacterDatabase.PQuery("SELECT * FROM character_bosskill WHERE guid = %I64u ORDER BY date", plr->GetGUID());
	...
	SendSysMessage("Bosskills:");
	do
	{
		auto* fields = result->Fetch();
		auto raid = fields[1].GetUInt32() - 1;
		auto boss = fields[2].GetUInt32();
		auto date = fields[3].GetString();
		...
		auto actualRaid = raids[raid];
		auto actualBoss = bosses[raid][n];

		PSendSysMessage("Raid: %s, Boss: %s, Date: %s", actualRaid.c_str(), actualBoss.c_str(), date);
...
bool ChatHandler::HandlePetLevelupCommand(char* args)
{
	...
	uint32 level = 0;
	if (!(strm >> level) || level > 60)
		return false;

	if(plr)
	{
		if(auto* pet = plr->GetPet())
		{
			pet->SetLevel(pet->getLevel() + level);
			pet->SavePetToDB(PetSaveMode::PET_SAVE_AS_CURRENT);
			return true;
		}
	}

	// If no player is selected, use petname instead
	...
	if (!(strm >> petName) || petName.length() == 0) {
		return false;
	}

	if (!(strm >> ownerName) || ownerName.length() == 0) {
		return false;
	}

	static SqlStatementID updatePet;
	SqlStatement stmt = CharacterDatabase.CreateStatement(updatePet, "UPDATEcharacter_pet SET level = ? WHERE owner = ? AND name = ?");
	if(!stmt.PExecute(level, ownerName.c_str(), petName.c_str()))
	{
		SendSysMessage("Fehler beim schreiben in die Datenbank (stimmen die Namen?)");
		return false;
	}
