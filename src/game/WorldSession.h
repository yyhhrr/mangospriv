...
#include "Item.h"
#include "Warden-System\WardenBase.h"
#include "World.h"
...
class MANGOS_DLL_SPEC WorldSession
{
    ...
    protected:
        class DosProtection
        {
            friend class World;
        public:
            DosProtection(WorldSession* s) : Session(s), _policy((Policy)sWorld.getConfig(CONFIG_UINT32_PACKET_SPOOF_POLICY)) { }
            bool EvaluateOpcode(WorldPacket& p, time_t time, uint32 factor = 1) const;
        protected:
            enum Policy
            {
                POLICY_LOG,
    ...
    private:
        ...
        bool VerifyMovementInfo(MovementInfo const& movementInfo, ObjectGuid const& guid, uint32 opcode);
        bool VerifyMovementInfo(MovementInfo const& movementInfo, uint32 opcode);
		bool SpeedhackCheck(MovementInfo const& movementInfo);
