    ...
    m_throwAIEventStep(0)
{
    ...
    if (creatureEventsItr != sEventAIMgr.GetCreatureEventAIMap().end())
    {
        ...
        if (events_count == 0)
			sLog.outErrorDb("Creature %u has events but no events added to list because of instance flags (spawned in map %u).", m_creature->GetEntry(), m_creature->GetMapId());
        else
        {
		...
		sLog.outErrorDb("EventMap for Creature %u is empty but creature is using CreatureEventAI.", m_creature->GetEntry());
...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker, Creature* pAIEventSender)
{
    switch (action.type)
    {
		...
		case ACTION_T_SUMMON_UNIQUE:
		{
			...
			MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_creature, action.summon_unique.creatureId, true, 100);
			...
			if (!pSpawn)
			{
				Unit* target = GetTargetByType(action.summon_unique.target, pActionInvoker, pAIEventSender);
				if (!target)
					sLog.outErrorDb("Event %u - NULL target for ACTION_T_SUMMON_UNIQUE(%u), target-type %u", EventId, action.type, action.summon_unique.target);

				CreatureEventAI_Summon_Map::const_iterator i = sEventAIMgr.GetCreatureEventAISummonMap().find(action.summon_unique.spawnId);
				if (i == sEventAIMgr.GetCreatureEventAISummonMap().end())
				{
					sLog.outErrorDb("failed to spawn creature %u. Summon map index %u does not exist. EventID %d. CreatureID %d", action.summon_unique.creatureId, action.summon_unique.spawnId, EventId, m_creature->GetEntry());
					return;
				}

				Creature* pCreature = NULL;
				if ((*i).second.SpawnTimeSecs)
					pCreature = m_creature->SummonCreature(action.summon_unique.creatureId, (*i).second.position_x, (*i).second.position_y, (*i).second.position_z, (*i).second.orientation, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, (*i).second.SpawnTimeSecs);
				else
					pCreature = m_creature->SummonCreature(action.summon_unique.creatureId, (*i).second.position_x, (*i).second.position_y, (*i).second.position_z, (*i).second.orientation, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0);

				if (!pCreature)
					sLog.outErrorDb("failed to spawn creature %u. EventId %d.Creature %d", action.summon_unique.creatureId, EventId, m_creature->GetEntry());
...
void CreatureEventAI::Reset()
{
	...
	m_movedToTarget = false;
	m_CheckTargetRange = 2000;
...
void CreatureEventAI::MovementInform(uint32 uiMovementType, uint32 uiData)
{
    if (m_isCombatMovement && m_creature->getVictim()
        && m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == IDLE_MOTION_TYPE)
    {
        m_creature->GetMotionMaster()->Clear(false);
        m_creature->GetMotionMaster()->MoveChase(m_creature->getVictim(), m_attackDistance, m_attackAngle);
    }
...
void CreatureEventAI::UpdateAI(const uint32 diff)
{
    ...
    if (!m_isCombatMovement || m_movedToTarget)
    {
	    if (m_CheckTargetRange < diff) {
            if (Unit* pTarget = m_creature->getVictim())
		    {
			    float fDistance = m_creature->GetCombatDistance(pTarget, false);

			    if (fDistance > 30 || !m_creature->IsWithinLOSInMap(pTarget))
			    {
				    if (!m_movedToTarget)
				    {
                        m_creature->InterruptNonMeleeSpells(false);
                        SetCombatMovement(true, true);
					    m_movedToTarget = true;
					    m_CheckTargetRange = 10000;
				    }
			    }
                else if (m_movedToTarget) {
                    SetCombatMovement(false, true);
				    m_movedToTarget = false;
				    m_CheckTargetRange = 5000;
			    }
		    }
	    } else m_CheckTargetRange -= diff;
    }
