...
bool ScriptMgr::OnEffectDummy(Object* pCaster, uint32 spellId, SpellEffectIndex effIndex, Object* pTarget)
{
	return m_pOnEffectDummy != nullptr && m_pOnEffectDummy(pCaster, spellId, effIndex, pTarget);
}
...
bool ScriptMgr::OnAuraApply(Aura const* pAura, bool apply, AuraType type) {
	return m_pOnAuraApply != nullptr && m_pOnAuraApply(pAura, apply, type);
}

void ScriptMgr::OnUpdateLoot(Loot& loot, Player* owner, uint32 id) {
	if(m_pFillLoot != nullptr)
		m_pFillLoot(loot, owner, id);
}
...
#   define GET_SCRIPT_HOOK_PTR(P,N)             \
	...
	GET_SCRIPT_HOOK_PTR(m_pOnEffectDummy,			   "EffectDummy");
    GET_SCRIPT_HOOK_PTR(m_pOnAuraDummy,                "AuraDummy");
	GET_SCRIPT_HOOK_PTR(m_pOnAuraApply,				   "AuraApply");
	GET_SCRIPT_HOOK_PTR(m_pFillLoot,				   "FillLoot");
