...
class MANGOS_DLL_SPEC CreatureAI
{
    public:
		...
		/**
		 * Called before entering evade mode
		 * return false to not enter it, otherwise true
		 */
		virtual bool PreEnterEvadeMode() { return true; }
		...
		/**
		 * Called after the respawn location for the creature has been calculated and the corpse gets removed.
		 * Note:	User for custom relocation of the creature depending on certain parameters
		 * @param x Holds the calculated x-coordinate and serves as input and output parameter
		 * @param y Holds the calculated y-coordinate and serves as input and output parameter
		 * @param z Holds the calculated z-coordinate and serves as input and output parameter
		 * @param orientation Points the calculated orientation (or null, if not requested) and serves as input and output parameter
		 * @param orientation Points the calculated radius (or null, if not requested) and serves as input and output parameter
		 */
		virtual void RequestRespawnCoords(float& x, float& y, float& z, float* orientation, float* radius) const { }
		...
		virtual void PreSpellHit(Unit* pCaster, const SpellEntry* pSpell, int32& damage) {}
        ...
        virtual void UpdateAI(const uint32 uiDiff) {}
		 ...
		 /**
         * Called at World update tick if the creature is dead, by default every 100ms
         * This setting is dependend on CONFIG_UINT32_INTERVAL_MAPUPDATE
         * Note: Use this function to handle Timers, Threat-Management and MeleeAttacking
         * @param uiDiff Passed time since last call
         */
        virtual void UpdateAIDead(const uint32 uiDiff) {}
		...
		CanCastResult DoCastSpellIfCan(float x, float y, float z, uint32 uiSpell, uint32 uiCastFlags = 0, ObjectGuid OriginalCasterGuid = ObjectGuid());
	...
	protected:
		...
		void BroadcastMessage(const std::string& msg, BroadcastColor color = BroadcastColor::None) const;
		void SendMail(MailMessageType type, Player* receiver, uint32 sender, const std::string& topic, const std::string& msg, uint32 money = 0, uint32 itemId = 0, uint32 amount = 0) const;
