...
void Spell::EffectInstaKill(SpellEffectIndex /*eff_idx*/)
{
	...
	if(m_caster->GetEntry() == 12056) //Hackfix: Geddon does not grant loot after killing himself. player has to kill him
		return;
        ...
        switch(m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    {
                        if (unitTarget->IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
                        {
                            damage = 0;
                            break;
                        }
...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            {
				case 24531:									// Refocus ZG trinket
				{
					if (m_caster->GetTypeId() != TYPEID_PLAYER)
						return;

					auto* plrCaster = static_cast<Player*>(m_caster);

					plrCaster->RemoveSpellCooldown(19434, true); //Aimed Shot R1
					plrCaster->RemoveSpellCooldown(20900, true); //Aimed Shot R2
					plrCaster->RemoveSpellCooldown(20901, true); //Aimed Shot R3
					plrCaster->RemoveSpellCooldown(20902, true); //Aimed Shot R4
					plrCaster->RemoveSpellCooldown(20903, true); //Aimed Shot R5
					plrCaster->RemoveSpellCooldown(20904, true); //Aimed Shot R6

					plrCaster->RemoveSpellCooldown(2643, true); //Multi-Shot R1
					plrCaster->RemoveSpellCooldown(14288, true); //Multi-Shot R2
					plrCaster->RemoveSpellCooldown(14289, true); //Multi-Shot R3
					plrCaster->RemoveSpellCooldown(14290, true); //Multi-Shot R4
					plrCaster->RemoveSpellCooldown(25294, true); //Multi-Shot R5

					plrCaster->RemoveSpellCooldown(1510, true); //Volley R1
					plrCaster->RemoveSpellCooldown(14294, true); //Volley R2
					plrCaster->RemoveSpellCooldown(14295, true); //Volley R3

					plrCaster->RemoveSpellCooldown(3044, true); //Arcane Shot R1
					plrCaster->RemoveSpellCooldown(14281, true); //Arcane Shot R2
					plrCaster->RemoveSpellCooldown(14282, true); //Arcane Shot R3
					plrCaster->RemoveSpellCooldown(14283, true); //Arcane Shot R4
					plrCaster->RemoveSpellCooldown(14284, true); //Arcane Shot R5
					plrCaster->RemoveSpellCooldown(14285, true); //Arcane Shot R6
					plrCaster->RemoveSpellCooldown(14286, true); //Arcane Shot R7
					plrCaster->RemoveSpellCooldown(14287, true); //Arcane Shot R8
                ...
                {
                    int32 basepoints = ((100 + urand(0, 249)) * m_caster->getLevel()) / 60; // calculate damage taken
                    m_caster->CastCustomSpell(m_caster, 13493, &basepoints, NULL, NULL, true);
                    m_caster->SetChannelObjectGuid(unitTarget->GetObjectGuid());
                    return;
                }
                case 13280:                                 // Gnomish Death Ray
                {
                    Unit* curTarget = m_caster->GetMap()->GetUnit(m_caster->GetChannelObjectGuid());
                    m_caster->SetChannelObjectGuid(ObjectGuid());

                    int roll = m_caster->GetSpellAuraHolder(13493)->m_auras[0]->GetBasePoints();
                    int32 basepoints = 0;
                    basepoints = 7 * roll; // calculate damage

                    if (roll_chance_i(5)) // 5% backfire
                    {
                        m_caster->CastCustomSpell(m_caster, 13279, &basepoints, NULL, NULL, true);
                        return;
                    }

                    if (!curTarget || !curTarget->isVisibleForOrDetect(m_caster, m_caster, true) || !m_caster->IsWithinLOSInMap(curTarget) || !m_caster->isInFront(curTarget, 30.0f))
                        return;

                    Unit* secondTarget = curTarget->SelectRandomFriendlyTarget(curTarget, 5.0f);

                    if (roll_chance_i(5)) // 5% double hit
                    {
                        int32 damage = basepoints / 2; // double hit deals half damage
                        // no 2nd target or no good target --> double hit main target
                        if (!secondTarget || !m_caster->IsHostileTo(secondTarget) || secondTarget->GetTypeId() != curTarget->GetTypeId())
                            secondTarget = curTarget;
                        if (m_caster->IsHostileTo(secondTarget) && m_caster->GetDistance2d(secondTarget) < 32.0f && m_caster->IsWithinLOSInMap(secondTarget))
                            m_caster->CastCustomSpell(secondTarget, 13279, &damage, NULL, NULL, true);
                    }

                    // normal hit
                    m_caster->CastCustomSpell(curTarget, 13279, &basepoints, NULL, NULL, true);
                    return;
                }
                ...
                case 20572:                                 // Blood Fury
                {
                    ...
                    uint32 ap = 0; // calculate base attackpower from level/class

                    switch (m_caster->getClass())
                    {
                        case CLASS_WARRIOR: ap = m_caster->getLevel()*3.0f + m_caster->GetCreateStat(STAT_STRENGTH) * 2.0f - 20.0f; break;
                        case CLASS_ROGUE:
                        case CLASS_HUNTER: ap = m_caster->getLevel()*2.0f + m_caster->GetCreateStat(STAT_STRENGTH) + m_caster->GetCreateStat(STAT_AGILITY) - 20.0f; break;
                        case CLASS_DRUID:
                        case CLASS_SHAMAN:  ap = m_caster->getLevel()*2.0f + m_caster->GetCreateStat(STAT_STRENGTH) * 2.0f - 20.0f; break;
                        case CLASS_WARLOCK:
                        case CLASS_PRIEST:
                        case CLASS_MAGE: ap = m_caster->GetCreateStat(STAT_STRENGTH) - 10.0f; break;
                    }

                    damage = uint32(damage * ap / 100);
				...
				case 19873:                                 // Ei zerstören Spell bei Feuerkralle der Ungezähmte
                {
					...
					if(gameObjTarget)
						sScriptMgr.OnEffectDummy(caster, m_spellInfo->Id, eff_idx, gameObjTarget);
                    return;
                }
				/*
                case 19869:									// Dragon Orb
                {
					...
					unitTarget->CastSpell(unitTarget, 23958, true);
                    unitTarget->CastSpell(unitTarget, 19832, true);
					unitTarget->CastSpell(unitTarget, 23014, true);
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags & UI64LIT(0x0000000000040000))
            {
                ...
                if (Player* modOwner = m_caster->GetSpellModOwner())
					modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_COST, cost, shared_from_this());
            ...
            if (m_spellInfo->SpellFamilyFlags & UI64LIT(0x400000))
            {
                ...
                for(int j = BASE_ATTACK; j <= OFF_ATTACK; ++j)
                {
                    if (Item* item = ((Player*)m_caster)->GetWeaponForAttack(WeaponAttackType(j)))
                    {
                        ...
                        {
							auto spell = std::make_shared<Spell>(m_caster, spellInfo, true);
	...
	Object* caster = m_CastItem ? (Object*)m_CastItem : (Object*)m_caster;
	if(unitTarget) {
		sScriptMgr.OnEffectDummy(caster, m_spellInfo->Id, eff_idx, unitTarget);
	} else if(gameObjTarget) {
		sScriptMgr.OnEffectDummy(caster, m_spellInfo->Id, eff_idx, (Object*)gameObjTarget);
	} else if(itemTarget) {
		sScriptMgr.OnEffectDummy(caster, m_spellInfo->Id, eff_idx, (Object*)itemTarget);
	}
}
...
void Spell::EffectTriggerSpell(SpellEffectIndex eff_idx)
{
    ...
    switch(triggered_spell_id)
    {
        ...
        case 15752:
        case 15753:
            // We use m_damage to decide which spell can proc
            if ((m_damage + triggered_spell_id) % 2 || rand_chance() < 97.0)
                return;
            else
                break;
		case 31770:
        ...
        case 29284:
            ...
            if (SpellAuraHolder *aura = unitTarget->GetSpellAuraHolder(24575))
...
void Spell::EffectTeleportUnits(SpellEffectIndex eff_idx)
{
    ...
    switch (targetType)
    {
        ...
        // SpellIds 25371, 26196, 26382
        case TARGET_DYNAMIC_OBJECT_BEHIND:
        {
            if (m_caster == unitTarget)
                return;
            
            unitTarget->NearTeleportTo(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ() + 2.f, m_caster->GetOrientation(), false);
            break;
        }
        default:
        {
...
void Spell::EffectApplyAura(SpellEffectIndex eff_idx)
{
    if(!unitTarget)
	 ...
	 /*if (m_spellInfo->Id == 22799)
	 {
		 Map* pMap;
		 pMap = unitTarget->GetMap();

		if (pMap == !429)
			unitTarget->RemoveAurasDueToSpell(22799);
		return;
	 }*/
	...
	m_spellAuraHolder->SetStackAmount(m_spellAuraHolder->GetStackAmount());
}
...
void Spell::EffectHeal(SpellEffectIndex eff_idx)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        // Apply Damage Multiplier on Bonus-Healing
        addhealth = damage + ((addhealth - damage) *  m_damageMultipliers[eff_idx]);
...
void Spell::DoCreateItem(SpellEffectIndex eff_idx, uint32 itemtype)
{
    switch(itemtype)
    {
		case 18492:						// Treated Ancient Blade
    ...
    if (!dispel_list.empty())
    {
        ...
        for (int32 count=0; count < damage && !dispel_list.empty(); ++count)
        {
            ...
            if (Unit *caster = holder->GetCaster())
            {
                if ( Player* modOwner = caster->GetSpellModOwner() )
					modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_RESIST_DISPEL_CHANCE, miss_chance, shared_from_this());
            }
        ...
        if (!success_list.empty())
        {
            ...
            if (unitTarget->IsFriendlyTo(m_caster))
                unitTarget->getHostileRefManager().threatAssist(m_caster /*real_caster ??*/, 0.f, m_spellInfo);
            else
            {
...
void Spell::EffectDistract(SpellEffectIndex /*eff_idx*/)
{
	...
	if (unitTarget->GetTypeId() == TYPEID_UNIT)
	{
		...
		// NPCs mit Wegpunkt:
		if (unitTarget->movespline->Finalized()) {
			unitTarget->StopMoving();
			unitTarget->SetFacingTo(angle);
		}
	}
	else {
		((Player*)unitTarget)->m_movementInfo.SetMovementFlags(MOVEFLAG_NONE);
		unitTarget->SetFacingTo(angle);
	}
}
...
void Spell::EffectEnchantItemTmp(SpellEffectIndex eff_idx)
{
    ...
    // Other things e.g. Sharpen Blade, Mana Oils, ...
        ...
        duration = 1800;
    else if(m_spellInfo->Id == 2829)                        // Sharpen Blade II
        duration = 1800;
    else if(m_spellInfo->Id == 3112)                        // Enhance Blunt Weapon
        duration = 1800;
	else if(m_spellInfo->Id == 20750)						// Wizard Oil
		duration = 1800;									// 30 mins
...
void Spell::EffectTameCreature(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (sWorld.getConfig(CONFIG_UINT32_START_PLAYER_LEVEL) > creatureTarget->getLevel())
        pet->GivePetLevel(sWorld.getConfig(CONFIG_UINT32_START_PLAYER_LEVEL));
...
void Spell::EffectSummonPet(SpellEffectIndex eff_idx)
{
    ...
    if( OldSummon )
    {
        if ((petentry == 0 || OldSummon->GetEntry() == petentry) && OldSummon->getPetType() != SUMMON_PET)
        {
            ...
            {
                ((Player*)m_caster)->PetSpellInitialize();
				if(((Player*)m_caster)->getClass() == CLASS_HUNTER)
					return;
            }
        }
                    ...
                    if (SpellAuraHolder* holder = (*i)->GetHolder())
                    {
                        for (int i = 0; i < 3; i++)
                            if (Aura* aura = holder->GetAuraByEffectIndex((SpellEffectIndex)i))
                                if (holder->GetSpellProto()->EffectApplyAuraName[i] == SPELL_AURA_MOD_DAMAGE_PERCENT_DONE
                                    && aura->GetModifier()->m_miscvalue & SPELL_SCHOOL_MASK_NORMAL)
                                    apply = false;
                    }
                    if (apply && (*i)->GetModifier()->m_miscvalue & GetSpellSchoolMask(m_spellInfo))
...
void Spell::EffectThreat(SpellEffectIndex /*eff_idx*/)
{
	...
	//very very ugly Hackfix
	const_cast<uint32&>(m_spellInfo->AttributesEx) &= ~SPELL_ATTR_EX_NO_THREAT;
...
void Spell::EffectInterruptCast(SpellEffectIndex /*eff_idx*/)
{
    ...
    {
        if (std::shared_ptr<Spell> spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
...
void Spell::EffectScriptEffect(SpellEffectIndex eff_idx)
{
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
				case 19633: //Onyxia Knock Away
				{
					if(m_caster->GetTypeId() != TYPEID_UNIT || !m_caster->getVictim()|| !unitTarget || !m_caster->getThreatManager().getThreat(unitTarget))
						return;
                ...
                {
                    uint32 spells[3] = {26272, 26273, 26274};
                    ...
                    unitTarget->CastSpell(unitTarget, spells[urand(0, 2)], true);
                    return;
                }
        ...
        case SPELLFAMILY_PRIEST:
        {
			...
			//Verblassen - Fade -20%
			if (m_spellInfo->SpellIconID == 586)
			{
				if (unitTarget->getThreatManager().getThreat(m_caster))
					unitTarget->getThreatManager().modifyThreatPercent(m_caster, -20);
				return;
			}
            break;
        }
...
void Spell::EffectStuck(SpellEffectIndex /*eff_idx*/)
{
    ...
    if(!spellInfo)
        return;
    auto spell = std::make_shared<Spell>(pTarget, spellInfo, true);
...
void Spell::EffectSummonPlayer(SpellEffectIndex /*eff_idx*/)
{
	...
	if (unitTarget->GetTypeId() == TYPEID_PLAYER && unitTarget->GetAntiCheatImmuneTimer() == 0)
		unitTarget->SetAntiCheatImmuneTimer(2000);
...
void Spell::EffectSummonTotem(SpellEffectIndex eff_idx)
{
	...
	//don't summon these weird totems. use the wild summoned totems instead
	if (m_spellInfo->Id == 23419 || m_spellInfo->Id == 23422 || m_spellInfo->Id == 23420 || m_spellInfo->Id == 23423)
		return;
...
void Spell::EffectSummonPossessed(SpellEffectIndex eff_idx)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
		...
		if (m_caster->GetPetGuid() || m_caster->GetCharmGuid())
		...
		if(m_spellInfo->Id == 126) //Eye of Kilrogg
		{
			player->RemovePetActionBar();
			spawnCreature->SetSpeedRate(MOVE_RUN, 2.0f, true);
		}
...
{ 
     ...
     if (m_caster->GetTypeId() == TYPEID_PLAYER)
         m_caster->SetAntiCheatImmuneTimer(2000);

     // Blink , has its own way of calculation - Rank 1 / Rank 2
     if (m_spellInfo->Id == 1953 || m_spellInfo->Id == 6139) 
     {
          bool bUnderwater = unitTarget->IsInWater() || unitTarget->IsUnderWater();
          float fGroundHeight;

          // Blink Range
          float flDistance = GetSpellRadius(sSpellRadiusStore.LookupEntry(m_spellInfo->EffectRadiusIndex[eff_idx]));

          // Target Point Coordinates, Ideal Case, Z Value is gonna be determined
          float flTargetX, flTargetY;

          // Z Offsets for several usages
          float const flOffsetZ = 1.8f; // 2.0f default?
          float const flOffsetWaterZ = 2.0f;
          float const flPlayerOffsetZ = 0.75f; // 0.2f default?
          float const flOffsetWaterTopZ = 1.0f;

          // Used for getObjectHitPos Function
          float const flModDist = -0.5f;

          // Used to modify VecUp Vector later
          float const flDirFactor = 1.5f; // 3.0f default?

          // Get Ideal Point, used for Target Vector calculation
          unitTarget->GetNearPoint2D(flTargetX, flTargetY, flDistance,unitTarget->GetOrientation());

          // Player Coordinates
          float flPlayerX, flPlayerY, flPlayerZ;
          unitTarget->GetPosition(flPlayerX, flPlayerY, flPlayerZ);

          // Get VMap Manager
          VMAP::IVMapManager *vMapManager = VMAP::VMapFactory::createOrGetVMapManager();

          // The Player Vector, always used for the current Position while Blinking is in progress
          Vector3 vecPlayer = Vector3(flPlayerX,flPlayerY,flPlayerZ);
          // Target Vector, used for calculating Blink direction
          Vector3 vecTarget = Vector3(flTargetX,flTargetY,flPlayerZ+flOffsetZ); 
          // The Vector which is responsible for detecting Objects in Front of the caster which shouldnt be passed
          Vector3 vecUp = Vector3(flPlayerX,flPlayerY,flPlayerZ);

          // Calucalate Direction Vector with Length = 1
          float flMaxDist = (vecTarget - vecPlayer).magnitude();
          Vector3 vecDirection = (vecTarget- vecPlayer)/flMaxDist; 

          // Vector for resulting Position
          Vector3 vecResult = Vector3(vecPlayer.x,vecPlayer.y,vecPlayer.z);

          // Collision Detection Vector now adjusted so he points slightly above player
          vecUp += vecDirection;
          vecUp.z += flOffsetZ;

          // Used for Terrain Collision
          float flTerrainZ = vecPlayer.z;
          float flTerrainZOld = vecPlayer.z;
          float flTerrainDiff = 0.0f;
          float flTerrainWaterLevel = VMAP_INVALID_HEIGHT_VALUE;
          float flTerrainHeightRadius = 300.0f;
          
          // Responsible for seperating Blink in 2 different Behaviours
          bool bIsAirBlink = false;

          // Number of Iterations
          uint8 uiIterations = 8;
          uint8 uiStartPos = 0;

          // Used to abort if Terrain Slope falls off too much
          float flMaxNegativeZDiff = -0.2f; // -3.0f default?
          float flMaxPositiveZDiff = 0.175f;  // 0.2f default?

          // Multiplies Direction Vector when stepping back (only used when preventing falling down cliffs)
          float flStepBackFactor = 3.0f; // 2.0f default?

      RestartAlgorithm:

          float flTerrainDiffFactor = (float)uiIterations / 100.f;

          // Now our tiny algorithm starts :)
          // 100 Iterations used for allowing small distances to blink and avoiding errors at high slope terrain
          for (uint8 i = uiStartPos; i <= uiIterations; i++)
          {
               // The Hit Coordinates
               // float flHitX;
               //float flHitY;
               ...
               // Stores Collision Check Result
               bool bHitFound = false;

               bHitFound |= !unitTarget->IsWithinLOS(vecUp.x, vecUp.y, vecUp.z);

               // Look if something is in front of us 
               // New : Lets check in a small Cone for better Detection of Tiny Objects (e.g Fences are problematic)
               /*for (int8 l = -1; l <= 1; l=l+2)
               {
                if (bHitFound) 
                    break;
                for (uint8 j = 0; j < 8; j++)
                {
                 ...
                 for (uint8 k = 0; k < 8; k++)
                 {
                  ...
                  for (uint8 m = 0; m < 8; m++)
                   {
                    bHitFound |= vMapManager->getObjectHitPos(unitTarget->GetMapId(),vecPlayer.x,vecPlayer.y,vecPlayer.z+flPlayerOffsetZ,
                                                              vecUp.x+(j*0.10f)*l,vecUp.y+(k*0.10f)*l,vecUp.z+(m*0.025f)*((l==-1) ? 1 : 2),
                                                              flHitX,flHitY,flHitZ,flModDist);
                    if (bHitFound) 
                        break;
                   }
                  }
               ...
               // If something "big" is in Front of Player (slope too high), abort & use last correct Position
               if (bHitFound)
               {
                   if (uiIterations < 128)
                   {
                       ...
                       {
                           uiStartPos = i * 2 - 1;
                           vecPlayer = vecResult + (vecDirection*(flDistance / uiIterations));
                           vecUp = vecPlayer;
                           vecUp += vecDirection*flDirFactor;
                           vecUp.z += flOffsetZ;
                       }
                       else
                           break;
                       goto RestartAlgorithm;
                   }
                   break;
               }
               else 
               {
                    // Terrain Collision -> If Terrain Z Rises/Lowers, Player Z will also
				   flTerrainZ = unitTarget->GetTerrain()->GetHeightStatic(vecPlayer.x, vecPlayer.y, vecPlayer.z + 0.2f, true, flTerrainHeightRadius);
                    
                    if (flTerrainZ != VMAP_INVALID_HEIGHT_VALUE)
                    {
                    flTerrainDiff = flTerrainZ-flTerrainZOld;

                    flTerrainDiff *= flTerrainDiffFactor;
                    
                    // If Player starts blinking when already "flying", some Things will Change
                    if (i==0 && abs(vecPlayer.z-flTerrainZ) > 4.0f)
                        bIsAirBlink |= true;

                    if (bIsAirBlink && unitTarget->GetTerrain()->GetWaterOrGroundLevel(vecPlayer.x, vecPlayer.y,
                        vecPlayer.z + 0.2f, &fGroundHeight) && fGroundHeight >= vecPlayer.z + 0.2f)
                        break;

                    // Break if Negative Difference is too high
                    if (flTerrainDiff < flMaxNegativeZDiff && !bIsAirBlink)
                    {
                        if (uiIterations < 128)
                        {
                            uiIterations *= 2;
                        ...
                        // Dont Step Back if Starting to Blink when too high above Ground Level
                        if (!(abs(vecPlayer.z-flTerrainZ) > 3.0f && i<=2))
                        {
                           flStepBackFactor = (i<uiIterations/4) ? 1.0f : 3.0f;
                           // Step Back, so Player wont fall down at Cliff-Border
                           vecResult -= (vecDirection*(flDistance / uiIterations))*flTerrainDiffFactor*flStepBackFactor;
                        }
                        break;
                    } 
                    ...
                    // Also Break if Positive Difference too High
                    if (flTerrainDiff > flMaxPositiveZDiff && !bIsAirBlink)
                    {
                        if (uiIterations < 128)
                        {
                            uiIterations *= 2;
                    ...
                    // Dont concern Terrain Difference if Air-Blinking
                    if (!bIsAirBlink)
                        vecPlayer.z += (flTerrainZ - flTerrainZOld);

                    flTerrainZOld = flTerrainZ;
                    }

                    else
                        // Also Break if getting VMAP_INVALIC_HEIGHT_VALUE, means were under map 
                        break;

                    // First take Player coords as target
                    vecResult = Vector3(vecPlayer.x,vecPlayer.y,vecPlayer.z);
                    // Advance in target direction step by step
                    vecPlayer += (vecDirection*(flDistance/uiIterations));

                    // Now modify vecUp Vector based on player coords
                    vecUp = vecPlayer;
                    vecUp += vecDirection*flDirFactor;
                    vecUp.z += flOffsetZ;
               }
          ...
          // Dont blink tiny distances
          if ((vecResult - Vector3(flPlayerX, flPlayerY, flPlayerZ)).magnitude() < 1.f)
              vecResult = Vector3(flPlayerX, flPlayerY, flPlayerZ);

          // Only Adjust Final Z Position when near Ground Level
          if (!(abs(vecPlayer.z - flTerrainZ) > 3.0f) && !bIsAirBlink && !unitTarget->GetTerrain()->IsInWater(vecPlayer.x, vecPlayer.y, vecPlayer.z))
          {
          // Adjust Position Z when PlayerZ is ABOVE Ground -> No Fall after teleport
          unitTarget->UpdateAllowedPositionZ(vecResult.x,vecResult.y,vecResult.z);
          // Always stay on top of Water Surface, also slightly above to avoid getting under Map at water border
         // Not my fault :) Water Surface Detection is sometimes bugged right there, see commented getLiquidLevel fct.
          flTerrainWaterLevel = unitTarget->GetTerrain()->GetWaterLevel(vecPlayer.x, vecPlayer.y, vecPlayer.z + flOffsetWaterZ, &fGroundHeight);
          if (flTerrainWaterLevel != VMAP_INVALID_HEIGHT_VALUE && fGroundHeight < flTerrainWaterLevel + flOffsetWaterTopZ)
               vecResult.z = flTerrainWaterLevel+flOffsetWaterTopZ;
          }

          // Finally Teleport to result vector
          unitTarget->NearTeleportTo(vecResult.x, vecResult.y, vecResult.z, unitTarget->GetOrientation(), unitTarget == m_caster);

          // Return because following mechanics are used for other teleport spells
          return;
     }
     else
     {
         ...
         // Z Offsets for several usages
         float flOffsetZ = 1.8f; // 2.0f default?
         float flOffsetWaterZ = 2.0f;
         float flPlayerOffsetZ = 0.75f; // 0.2f default?
         float flOffsetWaterTopZ = 1.0f;

         // Used for getObjectHitPos Function
         float flModDist = -0.5f;

         // Used to modify VecUp Vector later
         float flDirFactor = 1.5f; // 3.0f default?
         ...
         // Used to abort if Terrain Slope falls off too much
         float flMaxNegativeZDiff = -0.3f; // -3.0f default?
         float flMaxPositiveZDiff = 0.175f;  // 0.2f default?
         ...
         // Number of Iterations
         uint8 uiIterations = 100;

         // Now our tiny algorithm starts :)
         // 100 Iterations used for allowing small distances to blink and avoiding errors at high slope terrain
         for (uint8 i = 0; i < uiIterations; i++)
         {
             // The Hit Coordinates
             float flHitX;
             float flHitY;
             ...
             else
             {
                 ...
                 if (flTerrainZ != VMAP_INVALID_HEIGHT_VALUE)
                 {
                     ...
                     // If Player starts blinking when already "flying", some Things will Change
                     if (i <= 1 && abs(vecPlayer.z - flTerrainZ) > 4.0f)
                         bIsAirBlink |= true;
                     ...
                     if (flTerrainDiff < flMaxNegativeZDiff && !bIsAirBlink)
                     {
                         ...
                         if (!(abs(vecPlayer.z - flTerrainZ) > 3.0f && i <= 2))
                         {
                             flStepBackFactor = (i<uiIterations / 4) ? 1.0f : 3.0f;
                             // Step Back, so Player wont fall down at Cliff-Border
                             vecResult -= (vecDirection*(flDistance / uiIterations))*flStepBackFactor;
                         }
                         break;
                     }
         ...
         // Only Adjust Final Z Position when near Ground Level
         if (!(abs(vecPlayer.z - flTerrainZ) > 3.0f) && !bIsAirBlink)
         {
             // Adjust Position Z when PlayerZ is ABOVE Ground -> No Fall after teleport
             unitTarget->UpdateAllowedPositionZ(vecResult.x, vecResult.y, vecResult.z);
             // Always stay on top of Water Surface, also slightly above to avoid getting under Map at water border
             // Not my fault :) Water Surface Detection is sometimes bugged right there, see commented getLiquidLevel fct.
             flTerrainWaterLevel = unitTarget->GetTerrain()->GetWaterLevel(vecPlayer.x, vecPlayer.y, vecPlayer.z + flOffsetWaterZ);
             if (flTerrainWaterLevel != VMAP_INVALID_HEIGHT_VALUE)
                 vecResult.z = flTerrainWaterLevel + flOffsetWaterTopZ;
         }
...
void Spell::EffectCharge(SpellEffectIndex /*eff_idx*/)
{
	...
	float x, y, z;
    unitTarget->GetNearPoint(m_caster, x, y, z, m_caster->GetObjectBoundingRadius(), unitTarget->GetCombatReach(m_caster) * 0.25f, unitTarget->GetAngle(m_caster));

    if (unitTarget->GetTypeId() == TYPEID_PLAYER && unitTarget->m_movementInfo.HasMovementFlag(movementFlagsMask))
    {
        float offset = (unitTarget->m_movementInfo.HasMovementFlag(MovementFlags::MOVEFLAG_WALK_MODE) ?
            unitTarget->GetSpeed(UnitMoveType::MOVE_WALK) : unitTarget->GetSpeed(UnitMoveType::MOVE_RUN)) / 3.f;
        unitTarget->GetNearPoint2D(x, y, offset, unitTarget->GetOrientation());
        float height = unitTarget->GetTerrain()->GetWaterOrGroundLevel(x, y, z);
        if (!m_caster->IsUnderWater() && unitTarget->IsUnderWater() && height != VMAP_INVALID_HEIGHT_VALUE)
            z = height;

        if (!unitTarget->IsWithinLOS(x, y, z))
            unitTarget->GetNearPoint(m_caster, x, y, z, m_caster->GetObjectBoundingRadius(), unitTarget->GetCombatReach(m_caster) * 0.25f, unitTarget->GetAngle(m_caster));
    }
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT)
        m_caster->SetChargeTarget();

    if (m_caster->GetTypeId() == TYPEID_PLAYER)
        m_caster->SetAntiCheatImmuneTimer(2500);

	m_caster->MonsterMoveWithSpeed(x, y, z, 40.0f, true, true, 1400);
...
void Spell::EffectSummonDeadPet(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (pet->GetDistance2d(m_caster) >= 7.f)
    {
        float x, y, z;

        m_caster->GetNearPoint(pet, x, y, z, pet->GetObjectBoundingRadius(), PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
        pet->NearTeleportTo(x, y, z, pet->GetOrientation());
    }
...
void Spell::EffectSpiritHeal(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (BattleGround* bg = ((Player*)unitTarget)->GetBattleGround())
    {
        bg->ScheduleRevive(unitTarget->GetObjectGuid());
        return;
    }
