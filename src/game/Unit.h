...
enum UnitState
{
	...
	UNIT_STAT_CAN_NOT_TURN	  = 0x00080000,
	UNIT_STAT_CAN_NOT_TARGET  = 0x00100000,
...
enum UnitVisibility
{
	...
	VISIBILITY_REMOVE_CORPSE	  = 5                       // special totally not detectable visibility for force delete object at respawn command
};
...
enum UnitFlags
{
    ...
    UNIT_FLAG_UNK_15                = 0x00008000,			// seems to be related with swimming, since swimming creatures without this flag will fall through the water (maybe flying aswell? needs to be checked)
...
class MANGOS_DLL_SPEC Unit : public WorldObject
{
    public:
		...
		UnitFearManager& getFearManager() { return mFearManager; }
		...
		void StopAttackObject(const ObjectGuid& obj);
		...
		void EvadeStart();
		void EvadeStop();
		bool isEvading() const { return mEvading; }
        ...
        void SetInCombatState(bool PvP, Unit* enemy = NULL, bool enrage = false);
        void SetInCombatWith(Unit* enemy);
        void ClearInCombat();
        uint32 GetCombatTimer() const { return m_CombatTimer; }
		void SetCombatTimer(uint32 timer) { m_CombatTimer = timer; }

		uint32 GetAntiCheatImmuneTimer() const { return m_AntiCheatImmuneTimer; }
		void SetAntiCheatImmuneTimer(uint32 timer) { m_AntiCheatImmuneTimer = timer; }
		uint32 GetAntiCheatCounter() const { return m_AntiCheatCounter; }
		void SetAntiCheatCounter(uint32 count) { m_AntiCheatCounter = count; }
		...
		bool DebuffLimitCheck(SpellAuraHolder *holder);
		...
		void SendFlyingAnimation(uint32 uiSpell);
		...
		bool CanCast(Unit* target, uint32 spellId, bool triggered = false);
		...
		void MonsterMoveWithSpeed(float x, float y, float z, float speed, bool generatePath = false, bool forceDestination = false, uint32 maxTravelTime = 0);
		...
		{
			if (hasUnitState(UNIT_STAT_CAN_NOT_TARGET))
				return;
        ...
        void SetCurrentCastedSpell(std::shared_ptr<Spell> pSpell, bool triggered = false);
        virtual void ProhibitSpellSchool(SpellSchoolMask /*idSchoolMask*/, uint32 /*unTimeMs*/ ) { }
        void InterruptSpell(CurrentSpellTypes spellType, bool withDelayed = true, bool forced = true);
        ...
        std::shared_ptr<Spell> GetCurrentSpell(CurrentSpellTypes spellType) const { return m_currentSpells[spellType]; }
        std::shared_ptr<Spell> FindCurrentSpellBySpellId(uint32 spell_id) const;
		...
		void SetWeaponElementalDamage(SpellSchools school, WeaponAttackType attType, WeaponDamageRange damageRange, float value)
		{
			m_weaponElementalDamage[school][attType][damageRange] = value;
		}
		...
		void RemoveAurasOfTypeIf(AuraType type, FUN)
		{
			std::remove_if(m_modAuras[type].begin(), m_modAuras[type].end(), [](Aura* aura) -> bool {
		...
		void UnsummonIfTotem();
        Unit* SelectMagnetTarget(Unit *victim, std::shared_ptr<Spell> spell = NULL, SpellEffectIndex eff = EFFECT_INDEX_0);
        ...
        SpellAuraProcResult HandleCantTrigger(Unit* /*pVictim*/, uint32 /*damage*/, Aura* /*triggeredByAura*/, SpellEntry const* /*procstd::shared_ptr<Spell>*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
        {
		...
		bool IsImmuneToSpell(uint32 spellId);
		...
		bool getAttackEnabled() const { return !mIsAttackPrevented; }
		void setAttackEnabled(bool enabled) { mIsAttackPrevented = !enabled; }

        uint32 CalcArmorReducedDamage(Unit* pVictim, const uint32 damage);
		bool RollBinarySpellEffectResist(Unit *pCaster, SpellSchoolMask schoolMask);
		float RollSpellEffectResist(Unit *pCaster, SpellSchoolMask schoolMask);
        void CalculateDamageAbsorbAndResist(Unit *pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool canReflect = false, bool canResist = false);
		...
		void SetResilience(uint16 resilience) { m_resilience = resilience; }
		uint16 GetResilience() { return m_resilience; }
		...
		{
			return mCanAttackPlayer;
		}

		void SetCanAttackPlayer(bool canAttackPlayer)
		{
			mCanAttackPlayer = canAttackPlayer;
		}

        void SetChargeTarget() { m_chargeTarget = true; }

        uint32 GetWorldState(uint32 id); // for script usage. ToDo: remove this and properly add world-class to SD2

    protected:
		bool mEvading = false;
		bool mCanAttackPlayer = true;
        explicit Unit ();
        ...
        bool m_AutoRepeatPrepareAnimation;
		...
		float m_weaponElementalDamage[MAX_SPELL_SCHOOL][MAX_ATTACK][2];
    ...
    private:
		...
		uint32 m_AntiCheatImmuneTimer;
		uint32 m_AntiCheatCounter;
		uint32 m_AntiCheatCounterTimer;
        bool m_setInactiveOnEvade;

		bool m_canChase;

        std::shared_ptr<Spell> m_currentSpells[CURRENT_MAX_SPELL];
        ...
        bool m_chargeTarget;                                 // do not change target until movespline.finished()
