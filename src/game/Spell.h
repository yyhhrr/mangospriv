...
class Spell : public std::enable_shared_from_this<Spell>
{
    ...
    friend void Unit::SetCurrentCastedSpell(std::shared_ptr<Spell> pSpell, bool triggered);
    public:
        ...
        static uint32 CalculatePowerCost(SpellEntry const* spellInfo, Unit* caster, const std::shared_ptr<Spell> spell = NULL, Item* castItem = NULL);
		...
		static void MANGOS_DLL_SPEC SendCastResult(Player* caster, uint32 spellId, SpellCastResult result);
    ...
    protected:
        ...
        std::shared_ptr<Spell>* m_selfContainer;                            // pointer to our spell container (if applicable)
...
{
    public:
		SpellEvent(std::shared_ptr<Spell> spell);
        virtual ~SpellEvent();
