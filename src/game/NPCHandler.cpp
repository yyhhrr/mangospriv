...
void WorldSession::HandleTrainerBuySpellOpcode( WorldPacket & recv_data )
{
    ...
    if(proto->SpellVisual == 222)
		spell = std::make_shared<Spell>(_player, proto, false);
    else
		spell = std::make_shared<Spell>(unit, proto, false);
...
void WorldSession::HandleGossipSelectOptionOpcode( WorldPacket & recv_data )
{
	...
	else if(guid.IsPlayer()) //This only happens after the player typed .account pvp in chat
	{
		if (action == 0)
		{
			LoginDatabase.PQuery("UPDATE account SET pvp = 'PVP' WHERE id = %u", _player->GetAccountId());
			_player->IsPvpAccount(true);
		}
