...
CanCastResult CreatureAI::CanCastSpell(Unit* pTarget, const SpellEntry *pSpell, bool isTriggered)
{
    ...
    if (!isTriggered)
    {
        ...
        if (m_creature->hasUnitState(UNIT_STAT_FLEEING_MOVE))
            return CAST_FAIL_STATE;
...
CanCastResult CreatureAI::DoCastSpellIfCan(float x, float y, float z, uint32 uiSpell, uint32 uiCastFlags, ObjectGuid OriginalCasterGuid )
{
	Unit* pCaster = m_creature;
	...
	if (!pCaster->IsNonMeleeSpellCasted(false) || (uiCastFlags & (CAST_TRIGGERED | CAST_INTERRUPT_PREVIOUS)))
	{
		if (const SpellEntry* pSpell = sSpellStore.LookupEntry(uiSpell))
		{
			...
			if (!(uiCastFlags & (CAST_FORCE_TARGET_SELF | CAST_FORCE_CAST)))
			{
				CanCastResult castResult = CanCastSpell(pCaster, pSpell, uiCastFlags & CAST_TRIGGERED);
			...
			pCaster->CastSpell(x, y, z, pSpell, uiCastFlags & CAST_TRIGGERED, NULL, NULL, OriginalCasterGuid);
			return CAST_OK;
		} else
		{
			sLog.outErrorDb("DoCastSpellIfCan by creature entry %u attempt to cast spell %u but spell does not exist.", m_creature->GetEntry(), uiSpell);
...
{
	if (m_creature->getAttackEnabled())
...
void CreatureAI::BroadcastMessage(const std::string& msg, BroadcastColor color) const
{
	if (color == BroadcastColor::Emote)
	{
		std::stringstream ss;
		ss << "|cffEF7A3F" << msg << "|r";
...
void CreatureAI::SendMail(MailMessageType type, Player* receiver, uint32 sender, const std::string& topic, const std::string& msg, uint32 money, uint32 itemId, uint32 amount) const
{
	MailDraft draft;

	if(itemId)
	{
		ItemPrototype const* itemPrototype = ObjectMgr::GetItemPrototype(itemId);
		if (itemPrototype == nullptr)
			return;

		if (amount < 1 || (itemPrototype->MaxCount > 0 && amount > uint32(itemPrototype->MaxCount)))
			return;

		if (Item* item = Item::CreateItem(itemId, amount))
		{
			item->SaveToDB();
			draft.AddItem(item);
		}
	...
	draft.SetSubjectAndBody(topic, msg);

	MailSender _sender(type, sender);
	draft.SendMailTo(MailReceiver(receiver, receiver->GetGUID()), _sender);
}