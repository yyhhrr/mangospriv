...
class ObjectMgr
{
    ...
    public:
        ...
        const char* GetRandomHintText() { return m_hintText.at(urand(0, m_hintText.size() - 1)).c_str(); };
        ...
        uint8 GetCreatureTemplateResilience(uint32 entry) const
        {
            UNORDERED_MAP<uint32, uint8>::const_iterator itr = mCreatureResilienceMap.find(entry);
            if (itr != mCreatureResilienceMap.end())
                return itr->second;
            else
                return uint8(0);
        }
    ...
    private:
		...
		HonorStandingList PlayerHonorStandingList;

        typedef std::map<uint32,std::vector<std::string> > HalfNameMap;
        HalfNameMap PetHalfName0;
        HalfNameMap PetHalfName1;

        UNORDERED_MAP<uint32, uint8> mCreatureResilienceMap;
        ...
        std::vector<std::string> m_hintText;
};
