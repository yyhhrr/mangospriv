...
m_isPersistent(false), m_in_use(0), m_spellAuraHolder(holder)
{
    ...
    switch(GetId())
    {
        ...
        case 25345:             // Arcane Missiles (Rank 8)
            m_periodicTimer += m_modifier.periodictime / 2;
            break;
        default:
...
void Aura::ApplyModifier(bool apply, bool Real)
{
    ...
    SetInUse(true);
    if(sScriptMgr.OnAuraApply(this, apply, aura) == false && aura < TOTAL_AURAS)
        (*this.*AuraHandler [aura])(apply, Real);

    SetInUse(false);
    GetHolder()->SetInUse(false);

	GetHolder()->GetTarget()->getFearManager().updateAuras(aura, apply);
}
...
bool Aura::CanProcFrom(SpellEntry const *spell, uint32 EventProcEx, uint32 procEx, bool active, bool useClassMask) const
{
    ...
    if (m_spellAuraHolder && spell && m_spellAuraHolder->GetSpellProto() &&
        m_spellAuraHolder->GetSpellProto()->Id == 19390 && // Whitelist for spells proccing Entrapment
        (spell->Id != 13812 && spell->Id != 14314 && spell->Id != 14315 && // Explosive Trap Effect
        spell->Id != 13797 && spell->Id != 14298 && spell->Id != 14299 && spell->Id != 14300 && spell->Id != 14301 &&// Immolation Trap Effect
        spell->Id != 13810)) // Frost Trap Aura
        return false;
...
//                    case 25152: break;
                    case 25371:                             // Consume
                    {
                        int32 bpDamage = triggerTarget->GetMaxHealth()*15/100;
                        if (Unit* caster = GetCaster())
                            if (caster->isAlive() && triggerTarget->GetTypeId() == TYPEID_PLAYER)
                            {
                                if (!caster->IsRooted())
                                {
                                    if (caster->GetTypeId() == TYPEID_UNIT)
                                    {
                                        caster->SetCanChase(false);
                                        caster->GetMotionMaster()->Clear(false, true);
                                        caster->GetMotionMaster()->MoveIdle();
                                    }
                                    caster->SetRoot(true);
                                }
                                if (triggerTarget->GetDistance(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ() > 2.5f))
                                    caster->NearTeleportTo(triggerTarget->GetPositionX(), triggerTarget->GetPositionY(), triggerTarget->GetPositionZ() - 2.f, caster->GetAngle(triggerTarget));
                                caster->CastCustomSpell(triggerTarget, 25373, &bpDamage, NULL, NULL, true, NULL, this, casterGUID);
                            }
                            else
                                triggerTarget->RemoveAurasDueToSpell(25371);
                        return;
                    }
//                    // Pain Spike
//                    case 25572: break;
            ...
            case SPELLFAMILY_WARRIOR:
            {
                switch(auraId)
                {
                    ...
                    case 23410: 
					{
						...
						auto plr = static_cast<Player*>(target);
						auto grp = plr->GetGroup();
						...
						for (auto itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
							raidMembers.push_back(itr->getSource());

						static std::uniform_int_distribution<int> raidMemberDist(4, 8);
						static std::uniform_int_distribution<int> spellsDist(0, 3);

						int spells[] = { 28270, 28272, 28271, 12826 };

						std::shuffle(raidMembers.begin(), raidMembers.end(), randomEngine);
						
						auto numTargets = raidMemberDist(randomEngine);
						for (int i = 0; i < numTargets; ++i)
							target->CastSpell(raidMembers[i], spells[spellsDist(randomEngine)], true);

						//target->CastSpell(target, 23603, true);
						break;
					}
                    default:
                        break;
...
//            }
    ...
    else
    {
        ...
        switch(auraId)
        {
            ...
            case 13810:
            {
                uint32 entrapmentIds[5] = { 19184, 19387, 19388, 19389, 19390 };
                Unit* caster = GetCaster();
                if (!caster)
                    return;
                for (int i = 0; i < 5; i++)
                    if (caster->HasAura(entrapmentIds[i]))
                    {
                        if (roll_chance_f(5.f * (i + 1)))
                            caster->CastSpell(target, 19185, true, NULL, this);
                        return;
                    }
            ...
            case 21737:
            {
                ...
                MaNGOS::AnyPlayerInObjectRangeCheck u_check(GetTarget(), 10.0f);
                MaNGOS::PlayerListSearcher<MaNGOS::AnyPlayerInObjectRangeCheck > checker(targets, u_check);
                Cell::VisitWorldObjects(GetTarget(), checker, 10.0f);

                for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
                    GetTarget()->CastSpell((*itr), triggeredSpellInfo, true, NULL, NULL, casterGUID);
            }
			...
			case 23424:
			{
				...
				static std::uniform_int_distribution<int> spellsDist(0, 3);
				uint32 spells[] = { 23419, 23422, 23420, 23423 };

				target->CastSpell(target, spells[spellsDist(randomEngine)], true);
    ...
    if (triggeredSpellInfo)
    {
        ...
        else
        {
            ...
            if (triggeredSpellInfo->Id == 23619)
            {
				...
				if (health <= 200)
					//explosion:
					triggerTarget->CastSpell(triggerTarget, 23478, true, NULL, NULL, triggerTarget->GetObjectGuid());
        ...
        switch(GetSpellProto()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (GetId() == 23427)
				{
					...
					target->SummonCreature(14668, x + 2, y, z, 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 2000);
					target->SummonCreature(14668, x - 2, y, z, 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 2000);
				}
                break;
            }
    ...
    else
    {
        ...
        switch(GetId())
        {
            ...
            case 12479:                                     // Hex of Jammal'an
                if (GetCaster())
                {
                    GetCaster()->CastSpell(target, 12480, true, NULL, this);
                    GetCaster()->CastSpell(target, 12483, true, NULL, this);
                }
                return;
            ...
            case 22812:										// Barkskin
                ...
                return;
            case 25185:                                     // Itch (AQ 20)
                if (Unit* caster = GetCaster())
                    caster->CastSpell(target, 25187, true, NULL, this);
                break;
            case 26077:                                     // Itch (AQ 40)
                if (Unit* caster = GetCaster())
                    caster->CastSpell(target, 26078, true, NULL, this);
                break;
    ...
    switch(GetSpellProto()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(GetId())
            {
                ...
                {
					//wenn razorgore im kampf ist, wird es unterbrochen. ---> bug/falsch
                    ...
                    if (apply)
                    {
						...
						caster->CastSpell(target, 26264, true);
                    ...
                    else
                    {
						...
						caster->RemoveAurasDueToSpell(26264);
                    }
                    return;
                }
                ...
                case 24661:                                 // Restless Strength
                {
                    if (apply)
                    {
                        ...
                        if (SpellAuraHolder *aura = caster->GetSpellAuraHolder(24662))
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if(apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER && ((Player*)target)->getClass() == CLASS_PALADIN)
            target->m_extraAttacks = 0;

        if (target->GetPet())
            target->GetPet()->InterruptNonMeleeSpells(false);
    }
    else
    {
        target->Unmount(true);
        ...
        if (PowerType != POWER_MANA)
        {
            ...
            switch (form)
            {
                ...
                case FORM_BERSERKERSTANCE:
                {
					...
					if (target->HasAura(12303) && apply)
						target->RemoveAura(12303, SpellEffectIndex::EFFECT_INDEX_0);
					if (target->HasAura(12788) && apply)
						target->RemoveAura(12788, SpellEffectIndex::EFFECT_INDEX_0);
					if (target->HasAura(12789) && apply)
						target->RemoveAura(12789, SpellEffectIndex::EFFECT_INDEX_0);
					if (target->HasAura(12791) && apply)
						target->RemoveAura(12791, SpellEffectIndex::EFFECT_INDEX_0);
					if (target->HasAura(12792) && apply)
						target->RemoveAura(12792, SpellEffectIndex::EFFECT_INDEX_0);
    ...
    if (apply)
    {
        ...
        else
        {
            ...
            if (!ci)
            {
                if (GetId() == 23603)
                {
                    uint32 displayIds[] = { 4473, 7898, 16259, 16143, 1060, 16138, 6087, 1547 };
                    model_id = displayIds[urand(0, 7)];
                }
                else if (GetId() == 19937)
                    model_id = 8310;
                else
...
void Aura::HandleModCharm(bool apply, bool Real)
{
    ...
    if(apply)
	{
		...
		//caster->getThreatManager().modifyThreatPercent(target, -90);
		//target->DeleteThreatList();
        //target->getHostileRefManager().deleteReferences();
...
void Aura::HandleAuraModStun(bool apply, bool Real)
{
    ...
    if (apply)
    {
		...
		//Entfernt Verstohlenheit, wenn das Target gestunnt ist.
    ...
    else
    {
        ...
        if(!target->hasUnitState(UNIT_STAT_ROOT))         // prevent allow move if have also root effect
        {
            ...
            if (target && target->isAlive() && target->GetTypeId() == TYPEID_UNIT && target->GetCanChase())
            {
                if (HostileReference* ref = target->getThreatManager().getCurrentVictim())
                {
                    Unit* unitTarget = ref->getTarget();
                    if (unitTarget && unitTarget->isAlive())
                        target->GetMotionMaster()->MoveChase(unitTarget, 0.0f, 0.0f);
                }
            }
...
void Aura::HandleAuraModSilence(bool apply, bool Real)
{
    ...
    if(apply)
    {
        ...
        for (uint32 i = CURRENT_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
            if (std::shared_ptr<Spell> spell = target->GetCurrentSpell(CurrentSpellTypes(i)))
...
void Aura::HandleAuraModIncreaseSpeed(bool apply, bool Real)
{
    ...
    if (GetTarget()->GetTypeId() == TYPEID_PLAYER)
        GetTarget()->SetAntiCheatImmuneTimer(2000);
...
void Aura::HandlePeriodicTriggerSpell(bool apply, bool /*Real*/)
{
    ...
    if (!apply)
    {
        switch(GetId())
        {
            ...
            case 10212:
            case 25345:
                if (m_removeMode == AURA_REMOVE_BY_EXPIRE && m_periodicTick < ((uint32)GetAuraMaxDuration() / 1000))
                {
                    uint32 trigger_spell_id = GetSpellProto()->EffectTriggerSpell[m_effIndex];
                    if (trigger_spell_id)
            ...
            break;
            case 13278:                                     // Gnome Deathray
                target->RemoveAurasDueToSpell(13493);
                if (m_removeMode == AURA_REMOVE_BY_EXPIRE && !m_periodicTick)
                {
                    target->CastSpell(target, 13280, true);
                    return;
                }
                else
                    target->SetChannelObjectGuid(ObjectGuid());
                break;
...
void Aura::HandlePeriodicEnergize(bool apply, bool /*Real*/)
{
	...
	else if (GetId() == 29131) // Warrior Bloodrage
	{
		auto target = GetTarget();
		if (apply)
		{
			target->SetCombatTimer(1500);
			target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IN_COMBAT);
		}
		else
		{
			if (!target->isAttackingPlayer() && target->getAttackers().size() <= 0)
			{
    ...
    if (apply)
    {
        ...
        if(m_modifier.m_auraname == SPELL_AURA_PERIODIC_DAMAGE)
        {
            ...
            if (spellProto->SpellIconID == 536)
            {
                ...
                Item*  pWeapon = caster->GetTypeId() == TYPEID_PLAYER ? ((Player*)caster)->GetWeaponForAttack(RANGED_ATTACK, true, false) : NULL;
                Unit::AuraList const& mModDamagePercentDone = caster->GetAurasByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
                for (Unit::AuraList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
                {
                    if ((*i)->GetModifier()->m_miscvalue & GetSpellSchoolMask(GetSpellProto()) &&  // schoolmask has to fit with the intrinsic spell school
                ...
                m_modifier.m_amount = (int)(DonePercent * m_modifier.m_amount);
            }
            // SpellDamageBonusDone for magic spells
            else if (spellProto->DmgClass == SPELL_DAMAGE_CLASS_NONE || spellProto->DmgClass == SPELL_DAMAGE_CLASS_MAGIC)
                m_modifier.m_amount = caster->SpellDamageBonusDone(target, GetSpellProto(), m_modifier.m_amount, DOT, GetStackAmount());
...
void Aura::HandleAuraModResistanceExclusive(bool apply, bool /*Real*/)
{
    for(int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL;x++)
    {
        if(m_modifier.m_miscvalue & int32(1<<x))
        {
			if(GetTarget()->GetTypeId() == TYPEID_PLAYER && x == 1) //No holy resi for players
				continue;
...
void Aura::HandleModResistancePercent(bool apply, bool /*Real*/)
{
    ...
    for(int8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; i++)
    {
        ...
        {
			if(target->GetTypeId() == TYPEID_PLAYER && i == 1) //No holy resi for players
				continue;
...
void Aura::HandleModBaseResistance(bool apply, bool /*Real*/)
{
    ...
    if (GetTarget()->GetTypeId() == TYPEID_PLAYER || ((Creature*)GetTarget())->IsPet())
    {
        ...
        if (GetTarget()->GetTypeId() != TYPEID_PLAYER && m_modifier.m_miscvalue & SPELL_SCHOOL_MASK_NORMAL)
        ...
        for(int i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; i++)
		{
			...
			{
				if(GetTarget()->GetTypeId() == TYPEID_PLAYER && i == 1) //No holy resi for players
					continue;
...
void Aura::HandleAuraModIncreaseHealthPercent(bool apply, bool Real)
{
    ...
    switch (GetId())
	{
		case 23310:		// Time Lapse
			if(apply)
			{
				auto amount = target->GetMaxHealth() / 2;
			...
			{
				auto amount = target->GetMaxHealth();
...
void Aura::HandleAuraModRangedHaste(bool apply, bool /*Real*/)
{
	...
	// Berserker -> ranged comes as -5 instead of 5
...
void Aura::HandleShapeshiftBoosts(bool apply)
{
    ...
    if(apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
			...
			if (GetCaster() && GetCaster()->getClass() == CLASS_WARRIOR)
			{
				Unit::SpellAuraHolderMap& tAuras = target->GetSpellAuraHolderMap();
				for (Unit::SpellAuraHolderMap::iterator itr = tAuras.begin(); itr != tAuras.end();)
				{
					if (itr->second->IsRemovedOnShapeLost(form))
					{
...
void Aura::PeriodicTick()
{
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            target->CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), DOT, pdamage, &absorb, &resist, !spellProto->HasAttribute(SPELL_ATTR_EX2_CANT_REFLECTED), !spellProto->HasAttribute(SPELL_ATTR_EX4_BINARY));
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            if (!target->isAlive() && pCaster->IsNonMeleeSpellCasted(false))
                for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
                    if (std::shared_ptr<Spell> spell = pCaster->GetCurrentSpell(CurrentSpellTypes(i)))
        ...
        case SPELL_AURA_MOD_REGEN:
        {
            ...
            // Hack for Skorpid Surprise
            if (spellProto->Id == 6410)
            {
                const SpellEntry*  spell = sSpellStore.LookupEntry(spellProto->EffectTriggerSpell[1]);
                if (roll_chance_i(20))
...
bool SpellAuraHolder::IsRemovedOnShapeLost(ShapeshiftForm newForm) const {
	if (newForm == FORM_NONE || (GetCaster() && GetCaster()->getClass() != CLASS_WARRIOR))
        return m_isRemovedOnShapeLost;

	uint32 mask = (1 << (newForm - 1));
	bool isInNewForm = (m_spellProto->Stances & mask) != 0;
	bool isNotInNewForm = (m_spellProto->StancesNot & mask) != 0;
	...
	if(m_isRemovedOnShapeLost == true) {
		return isNotInNewForm;
	}

	return false;

    //return !(m_spellProto->Stances & (1 << (newForm - 1))) && m_isRemovedOnShapeLost;
}
...
bool SpellAuraHolder::_AddSpellAuraHolder()
{
    if (!GetId())
        return false;
    if(!m_target)
...
void SpellAuraHolder::_RemoveSpellAuraHolder()
{
    ...
    if (m_removeMode != AURA_REMOVE_BY_DELETE)
    {
        ...
        if (caster && caster->GetTypeId() == TYPEID_UNIT && caster->GetOwner() && caster->GetOwner()->GetTypeId() == TYPEID_PLAYER)
        {
            if (GetSpellProto() && GetSpellProto()->HasAttribute(SPELL_ATTR_DISABLED_WHILE_ACTIVE))
            {
                //((Creature*)caster)->AddCreatureSpellCooldown(GetSpellProto()->)
                // Send activate cooldown timer (possible 0) at client side
                WorldPacket data(SMSG_COOLDOWN_EVENT, (4 + 8));
                data << uint32(GetSpellProto()->Id);
                data << caster->GetObjectGuid();
                ((Player*)caster->GetOwner())->SendDirectMessage(&data);
            }
        }
    }
}
...
bool SpellAuraHolder::IsWeaponBuffCoexistableWith(SpellAuraHolder const* ref) const
{
    ...
    // Limit to Weapon-Slots and head/leg slots
    if (!castItem->IsEquipped() ||
        (castItem->GetSlot() != EQUIPMENT_SLOT_MAINHAND && castItem->GetSlot() != EQUIPMENT_SLOT_OFFHAND &&
		castItem->GetSlot() != EQUIPMENT_SLOT_HEAD && castItem->GetSlot() != EQUIPMENT_SLOT_LEGS))
        return false;
