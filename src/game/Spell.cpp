...
bool SpellCastTargets::Update(Unit* caster)
{
...
void Spell::FillTargetMap()
{
    ...
    for(int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
                    ...
                    case TARGET_NONE:
                        ...
                        if(m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && m_spellInfo->SpellFamilyFlags & UI64LIT(0x00000800))
                        {
                            if (m_caster->GetTypeId() == TYPEID_PLAYER)
                                if (Unit *target = ObjectAccessor::Instance().GetUnit(*m_caster, ((Player*)m_caster)->GetTargetGuid()))
		...
		//Arcane Missiles triggers ignore LoS
		if (m_spellInfo->Id == 10274 && tmpUnitMap.size() == 1)
		{
			AddUnitTarget(*(tmpUnitMap.begin()), SpellEffectIndex(i));
			return;
		}
        ...
        // Hack for Intimidating Shout
        if (m_spellInfo->Id == 5246 && i == 1)
        {
            for (TargetList::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
            {
                if (ihit->effectMask & 1)
                    ihit->effectMask = 1;                              // stun our first target
                else
                    ihit->effectMask |= 0x6;                           // fear + slow other targets
            }
            // skip the last effect index, because we want 6 targets
            break;
        }
    }
}
    ...
    if (!m_canTrigger)                // Exceptions (some periodic triggers)
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_MAGE:
                // Arcane Missiles triggers need it on first hit
                if (m_spellInfo->IsFitToFamilyMask(UI64LIT(0x0000000000200000)))
                {
                    if (m_caster && m_triggeredByAuraSpell)
                        if (auto source = m_caster->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
                            if (source && !source->m_currentBasePoints[2])
                            {
                                source->m_currentBasePoints[2] = 1;
                                m_canTrigger = true;
                            }
                }   
                // Blizzard triggers need do it
                if (m_spellInfo->IsFitToFamilyMask(UI64LIT(0x0000000000000080)))
                    m_canTrigger = true;
                break;
...
void Spell::AddUnitTarget(Unit* pVictim, SpellEffectIndex effIndex)
{
    ...
    // one single roll for binary spells
    if (target.missCondition == SPELL_MISS_NONE && m_spellInfo->AttributesEx4 & SPELL_ATTR_EX4_BINARY && !(m_spellInfo->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS))
        if (!pVictim->RollBinarySpellEffectResist(m_caster, GetSpellSchoolMask(m_spellInfo)))
    ...
    if (target.missCondition == SPELL_MISS_REFLECT)
    {
        ...
        // approximated speed through client tests
        target.timeDelay += (uint64)floor(dist / 120 * 1000.0f);;
    }
    else
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else
    {
        ...
        {
            if (unit->IsImmunedToDamage(GetSpellSchoolMask(m_spellInfo)) ||
        ...
        else if (target->missCondition == SPELL_MISS_IMMUNE && 
            !unit->IsImmunedToDamage(GetSpellSchoolMask(m_spellInfo)) &&
            !unit->IsImmuneToSpell(m_spellInfo))
        {
            // Recalculate Hit-Result/Damage
            target->missCondition = m_caster->SpellHitResult(unit, m_spellInfo, m_canReflect);
            HandleDelayedSpellLaunch(target);
        }
    }
    ...
    if (target->damage)
    {
        ...
        {
            damageInfo.attacker = damageInfo.target;
            damageInfo.target = caster;
            damageInfo.HitInfo = target->reflectResult;
            caster->CalculateAbsorbResistBlock(caster, &damageInfo, m_spellInfo);
        }
        ...
        if (target->missCondition == SPELL_MISS_DODGE || target->missCondition == SPELL_MISS_PARRY || target->missCondition == SPELL_MISS_BLOCK)
        {
            switch (m_spellInfo->SpellFamilyName)
            {
                case SPELLFAMILY_WARRIOR:
                    caster->ModifyPower(POWER_RAGE, (int32)((CalculatePowerCost(m_spellInfo, caster, shared_from_this(), m_CastItem))*0.8f));
                    break;
                case SPELLFAMILY_ROGUE:
                {
                    if (!NeedsComboPoints(m_spellInfo))
						caster->ModifyPower(POWER_ENERGY, (int32)((CalculatePowerCost(m_spellInfo, caster, shared_from_this(), m_CastItem))*0.8f));
                    break;
                }
    ...
    if(real_caster && real_caster != unit)
    {
        ...
        {
            bool sameFaction = unit->getFaction() == real_caster->getFaction();
            if (!unit->isInCombat() && unit->GetTypeId() != TYPEID_PLAYER && ((Creature*)unit)->AI() && !sameFaction)
            {
                ((Creature*)unit)->AI()->AttackedBy(real_caster);
                unit->AddThreat(real_caster);
            }

            if (!unit->isInCombat() && real_caster->GetTypeId() != TYPEID_PLAYER && ((Creature*)real_caster)->AI() && !sameFaction)
            ...
            !IsPositiveSpell(m_spellInfo->Id) && m_caster->isVisibleForOrDetect(unit, unit, false))
        {
            ...
            if ((missInfo == SPELL_MISS_MISS || missInfo == SPELL_MISS_RESIST)  && !sameFaction)
            {
                unit->AddThreat(real_caster);
                unit->SetInCombatWith(real_caster);
                real_caster->SetInCombatWith(unit);
	...
	{
		auto ai = static_cast<Creature*>(unit)->AI();
	
		if(ai != nullptr)
			ai->PreSpellHit(m_caster, m_spellInfo, m_damage);
	}
    ...
    if (m_healing)
    {
        ...
        // Don't proc on extra heals from Healing Wave
        if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellIconID == 13 && m_spellInfo->SpellVisual == 58)
        {
            for (TargetList::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                {
                    if (uint64((*ihit).targetGUID) != unitTarget->GetGUID() && (*ihit).processed)
                    {
                        m_canTrigger = false;
                        break;
                    }
    ...
    {
		//Stormstrike does white dmg
		if(damageInfo.SpellID == 17364 && unitTarget)
		{
            caster->AttackerStateUpdate(unitTarget, WeaponAttackType::BASE_ATTACK, true);
		}
		else
		{
            ...
            if (damageInfo.damage || damageInfo.target->isAlive())
			    caster->SendSpellNonMeleeDamageLog(&damageInfo);
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask, bool isReflected)
{
    ...
    if (realCaster && realCaster != unit)
    {
        ...
        if (!realCaster->IsFriendlyTo(unit))
        {
            ...
            bool breakStealth = !(m_spellInfo->AttributesEx & SPELL_ATTR_EX_NOT_BREAK_STEALTH);

            // Earthbind Totem Hack (can only hit stealth targets with the first tick)
            if (m_triggeredByAuraSpell && m_spellInfo->Id == 3600)
                if (SpellAuraHolder *holder = m_originalCaster->GetSpellAuraHolder(m_triggeredByAuraSpell->Id))
                    if (holder->GetAuraApplyTime() < time(NULL) - 2)
                        breakStealth = false;
			...
			if (!(m_spellInfo->AttributesEx3 & SPELL_ATTR_EX3_NO_INITIAL_AGGRO)  && !(m_spellInfo->AttributesEx & SPELL_ATTR_EX3_UNK21)
				&& !IsPositiveSpell(m_spellInfo->Id) && m_caster->isVisibleForOrDetect(unit, unit, false)
    ...
    if (rollEffectChances && m_spellInfo->AttributesEx4 & SPELL_ATTR_EX4_BINARY)
        rollEffectChances = false;

	//No hit check for class spells
	switch(m_spellInfo->Id)
	{
	case 23410:
	case 23398:
            ...
            if ( m_applyMultiplierMask & (1 << effectNumber) )
            {
                ...
                if (realCaster)
                    if(Player* modOwner = realCaster->GetSpellModOwner())
						modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_EFFECT_PAST_FIRST, multiplier, shared_from_this());
    ...
    if (m_spellAuraHolder)
    {
		...
		case 26464:                                         // Mercurial Shield
			m_spellAuraHolder->SetStackAmount(m_spellInfo->StackAmount);
			break;
		}
...
void Spell::InitializeDamageMultipliers()
{
    for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        if (Unit* realCaster = GetAffectiveCaster())
            if(Player* modOwner = realCaster->GetSpellModOwner())
				modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_JUMP_TARGETS, EffectChainTarget, shared_from_this());
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    if (Unit* realCaster = GetAffectiveCaster())
    {
        if(Player* modOwner = realCaster->GetSpellModOwner())
        {
			modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_RADIUS, radius, shared_from_this());
			modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_JUMP_TARGETS, EffectChainTarget, shared_from_this());
        }
    }
    ...
    switch(targetMode)
    {
		case TARGET_CURRENT_TARGET:
			targetUnitMap.push_back(m_caster->GetMap()->GetUnit(m_caster->GetTargetGuid()));
			break;
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            {
				if (Unit* pUnitTarget = m_caster->SelectMagnetTarget(m_targets.getUnitTarget(), shared_from_this(), effIndex))
                {
            ...
            else
            {
                ...
                while(t && next != tempTargetUnitMap.end() )
                {
                    ...
                    if(!prev->IsWithinLOSInMap(*next) || (m_spellInfo->Attributes & (SPELL_ATTR_ON_NEXT_SWING_1|SPELL_ATTR_UNK4|SPELL_ATTR_UNK18)) && !m_caster->isInFront(*next,radius))
                    {
                        ++next;
                        continue;
                    }
...
void Spell::prepare(SpellCastTargets const* targets, Aura* triggeredByAura)
{
	...
	SpellEvent* Event = new SpellEvent(shared_from_this());
	Event->m_owner = m_caster;
    m_caster->m_Events.AddEvent(Event, m_caster->m_Events.CalculateTime(1));

    //Prevent casting at cast another spell (ServerSide check)
	if (!m_IsTriggeredSpell && m_caster->IsNonMeleeSpellCasted(false, true, true) && m_spellInfo->Id != 25863 && m_spellInfo->Id != 26655)
    {
    ...
    // Fill cost data
	m_powerCost = CalculatePowerCost(m_spellInfo, m_caster, shared_from_this(), m_CastItem);

    SpellCastResult result = CheckCast(true);
    if(result != SPELL_CAST_OK && !IsAutoRepeat() && m_spellInfo) //always cast autorepeat dummy for triggering
    {
        if(triggeredByAura)
        {
            SendChannelUpdate(0);
    ...
    // create a visual effect while tapping flags in bg
    if (targets->getGOTarget() && targets->getGOTarget()->GetMap()->IsBattleGround())
    {
        int32 duration = GetSpellCastTime(m_spellInfo);
        DynamicObject* dynObj = new DynamicObject;

        float x, y, z;

        targets->getGOTarget()->GetPosition(x, y, z);

        // set radius to 0: spell not expected to work as persistent aura
        if (!duration || !dynObj->Create(m_caster->GetMap()->GenerateLocalLowGuid(HIGHGUID_DYNAMICOBJECT),  
            m_caster, 6197, EFFECT_INDEX_0, x, y, z + 2.f, duration, 0, DYNAMIC_OBJECT_FARSIGHT_FOCUS))
            delete dynObj;
        else
        {
	...
	m_casttime = GetSpellCastTime(m_spellInfo, shared_from_this());
    ...
    if (!m_IsTriggeredSpell)
    {
		...
		m_caster->SetCurrentCastedSpell(shared_from_this());

        // Hack for Succubus Seduction
        if (m_spellInfo->Id == 6358)
            m_caster->RemoveAurasDueToSpell(7870);
    ...
    else
    {
        ...
        if (IsChanneledSpell(m_spellInfo) && !m_caster->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
			m_caster->SetCurrentCastedSpell(shared_from_this(), true);
...
void Spell::cast(bool skipCheck)
{
    ...
    if (m_spellInfo->Id == 20647)                           // execute won't drain rage on miss/dodge/parry etc.
    {
        for (auto itr : m_UniqueTargetInfo)
            if (itr.missCondition == SPELL_MISS_BLOCK || itr.missCondition == SPELL_MISS_DODGE
                || itr.missCondition == SPELL_MISS_MISS || itr.missCondition == SPELL_MISS_PARRY)
                m_powerCost = 0;
    }
    ...
    if (speed > 0.0f)
    {
        ...
        // remove spell mods immediately
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
        {
            auto* player = static_cast<Player*>(m_caster);
            player->RemoveSpellMods(shared_from_this());
            if (player->mAI)
                player->mAI->Chase();
        }
    ...
    // remove spell mods for channeled spell
...
void Spell::handle_immediate()
{
	...
	//Hackfix
	if(m_spellInfo->Id == 28441)
		sScriptMgr.OnEffectDummy(m_caster, 28441, EFFECT_INDEX_0, m_caster);
...
void Spell::SendSpellCooldown()
{
	...
	if(m_caster->GetTypeId() == TYPEID_PLAYER)
		...
		Unit* uCaster = (Unit*)m_caster;
		if(uCaster->IsControlledByPlayer() == false)
			return;

        if (uCaster->GetTypeId() == TYPEID_UNIT && ((Creature*)uCaster)->IsPet())
            return;

		_player = (Player*)uCaster->GetCharmerOrOwner();
	...
	_player->AddSpellAndCategoryCooldowns(m_spellInfo, m_CastItem ? m_CastItem->GetEntry() : 0, shared_from_this());
}
...
void Spell::update(uint32 difftime)
{
    ...
    if (((m_caster->GetTypeId() == TYPEID_PLAYER || m_caster->GetCharmer()) && m_timer != 0) &&
    ...
    switch(m_spellState)
    {
        case SPELL_STATE_PREPARING:
        {
			...
			//I have no idea what this should check, but this prevents razorgore from casting while being possessed
...
void Spell::finish(bool ok)
{
    ...
    if(!ok)
    {
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
        {
            auto* player = static_cast<Player*>(m_caster);
            if (player->IsInWorld())
                player->ResetSpellModsDueToCanceledSpell(shared_from_this());
        }
    ...
    // remove spell mods
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->speed <= 0.f)
	{
		auto* player = static_cast<Player*>(m_caster);
		player->RemoveSpellMods(shared_from_this());
		if(player->mAI)
    ...
    if (IsMeleeAttackResetSpell())
    {
        ...
        {
            if (m_caster->getAttackTimer(BASE_ATTACK) < ATTACK_DISPLAY_DELAY)
                m_caster->setAttackTimer(BASE_ATTACK, ATTACK_DISPLAY_DELAY);
            if (m_caster->haveOffhandWeapon() && m_caster->getAttackTimer(OFF_ATTACK) < ATTACK_DISPLAY_DELAY)
                m_caster->setAttackTimer(OFF_ATTACK, ATTACK_DISPLAY_DELAY);
        }
    }
...
void Spell::SendCastResult(Player* caster, uint32 spellId, SpellCastResult result) {
	SpellEntry const* info = sSpellStore.LookupEntry(spellId);
	if(info == nullptr)
		return;

	SendCastResult(caster, info, result);
}
...
void Spell::WriteSpellGoTargets(WorldPacket* data)
{
    ...
    *data << (uint8)(0);
    ...
    uint8 hit = m_UniqueGOTargetInfo.size();
...
void Spell::CastTriggerSpells()
{
    ...
    {
		auto spell = std::make_shared<Spell>(m_caster, (*si), true, m_originalCasterGUID);
...
{
	if(m_spellInfo->Id == 18947 || m_spellInfo->Id == 19773)
    ...
    if (strict && !m_IsTriggeredSpell)
    {
		...
		if(m_spellInfo->Id == 2457 || m_spellInfo->Id == 71) //Battle Stance & Defensive Stance
		{
			Unit *owner = m_caster;
			auto const& mOverrideClassScript= owner->GetAurasByType(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
			for(auto i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
			{
				if((*i)->GetModifier()->m_miscvalue == 3654) //can not change stance while having nefs berserker debuff
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // some more anti-cheat checks
        if (!m_IsTriggeredSpell && strict && !m_spellInfo->HasAttribute(SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY))
        {
            if (m_caster->HasAuraType(SPELL_AURA_MOD_STUN))
                return SPELL_FAILED_STUNNED;

            if (m_caster->HasAuraType(SPELL_AURA_MOD_FEAR))
                return SPELL_FAILED_FLEEING;

            if (m_caster->HasAuraType(SPELL_AURA_MOD_CONFUSE))
	...
	for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
	{
		...
		{
			// do not check for Shield Bash, Poison Cleansing Totem, Disease Cleansing Totem and Abolish Poison
			if (m_spellInfo->SpellIconID ==  413 ||
                m_spellInfo->SpellIconID == 1679 ||
                m_spellInfo->SpellIconID == 1673 ||
				m_spellInfo->SpellIconID ==  265 ||
                m_spellInfo->Id == 25756)
				continue;

			// It is a dispell spell
			isDispell = true;

            if (isDispell && !target->IsFriendlyTo(m_caster) && target->IsImmunedToDamage(GetSpellSchoolMask(m_spellInfo)))
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && IsAreaOfEffectSpell(m_spellInfo) && !m_targets.getUnitTarget())
	{
		if (!GetCaster()->IsWithinLOS(m_targets.m_destX, m_targets.m_destY, m_targets.m_destZ))
    ...
    if(Unit *target = m_targets.getUnitTarget())
    {
        ...
        // Special Case: Arcane Missiles
        if (m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && m_spellInfo->SpellFamilyFlags & UI64LIT(0x00000800))
            non_caster_target = true;
        ...
        if(non_caster_target)
        {
            ...
            if (!explicit_target_mode && m_caster->GetTypeId() == TYPEID_UNIT && m_caster->GetCharmerOrOwnerGuid())
            {
                ...
                if(IsPositiveSpell(m_spellInfo->Id))
                {
                    ...
                    if(target_hostile && m_caster->GetEntry() != 417)
                        return SPELL_FAILED_BAD_TARGETS;
                }
                else
                {
    ...
    if (m_UniqueTargetInfo.empty())                         // skip second CheckCast apply (for delayed spells for example)
    {
        for(int j = 0; j < MAX_EFFECT_INDEX; ++j)
        {
               ...
               m_spellInfo->EffectImplicitTargetA[j] == TARGET_FOCUS_OR_SCRIPTED_GAMEOBJECT)
            {
				...
				if(m_spellInfo->Id == 23415) //Siphon Blessing
				{
                    MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster, 11583, true, 100);
                    MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(creatureScriptTarget, u_check);
					Cell::VisitAllObjects(m_caster, searcher, range);
				}
                ...
                {
//					if(goScriptTarget->GetRespawnTime() != 0)
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            case SPELL_EFFECT_DUMMY:
            {
                ...
                else if (m_spellInfo->Id == 8892)          // Rocket Boots
                {
                    if (m_caster->IsInWater())
                        return SPELL_FAILED_ONLY_ABOVEWATER;
                }
                else if (m_spellInfo->Id == 13141)		  // Rocket Boots
                {
                    if (m_caster->IsInWater())
                        return SPELL_FAILED_ONLY_ABOVEWATER;
                }
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    float x, y, z;
                    m_targets.getUnitTarget()->GetContactPoint(m_caster, x, y, z, 0.2f);

                    Movement::MoveSplineInit init(*m_caster);
                    init.MoveTo(x, y, z, true, true);
                    init.SetVelocity(40.0f);
                    uint32 travelTime = init.Launch(1400, true);
                    if (travelTime > 1400)
                    {
                        init.Stop(true);
                        ...
                        ((Player*)m_caster)->setAttackTimer(BASE_ATTACK, (uint32)(travelTime / m_caster->GetSpeedRate(MOVE_RUN)));
                }
                break;
            }
            case SPELL_EFFECT_SKINNING:
            {
                ...
                if( (m_selfContainer && (*m_selfContainer).get() == this) &&
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
				...
				if (skillId != SKILL_NONE && (!m_selfContainer || ((*m_selfContainer).get() != this)))
                {
            ...
            case SPELL_EFFECT_SUMMON_DEMON:
            {
                ...
                {
                    // Unsummon Pet for Inferno
                    if (m_spellInfo->Id == 1122
                        && m_caster->GetPet())
                        m_caster->GetPet()->Unsummon(PET_SAVE_AS_DELETED);
                    else
                        return SPELL_FAILED_ALREADY_HAVE_SUMMON;
                }
        ...
        switch(m_spellInfo->EffectApplyAuraName[i])
        {
            case SPELL_AURA_MOD_POSSESS:
            {
                ...
                if ((int32(m_targets.getUnitTarget()->getLevel()) > CalculateDamage(SpellEffectIndex(i),m_targets.getUnitTarget())
					|| int32(m_targets.getUnitTarget()->getLevel()) > 62) && m_spellInfo->Id != 19832)
...
SpellCastResult Spell::CheckPetCast(Unit* target)
{
    ...
    if(m_caster->GetTypeId()==TYPEID_UNIT && (((Creature*)m_caster)->IsPet() || m_caster->isCharmed()))
    {
		...
		{
			if(m_caster->isCharmed() == false || m_caster->GetMap() == nullptr || m_caster->GetCharmer() == nullptr)
				target = m_targets.getUnitTarget();
			else
				target = m_caster->GetMap()->GetCreature(m_caster->GetCharmer()->GetTargetGuid());
		}
        ...
        if(_target)                                         //for target dead/target not valid
        {
            ...
            if(IsPositiveSpell(m_spellInfo->Id))
            {
                if(m_caster->IsHostileTo(_target) && m_caster->GetEntry() != 417)
                    return SPELL_FAILED_BAD_TARGETS;
            }
            else
            {
                ...
                if(!target || (m_caster->IsFriendlyTo(target) && !duelvsplayertar))
                {
                    return SPELL_FAILED_BAD_TARGETS;
                }
            }
...
SpellCastResult Spell::CheckRange(bool strict)
{
        ...
        case SPELL_RANGE_IDX_COMBAT:
        {
            if (target)
            {
                ...
                if (Player* modOwner = m_caster->GetSpellModOwner())
					range_mod += modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_RANGE, base, shared_from_this());
    ...
    if(Player* modOwner = m_caster->GetSpellModOwner())
		modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_RANGE, max_range, shared_from_this());
...
uint32 Spell::CalculatePowerCost(SpellEntry const* spellInfo, Unit* caster, const std::shared_ptr<Spell> spell, Item* castItem)
{
...
SpellCastResult Spell::CheckItems()
{
    ...
    if(m_CastItem)
    {
        ...
        if(itemid && !p_caster->HasItemCount(itemid, 1))
            return SPELL_FAILED_ITEM_NOT_READY;
...
void Spell::Delayed()
{
	...
	((Player*)m_caster)->ApplySpellMod(m_spellInfo->Id, SPELLMOD_NOT_LOSE_CASTING_TIME, resistChance, shared_from_this());
...
bool Spell::UpdatePointers()
{
	...
	return m_targets.Update(m_caster);
}
...
bool Spell::CheckTarget(Unit* target, SpellEffectIndex eff)
{
	...
	//Nefs Shadow Flame will always hit every player, so don't perform LoS check
	switch(m_spellInfo->Id)
	{
	case 22992:
	case 22972:
    ...
    switch(m_spellInfo->Effect[eff])
    {
        ...
        default:                                            // normal case
            ...
            if (target != m_caster && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_triggeredByAuraSpell)
                if (WorldObject *caster = GetCastingObject())
...
SpellEvent::SpellEvent(std::shared_ptr<Spell> spell) : BasicEvent()
{
	m_Spell = spell;
	this->m_owner = m_Spell->GetCaster();
	if(spell->m_CastItem)
	{
		this->m_guid = spell->m_CastItem->GetGUID();
		this->m_name = spell->m_CastItem->GetProto()->Name1;
	}
	this->m_spellId = spell->m_spellInfo->Id;
	this->m_target = sObjectAccessor.GetUnit(*m_owner, m_owner->GetTargetGuid());
...
void Spell::CancelGlobalCooldown()
{
	...
	if (m_caster->GetCurrentSpell(CURRENT_GENERIC_SPELL).get() != this)
        return;
