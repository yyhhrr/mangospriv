...
namespace MaNGOS
{
    ...
    */
    ...
    class AnyUnfriendlyTargetableUnitToOwnerInObjectRangeCheck
    {
    public:
        AnyUnfriendlyTargetableUnitToOwnerInObjectRangeCheck(WorldObject const* obj, Unit const* funit, float range) : i_obj(obj), i_funit(funit), i_range(range) {}
        WorldObject const& GetFocusObject() const { return *i_obj; }
        bool operator()(Unit* u)
        {
            if (u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u) && u->isTargetableForAttack())
                return true;
            else
