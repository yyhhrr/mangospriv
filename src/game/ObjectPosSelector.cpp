...
#define OCCUPY_POS_ANGLE_ATAN_FACTOR 1.8f / 2.f
...
m_centerX(x), m_centerY(y), m_searcherDist(dist), m_searchPosFor(searchPosFor)
{
    ...
    m_searchedForReqHAngle = atan(OCCUPY_POS_ANGLE_ATAN_FACTOR * searchedForSize / m_searcherDist) / 3.f;
...
void ObjectPosSelector::AddUsedArea(WorldObject const* obj, float angle, float dist)
{
    ...
    if (dist == 0.0f)
        return;
    float objSize = obj->GetObjectBoundingRadius();
    // possible db issue?
    if (!objSize)
        objSize = (obj->GetTypeId() == TYPEID_UNIT ? DEFAULT_OBJECT_SCALE : DEFAULT_WORLD_OBJECT_SIZE);
    // (half) angle that obj occupies
    float sr_angle = atan(OCCUPY_POS_ANGLE_ATAN_FACTOR * objSize / dist);
    if (angle >= 0)
...
bool ObjectPosSelector::NextAngle(float& angle)
{
    ...
    for (;;)
    {
        ...
        if (m_stepAngle[USED_POS_PLUS] < M_PI_F / 3.f && m_stepAngle[USED_POS_PLUS] <= m_stepAngle[USED_POS_MINUS])
        {
            if (NextSideAngle(USED_POS_PLUS, angle))
                return true;
        }
        // -- direction less updated
        else if (m_stepAngle[USED_POS_MINUS] < M_PI_F / 3.f)
        {
...
bool ObjectPosSelector::NextSideAngle(UsedAreaSide side, float& angle)
{
    ...
    if (m_stepAngle[side] > M_PI_F / 3.f)
        return false;
