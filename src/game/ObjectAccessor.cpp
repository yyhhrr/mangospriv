...
void ObjectAccessor::RemoveObject(Player *object)
{
    ...
    for (std::deque<Player*>::const_iterator itr = i_deletablePlayer.cbegin(); itr != i_deletablePlayer.cend(); itr++)
        if (*itr == object)
            return;
    i_deletablePlayer.push_back(object);
}

void ObjectAccessor::DeletePlayer()
{
    while (!i_deletablePlayer.empty())
    {
        Player* plr = i_deletablePlayer.back();
        i_deletablePlayer.pop_back();
        if (plr)
            delete plr;
    }
}
