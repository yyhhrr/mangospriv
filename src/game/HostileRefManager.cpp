...
void HostileRefManager::threatAssist(Unit *pVictim, float pThreat, SpellEntry const *pThreatSpell, bool pSingleTarget)
{
    ...
    while (ref)
    {
		...
		if (pThreatSpell && pThreatSpell->EffectApplyAuraName[EFFECT_INDEX_0] == SPELL_AURA_MOD_TOTAL_THREAT)
...
void HostileRefManager::deleteReferencesForGUID(const ObjectGuid& guid)
{
    HostileReference* ref = getFirst();
    while(ref)
    {
        HostileReference* nextRef = ref->next();
        if(ref->getSource()->getOwner()->GetObjectGuid() == guid)
        {
            ref->removeReference();
            delete ref;
        }
