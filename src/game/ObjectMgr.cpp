...
Player* ObjectMgr::GetPlayer(ObjectGuid guid, bool inWorld) { return ObjectAccessor::FindPlayer(guid, inWorld); }
...
void ObjectMgr::LoadHintText()
{
    QueryResult* result = WorldDatabase.Query("SELECT tipp_text FROM world_tipp;");
...
void ObjectMgr::LoadCreatureResilience()
{
    QueryResult *result = WorldDatabase.Query("SELECT entry,resilience FROM creature_template_resilience");
    ...
    if (!result)
    {
        ...
        sLog.outString("");
        sLog.outString(">> Loaded 0 creature resilience templates. DB table `creature_template_resilience` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (mCreatureResilienceMap.find(entry) != mCreatureResilienceMap.end())
            continue;

        mCreatureResilienceMap[entry] = fields[1].GetUInt8();
    ...
    sLog.outString();
    sLog.outString(">> Loaded %u creature resilience templates", mCreatureResilienceMap.size());
}
        ...
        do {
            ...
            if (honorResult)
            {
                do
                {
                    ...
                    else if (type == TYPEID_OBJECT) // Bonus - Extra honor
                        honor += fields[2].GetUInt32();
                }
