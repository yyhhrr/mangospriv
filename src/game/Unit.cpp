...
void MovementInfo::Read(ByteBuffer &data)
{
	...
	if (HasMovementFlag(MOVEFLAG_FALLING) || (HasMovementFlag(MOVEFLAG_FLYING) && data.size() != 28))
	{
		data >> jump.velocity;
		data >> jump.sinAngle;
		data >> jump.cosAngle;
...
Unit::Unit()
    : WorldObject(), i_motionMaster(this), m_ThreatManager(this), m_HostileRefManager(this), mFearManager(this), m_canChase(true), movespline(new Movement::MoveSpline()), m_setInactiveOnEvade(false), m_chargeTarget(false)
{
    ...
    m_AutoRepeatPrepareAnimation = false;
	m_extraAttacks = 0;
	...
	m_AntiCheatImmuneTimer = 0;
	m_AntiCheatCounter = 0;
	m_AntiCheatCounterTimer = 15000;
	...
	for (uint32 i = 0; i < MAX_SPELL_SCHOOL; ++i)
	{
		for(uint32 j = 0; j < 3; ++j)
		{
			...
			{
				m_weaponElementalDamage[i][j][k] = 0;
				m_weaponElementalDamage[i][j][k] = 0;
			}
		}
	}
...
void Unit::Update(uint32 update_diff, uint32 p_time)
{
	...
	if (m_AntiCheatCounter > 0)
	{
		debug_log("CleatAntiCheatCounter: %u", m_AntiCheatCounterTimer);
		uint32 pCount;
		pCount = GetAntiCheatCounter();
		if (m_AntiCheatCounterTimer <= update_diff)
		{
			pCount = pCount - 1;
			SetAntiCheatCounter(pCount);
			debug_log("AntiCheatCounter wurde um 1 reduziert. Aktuelle Anzahl: %u", pCount);
			m_AntiCheatCounterTimer = 15000;
		}
		else
		{
			m_AntiCheatCounterTimer -= update_diff;
		}
	}

	if (m_AntiCheatImmuneTimer)
	{
		debug_log("Zeit: %u", m_AntiCheatImmuneTimer);
		if (m_AntiCheatImmuneTimer <= update_diff)
		{
			m_AntiCheatImmuneTimer = 0;
		}
		else
		{
			m_AntiCheatImmuneTimer -= update_diff;
		}
	}
...
bool Unit::UpdateMeleeAttackingState()
{
	...
	if (player && !player->GetCharmerGuid() && swingError != player->LastSwingErrorMsg())
	{
		if (swingError == 1)
	...
	for (SpellAuraHolderMap::iterator iter = m_spellAuraHolders.begin(); iter != m_spellAuraHolders.end();)
	{
		...
		if (spell->Attributes & SPELL_ATTR_UNK30 || spell->Attributes & SPELL_ATTR_STOP_ATTACK_TARGET || spell->Id == 5246)
		{
			for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
			{
                ...
                if (spell->Id == 20511)   // Intimidating Shout
					instantBreak = true;
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
	...
	if (pVictim != this)
	{
		...
		if (damagetype == DIRECT_DAMAGE || damagetype == SPELL_DIRECT_DAMAGE)
			GetPosition(pVictim->mLastAttackPosition.x, pVictim->mLastAttackPosition.y, pVictim->mLastAttackPosition.z);

        if (pVictim && GetTypeId() == TYPEID_PLAYER && pVictim->GetTypeId() == TYPEID_PLAYER && pVictim->IsPvP())
            ((Player*)this)->UpdatePvP(true);
	}
	...
	if (pVictim->GetTypeId() == TYPEID_UNIT && pVictim->GetCreatureType() == CREATURE_TYPE_CRITTER)
	{
		...
		pVictim->getFearManager().onDamageReceived(damage);
		return damage;
	}
	...
	//Get in CombatState
	if (pVictim != this && damagetype != DOT && damage < health)
	{
	...
	if (health <= damage)
	{
        ...
        if (pVictim->HasAura(27827)) // Prevent dying twice with spirit of redemption
        {
            pVictim->SetHealth(1);
            return 0;
        }
		...
		if (pVictim->GetTypeId() == TYPEID_PLAYER && ((Player*)pVictim)->InBattleGround())
		{
			...
			if (player_tap)
			{
				...
				// Used by Eluna
				sEluna->OnPVPKill(player_tap, killed);
			}
		}
	...
	else                                                    // if (health <= damage)
	{
		...
		for (SpellAuraHolderMap::const_iterator i = vAuras.begin(), next; i != vAuras.end(); i = next)
		{
			...
			next = i; ++next;
			if (spellProto && spellProto->Id == se->Id || se->Id == 23018) // Not drop auras added by self
				continue;
		...
		if (damagetype != NODAMAGE && damage && pVictim->GetTypeId() == TYPEID_PLAYER)
		{
			if (damagetype != DOT)
			{
				for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
				{
					...
					if (std::shared_ptr<Spell> spell = pVictim->GetCurrentSpell(CurrentSpellTypes(i)))
					{
			...
			if (std::shared_ptr<Spell> spell = pVictim->m_currentSpells[CURRENT_CHANNELED_SPELL])
			{
...
void Unit::JustKilledCreature(Creature* victim)
{
	...
	if (spellInfo && spellInfo->HasAttribute(SPELL_ATTR_EX_FARSIGHT) && spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNELED_1))
	{
		...
		{
			std::shared_ptr<Spell> channeledSpell = creator->GetCurrentSpell(CURRENT_CHANNELED_SPELL);
...
void Unit::CastSpell(Unit* Victim, SpellEntry const *spellInfo, bool triggered, Item *castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
	...
	auto spell = std::make_shared<Spell>(this, spellInfo, triggered, originalCaster, triggeredBy);
...
bool Unit::CanCast(Unit* target, uint32 spellId, bool triggered)
{
	if (!target)
		return false;
	...
	if (!triggered)
	{
		...
		if (this->hasUnitState(UNIT_STAT_CAN_NOT_REACT_OR_LOST_CONTROL))
			return false;

		if (this->hasUnitState(UNIT_STAT_FLEEING_MOVE))
			return false;

		if (spell->PreventionType == SPELL_PREVENTION_TYPE_SILENCE && this->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SILENCED))
			return false;

		if (spell->PreventionType == SPELL_PREVENTION_TYPE_PACIFY && this->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED))
			return false;

		// Check for power (also done by Spell::CheckCast())
		if (this->GetPower((Powers)spell->powerType) < Spell::CalculatePowerCost(spell, this))
			return false;
	}

	if (const SpellRangeEntry *pSpellRange = sSpellRangeStore.LookupEntry(spell->rangeIndex))
	{
		if (target != this)
		{
			// pTarget is out of range of this spell (also done by Spell::CheckCast())
			float fDistance = GetCombatDistance(target, spell->rangeIndex == SPELL_RANGE_IDX_COMBAT);
...
void Unit::CalculateSpellDamage(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType)
{
	...
	auto dmgClass = spellInfo->DmgClass;
    ...
    if (spellInfo->Id == 13279)
    {
        if (crit)
        {
            ...
            int mult = 4;
            while (roll_chance_i(40) && mult <= 16)
                mult++;
            damage = (damage * mult) / 2;
        }
        damageInfo->damage = damage;
        return;
    }

    // Hammer of Wrath & Judgement of Command
    if ((spellInfo->SpellIconID == 42 && spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN)
        || (spellInfo->SpellIconID == 561 && spellInfo->Effect[0] == 2))
        dmgClass = SPELL_DAMAGE_CLASS_MAGIC;

    // Seal of Command  (Weapon +  Spelldmg-Scale in SpellEffect)
    if (spellInfo->SpellIconID == 561 && spellInfo->Effect[0] == 31)
        dmgClass = SPELL_DAMAGE_CLASS_SPECIAL;

	// Arcane Shot
    if (spellInfo->SpellIconID == 218 && spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER)
    {
        // Apply SpellDamage Bonus
        ...
        float DonePercent = 1.f;
        Item*  pWeapon = GetTypeId() == TYPEID_PLAYER ? ((Player*)this)->GetWeaponForAttack(RANGED_ATTACK, true, false) : NULL;
        Unit::AuraList const& mModDamagePercentDone = GetAurasByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
        for (Unit::AuraList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        {
            if ((*i)->GetModifier()->m_miscvalue & GetSpellSchoolMask(spellInfo) &&  // schoolmask has to fit with the intrinsic spell school
                (*i)->GetModifier()->m_miscvalue & SPELL_SCHOOL_MASK_NORMAL &&             // AND schoolmask has to fit with weapon damage school (essential for non-physical spells)
	...
	case SPELL_DAMAGE_CLASS_MAGIC:
	{
		...
		break;
    case SPELL_DAMAGE_CLASS_SPECIAL:
    {
        // If crit add critical bonus
        if (crit)
        {
...
void Unit::CalculateMeleeDamage(Unit *pVictim, uint32 damage, CalcDamageInfo *damageInfo, WeaponAttackType attackType)
{
	...
	for (uint32 i = 0; i < MAX_SPELL_SCHOOL; ++i)
	{
		if (damageInfo->target->IsImmunedToDamage(SpellSchoolMask(1 << i)))
			continue;

		auto min = m_weaponElementalDamage[i][attackType][MINDAMAGE];
		auto max = m_weaponElementalDamage[i][attackType][MAXDAMAGE];

		auto dmg = frand(min, max);
		
		uint32 absorb = 0, resist = 0;

		damageInfo->target->CalculateDamageAbsorbAndResist(this, static_cast<SpellSchoolMask>(1 << i), DIRECT_DAMAGE, dmg, &absorb, &resist, true, true);
	...
	case MELEE_HIT_CRIT:
	{
						   ...
						   if (mod != 0)
						   {
							   ...
							   eleDamage = uint32(eleDamage * float((100.f + mod) / 100.f));
						   }
						   break;
	}
	...
	if (int32(damageInfo->damage) > 0)
	{
		...
		damageInfo->target->CalculateDamageAbsorbAndResist(this, damageInfo->damageSchoolMask, DIRECT_DAMAGE, damageInfo->damage, &damageInfo->absorb, &damageInfo->resist, true, true);
	...
	else // Inpossible to get negative result but....
		damageInfo->damage = 0;

	damageInfo->damage += eleDamage;
	damageInfo->cleanDamage += eleDamage;
}
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
		...
		GetTypeId() != TYPEID_PLAYER && !((Creature*)this)->GetCharmerOrOwnerGuid() && !pVictim->HasInArc(M_PI_F, this))
	{
		...
		else
		{
			...
			Probability -= (VictimDefense - AttackerMeleeSkill) / 5.f;
		}
...
void Unit::CalculateDamageAbsorbAndResist(Unit *pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool /*canReflect*/, bool canResist)
{
	if (!pCaster || !isAlive() || !damage)
		return;

	// Magic damage, check for resists
	if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0 && canResist)
	{
		// Get base victim resistance for school
		float tmpvalue2 = (float)GetResistance(GetFirstSchoolInMask(schoolMask));
		// Ignore resistance by self SPELL_AURA_MOD_TARGET_RESISTANCE aura
		...
		tmpvalue2 *= 0.15f / (float)pCaster->getLevel();
		if (tmpvalue2 < 0.0f)
            tmpvalue2 = 0.0f;
        // level-based resistance for npcs
        else if (GetTypeId() == TYPEID_UNIT && (!GetOwnerGuid() || !GetOwnerGuid().IsPlayer())
            && getLevel() > pCaster->getLevel() && GetFirstSchoolInMask(schoolMask) != SPELL_SCHOOL_HOLY) 
            tmpvalue2 += (getLevel() - pCaster->getLevel()) * 0.02f;
		if (tmpvalue2 > 0.75f)
			tmpvalue2 = 0.75f;
		uint32 ran = urand(0, 100);
		float faq[4] = { 24.0f, 6.0f, 4.0f, 6.0f };
...
void Unit::CalculateAbsorbResistBlock(Unit *pCaster, SpellNonMeleeDamage *damageInfo, SpellEntry const* spellProto, WeaponAttackType attType)
{
    ...
    SpellSchoolMask schoolMask = GetSpellSchoolMask(spellProto);
    if (spellProto->Id == 5019)
        schoolMask = SpellSchoolMask(1 << damageInfo->school);

    CalculateDamageAbsorbAndResist(pCaster, schoolMask, SPELL_DIRECT_DAMAGE, damageInfo->damage, &damageInfo->absorb, &damageInfo->resist, !spellProto->HasAttribute(SPELL_ATTR_EX2_CANT_REFLECTED), !spellProto->HasAttribute(SPELL_ATTR_EX4_BINARY));
	damageInfo->damage -= damageInfo->absorb + damageInfo->resist;
}
...
void Unit::AttackerStateUpdate(Unit *pVictim, WeaponAttackType attType, bool extra)
{
	...
	if (IsNonMeleeSpellCasted(false) && !extra)
		return;
    ...
    if (!damageInfo.damage && GetTypeId() == TYPEID_PLAYER && pVictim->IsPvP())
    {
        SetInCombatState(true, pVictim);
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
            pVictim->SetInCombatState(true, this);
    }
...
SpellMissInfo Unit::SpellHitResult(Unit *pVictim, SpellEntry const *spell, bool CanReflect)
{
    ...
    // Hack for Rouge T2
    if (spell->Id == 23580 && pVictim == this)
        return SPELL_MISS_NONE;
...
uint32 Unit::GetDefenseSkillValue(Unit const* target) const
{
	if (GetTypeId() == TYPEID_PLAYER)
	{
		...
		uint32 value = (target && target->GetTypeId() == TYPEID_PLAYER)
            ? std::max(((Player*)this)->GetMaxSkillValue(SKILL_DEFENSE), 
...
void Unit::_UpdateAutoRepeatSpell()
{
	...
	{
        if (GetTypeId() == TYPEID_PLAYER)
            SetTargetGuid(((Player*)this)->GetSelectionGuid());
		...
		// we want to shoot
		auto spell = std::make_shared<Spell>(this, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo, true);
		SpellCastTargets targets;
		targets.setUnitTarget(ObjectAccessor::GetUnit(*this, GetTargetGuid()));

        Unit* pTarget = ObjectAccessor::GetUnit(*this, GetTargetGuid());
        ...
        if (result == SPELL_CAST_OK &&  this->IsFriendlyTo(pTarget))
            result = SPELL_FAILED_TARGET_FRIENDLY;

        if (result == SPELL_CAST_OK && !this->IsWithinLOSInMap(pTarget))
    ...
    {
        if (!m_AutoRepeatPrepareAnimation && ((GetAttackTime(RANGED_ATTACK) * m_modAttackSpeedPct[RANGED_ATTACK]) - m_attackTimer[RANGED_ATTACK]) > 500)
        {
            auto spell = std::make_shared<Spell>(this, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo, true);
            m_AutoRepeatPrepareAnimation = true;
            spell->SendSpellStart();
        }
    }
}

void Unit::SetCurrentCastedSpell(std::shared_ptr<Spell> pSpell, bool triggered)
{
...
void Unit::InterruptSpell(CurrentSpellTypes spellType, bool withDelayed, bool forced)
{
	MANGOS_ASSERT(spellType < CURRENT_MAX_SPELL);

    if (m_currentSpells[spellType] && (withDelayed || m_currentSpells[spellType]->getState() != SPELL_STATE_DELAYED))
	{
		if (!forced && m_currentSpells[spellType]->GetCastedTime() == 0)
			return;
...
{
	std::shared_ptr<Spell> spell = m_currentSpells[spellType];
	if (!spell)
		return;
...
void Unit::InterruptNonMeleeSpells(bool withDelayed, uint32 spell_id)
{
	...
	if (m_currentSpells[CURRENT_GENERIC_SPELL] && (!spell_id || m_currentSpells[CURRENT_GENERIC_SPELL]->m_spellInfo->Id == spell_id))
		InterruptSpell(CURRENT_GENERIC_SPELL, withDelayed, false);
...
std::shared_ptr<Spell> Unit::FindCurrentSpellBySpellId(uint32 spell_id) const
{
	for (uint32 i = 0; i < CURRENT_MAX_SPELL; ++i)
...
{
	if (hasUnitState(UNIT_STAT_CAN_NOT_TURN))
		return;
...
bool Unit::DebuffLimitCheck(SpellAuraHolder *holder)
{
	...
	{
		if (GetUInt32Value(static_cast<uint16>(UNIT_FIELD_AURA + i)) == 0)
			return true;
	}

	auto priority = holder->GetSpellProto()->spellPriority;

    std::vector<SpellAuraHolder*> lowestPriority;
    auto current_priority = priority;

    for (auto& aura : m_spellAuraHolders)
    {
        auto* spellInfo = aura.second->GetSpellProto();

        if (!spellInfo || aura.second->IsPositive())
            continue;

        if (spellInfo->spellPriority == current_priority)
            lowestPriority.push_back(aura.second);
        else if (spellInfo->spellPriority > current_priority)
        {
            current_priority = spellInfo->spellPriority;
            lowestPriority.clear();
            lowestPriority.push_back(aura.second);
        }
    }

	SpellAuraHolder* shortestRunningAura = nullptr;
	int32 duration = std::numeric_limits<int32>().max();

    for (auto& aura : lowestPriority)
	{
        if (duration > aura->GetAuraDuration())
		{
		    shortestRunningAura = aura;
			duration = aura->GetAuraDuration();
        }
	}
	
	if (shortestRunningAura)
		RemoveSpellAuraHolder(shortestRunningAura);
	else
        return false;
...
bool Unit::AddSpellAuraHolder(SpellAuraHolder *holder)
{
    ...
    if (!DebuffLimitCheck(holder) || !holder->_AddSpellAuraHolder())
    {
        delete holder;
        return false;
    }
...
bool Unit::CanStackAura(SpellAuraHolder *holder)
{
	...
	if ((!holder->IsPassive() && !holder->IsPersistent()) || holder->IsAreaAura())
	{
		...
		for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
		{
			...
			if (foundHolder->GetCasterGuid() == holder->GetCasterGuid() ||
                ...
                aurSpellInfo->Id == 12654 ||                                    // Ignite
				(aurSpellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR &&
				aurSpellInfo->SpellFamilyFlags & UI64LIT(0x00000004000)))       // Sunder Armor
			{
                if (aurSpellInfo->Id == 21925 || aurSpellInfo->Id == 21927     // Nature's Ally (Hunter T1/T2 5p Bonus)
                    || aurSpellInfo->Id == 21740 || aurSpellInfo->Id == 21921) // Demonic Ally (Warlock T1/T2 5p Bonus)
                {
                    foundHolder->RefreshHolder();
                    return false;
                }

                // special behavior for Ignite
                if (aurSpellInfo->Id == 12654)
                {
                    int stacks = foundHolder->m_auras[0]->GetStackAmount();
                    if (stacks >= 5)
                    {
                        foundHolder->RefreshHolder();
                        return false;
                    }
                    int damage = foundHolder->m_auras[0]->GetModifier()->m_amount + holder->m_auras[0]->GetBasePoints();
                    foundHolder->ModStackAmount(holder->GetStackAmount());
                    foundHolder->m_auras[0]->SetModifier(AuraType::SPELL_AURA_PERIODIC_DAMAGE, damage, 2000, 0);
                    return false;
                }
            ...
            if (holder->GetSpellProto()->AttributesEx3 & SPELL_ATTR_EX3_UNK7)
            {
                if (holder->GetCasterGuid() != foundHolder->GetCasterGuid())
                    continue;
            }
	...
	{
        if (IsMorePowerfulSpellActive(holder->GetSpellProto()->Id, holder->GetCaster()) || !RemoveNoStackAurasDueToAuraHolder(holder))
		{
	...
	if (holder->IsSingleTarget())
	{
		...
		{
            if (caster->GetTypeId() == TYPEID_PLAYER && ((Player*)caster)->isGameMaster())
                return true;
            ...
            if (sSpellMgr.IsRankSpellDueToSpell((*i)->GetSpellProto(), spellInfo->Id) || (*i)->GetSpellProto()->Id == spellInfo->Id)
            {
                ...
                else
                {
                    ...
                    if (Player* modOwner = caster->GetSpellModOwner())
                    {
                        ...
                        {
                        case EFFECT_INDEX_0:
                            modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_EFFECT1, freshSpell);
                            break;
                        case EFFECT_INDEX_1:
                            modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_EFFECT2, freshSpell);
                            break;
                        case EFFECT_INDEX_2:
                            modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_EFFECT3, freshSpell);
                            break;
                        }
                    }
                    if (caster)
...
bool Unit::RemoveNoStackAurasDueToAuraHolder(SpellAuraHolder *holder)
{
	...
	for (i = m_spellAuraHolders.begin(); i != m_spellAuraHolders.end(); i = next)
	{
		...
		if (is_spellSpecPerTarget || (is_spellSpecPerTargetPerCaster && holder->GetCasterGuid() == (*i).second->GetCasterGuid()))
		{
			...
			if (!IsPositiveSpell(spellProto) && sSpellMgr.IsRankSpellDueToSpell(spellProto, i_spellId))
...
void Unit::RemoveAura(uint32 spellId, SpellEffectIndex effindex, Aura* except)
{
	...
	for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second;)
	{
		...
		if (aur && aur != except)
		{
            if (IsChanneledSpell(aur->GetSpellProto()) && aur->GetSpellProto()->Effect[effindex] != SPELL_EFFECT_PERSISTENT_AREA_AURA && aur->GetCaster())
				aur->GetCaster()->CastStop();
...
{
    ObjectGuid targetGuid = castItem ? castItem->GetObjectGuid() : ObjectGuid();

	SpellAuraHolderBounds bounds = GetSpellAuraHolderBounds(spellId);
	for (SpellAuraHolderMap::iterator iter = bounds.first; iter != bounds.second;)
	{
        if (iter->second->GetCastItemGuid() == targetGuid)
		{
			RemoveSpellAuraHolder(iter->second);
			bounds = GetSpellAuraHolderBounds(spellId);
			iter = bounds.first;
...
void Unit::RemoveSpellAuraHolder(SpellAuraHolder *holder, AuraRemoveMode mode)
{
	...
	/*
	if(AurSpellInfo->Id == 19695)
	BASIC_LOG("GD RemoveSpellAuraHolder - %u", mode);
	*/
...
{
	auto guid = pAttacker->GetGUID();
	AttackerSet::const_iterator itr = m_attackers.find(guid);
...
{
	auto guid = pAttacker->GetGUID();
	auto itr = m_attackers.find(guid);
	if (itr == m_attackers.end())
		return;
...
Unit* Unit::getAttackerForHelper()
{
	...
	if (!m_attackers.empty())
		return GetMap()->GetUnit(*m_attackers.begin());
...
bool Unit::Attack(Unit *victim, bool meleeAttack)
{
	...
	if (victim->GetTypeId() == TYPEID_PLAYER && !CanAttackPlayer())
		return false;
	...
	// Units can not attack when polymorphed or feared :)))))
	if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED) || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED) || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FLEEING))
		return false;
	...
	m_attacking = victim->GetGUID();
	if(auto* map = GetMap())
	{
		if(auto* unit = map->GetUnit(m_attacking))
			unit->_addAttacker(this);
	}
...
bool Unit::AttackStop(bool targetSwitch /*=false*/)
{
	...
	if (auto* map = GetMap())
	{
		if (auto* unit = map->GetUnit(m_attacking))
		{
			unit->_removeAttacker(this);
			SendMeleeAttackStop(unit);
		}
	}
...
bool Unit::AttackStop(Unit* target)
{
	auto itr = m_attackers.find(target->GetGUID());
	if (itr == m_attackers.end())
		return false;

	m_attacking = target->GetGUID();
	return Unit::AttackStop(true);
}
...
void Unit::Evade()
{
	...
	{
		if (creature->AI()->PreEnterEvadeMode() == false)
			return;
	}
	...
	creature->LoadCreatureAddon(false);
	creature->SetLootRecipient(NULL);

	if (creature->AI() != nullptr)
	{
		creature->AI()->EnterEvadeMode();
		//creature->AI()->Reset(); //not sure about this one - keeping it in mind though
	}

    if (!isActiveObject())
    {
        SetActiveObjectState(true);
        m_setInactiveOnEvade = true;
    }

    if (!i_motionMaster.empty() && i_motionMaster.GetCurrentMovementGeneratorType() != HOME_MOTION_TYPE)
    {
        EvadeStop();
        creature->RegenerateAll(REGEN_TIME_FULL);
    }
}

void Unit::EvadeStart()
{
	SetCanChase(false);
...
{
    if (GetTypeId() == TYPEID_UNIT)
        ((Creature*)this)->ResetCombatReachTimer();
    if (m_setInactiveOnEvade)
    {
        m_setInactiveOnEvade = false;
...
void Unit::RemoveAllAttackers()
{
	while (!m_attackers.empty())
	{
		...
		if (auto* attacker = GetMap()->GetUnit(*iter))
		{
		...
		{
			sLog.outError("WORLD: Unit has an attacker that doesn't excist!");
			m_attackers.erase(iter);
		}
	}
}
...
Unit* Unit::SelectMagnetTarget(Unit *victim, std::shared_ptr<Spell> spell, SpellEffectIndex eff)
{
	if (!victim)
		return NULL;
	...
	{
		if (spell->m_spellInfo->Attributes & SPELL_ATTR_UNK4 || spell->m_spellInfo->AttributesEx & SPELL_ATTR_EX_UNK3)
			return victim;
		...
		for (Unit::AuraList::const_iterator itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
		{
			if (Unit* magnet = (*itr)->GetCaster())
			{
				if (magnet->isAlive() && magnet->IsWithinLOSInMap(this) && (spell->CheckTarget(magnet, eff)
                    || GetSpellSpecific(spell->m_spellInfo->Id) == SPELL_MAGE_POLYMORPH))
				{
					if (SpellAuraHolder *holder = (*itr)->GetHolder())
                    {
                        // Remove Grounding Totem Effect
                        if (holder->GetCaster() && holder->GetCaster()->GetEntry() == 5925)
                            holder->GetCaster()->RemoveAura(8178, EFFECT_INDEX_0);
...
void Unit::SendFlyingAnimation(uint32 uiSpell)
{
	...
	{
		auto anim = std::make_shared<Spell>(this, pSpell, false);
...
void Unit::EnergizeBySpell(Unit *pVictim, uint32 SpellID, uint32 Damage, Powers powertype)
{
	...
	float threadCoeff = 0.5f;
	if (powertype == POWER_RAGE)
		threadCoeff = 5.0f;

	pVictim->getHostileRefManager().threatAssist(pVictim, Damage * threadCoeff);
...
int32 Unit::SpellBonusWithCoeffs(SpellEntry const *spellProto, int32 total, int32 benefit, int32 ap_benefit, DamageEffectType damagetype, bool donePart)
{
	...
	if (spellProto->SpellIconID == 19) //Blessing of Sanctuary does not scale with spelldmg
		coeff = 0;
...
bool Unit::IsSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType)
{
	...
	case SPELL_DAMAGE_CLASS_NONE:
	{
                                    ...
                                    if (spellProto->Id == 13279) // Gnome Death Ray
                                        return roll_chance_f(10.f);
									return false;
	}
	case SPELL_DAMAGE_CLASS_MAGIC:
	{
...
uint32 Unit::SpellCriticalDamageBonus(SpellEntry const *spellProto, uint32 damage, Unit *pVictim)
{
	...
	if (critPctDamageMod != 1.f)
        crit_bonus *= critPctDamageMod;
...
uint32 Unit::SpellHealingBonusDone(Unit *pVictim, SpellEntry const *spellProto, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
	...
	for (AuraList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
	{
		...
		case 3656: // Corrupted Healing
			if (spellProto->SpellIconID != 321)
				pVictim->CastSpell(pVictim, 23402, true);
		default:
			break;
		}
	}
...
uint32 Unit::SpellHealingBonusTaken(Unit *pCaster, SpellEntry const *spellProto, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
	...
	if (spellProto->SpellFamilyName == SPELLFAMILY_PALADIN && (spellProto->SpellFamilyFlags & UI64LIT(0x0000000080002000)))
	{
...
bool Unit::IsImmuneToSpell(uint32 spellId) {
	return IsImmuneToSpell(sSpellStore.LookupEntry(spellId));
}
...
bool Unit::IsImmuneToSpell(SpellEntry const* spellInfo)
{
	...
	if (spellInfo->Id == 13879) //Golemagg Magma Splash
		return false;
	if (spellInfo->Id == 19695) //Geddon Inferno
		return false;
	if (spellInfo->Id == 18947) //Geddon Inferno-Dummy
		return false;
...
void Unit::Mount(uint32 mount, uint32 spellId)
{
	...
	if (GetTypeId() == TYPEID_PLAYER)
	{
		...
		else
		{
			...
			if (Pet* pet = GetPet())
			{
				if (pet->IsPermanentPetFor((Player*)this) && sWorld.getConfig(CONFIG_BOOL_PET_UNSUMMON_AT_MOUNT))
				{
					((Player*)this)->UnsummonPetTemporaryIfAny();
				}
				else
...
void Unit::Unmount(bool from_aura)
{
	...
	if (GetTypeId() == TYPEID_PLAYER && GetAntiCheatImmuneTimer() == 0)
		SetAntiCheatImmuneTimer(1000);
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, bool enrage)
{
	...
	for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
	if (std::shared_ptr<Spell> spell = GetCurrentSpell(CurrentSpellTypes(i)))
...
int32 Unit::ModifyHealth(int32 dVal)
{
    ...
    if (m_resilience && dVal < 0 && curHealth != maxHealth)
        val = ((dVal * m_resilience) / 100) + curHealth;
    else
        val = dVal + curHealth;
	if (val <= 0)
	{
...
bool Unit::isVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool inVisibleList, bool is3dDistance) const
{
	...
	// PvP - Geistheiler Allianz
	if (GetTypeId() == TYPEID_UNIT && GetEntry() == 13116) {
		if (((Player*)u)->GetTeam() == ALLIANCE && !((Player*)u)->isAlive() || ((Player*)u)->isGameMaster()) {
			return true;
		}
		else {
			return false;
	...
	// PvP - Geistheiler Horde
	if (GetTypeId() == TYPEID_UNIT && GetEntry() == 13117) {
		if (((Player*)u)->GetTeam() == HORDE  && !((Player*)u)->isAlive() || ((Player*)u)->isGameMaster()) {
			return true;
		}
		else {
			return false;
	...
	//Hackfix, because fuck this shit! -.-
	if (GetTypeId() == TYPEID_UNIT && GetEntry() == 14986)
	{
		if (u->HasAura(24306) || (u->GetTypeId() == TYPEID_PLAYER && ((Player*)u)->isGameMaster()))
			return true;
		else
			return false;
	}
	else if (u->GetTypeId() == TYPEID_UNIT && u->GetEntry() == 14986)
		return true;
	...
	if (u->GetTypeId() == TYPEID_PLAYER)
	{
		...
		// Ancient Trees - Players need The Ancient Leaf to see these NPCs
		auto entry = GetEntry();
		if ((entry == 14524 || entry == 14525 || entry == 14526))
		{
			const Player* plr = static_cast<const Player*>(u);
			if (!plr->HasItemCount(18703, 1) && plr->GetQuestStatus(7632) != QuestStatus::QUEST_STATUS_COMPLETE)
				return false;
		}
	}
	else
	...
	bool isInFront = viewPoint->isInFrontInMap(this, visibleDistance, 4.712388f);
	if (!isInFront)
		return false;
    ...
    visibleDistance += (int32(u->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_STEALTH_DETECT, 0)) - stealthMod) / 5.0f;
...
{
	if (GetEntry() == 14986 && x == VISIBILITY_ON) //Hackfix, but I'm too lazy to fix this properly. Deal with it
		return;
...
void Unit::SetSpeedRate(UnitMoveType mtype, float rate, bool forced)
{
	...
	if (m_speed_rate[mtype] != rate)
	{
		...
		if (forced && GetTypeId() == TYPEID_PLAYER)
		{
			...
			WorldPacket data(SetSpeed2Opc_table[mtype][0], 12);
			data << GetPackGUID();
			data << (uint32)0;
...
bool Unit::IsSecondChoiceTarget(Unit* pTarget, bool checkThreatArea)
{
    ...
    AuraType CheckedTypes[] = { SPELL_AURA_MOD_FEAR, SPELL_AURA_MOD_CONFUSE, SPELL_AURA_MOD_STUN };

    for (auto type : CheckedTypes)
    {
        for (auto aura : pTarget->GetAurasByType(type))
        {
            if (aura->GetAuraMaxDuration() >= 10000)
                return true;
        }
    }
	...
	return
        ...
        pTarget->HasAuraType(SPELL_AURA_MOD_UNATTACKABLE) ||
		checkThreatArea && ((Creature*)this)->IsOutOfThreatArea(pTarget) ||
        (pTarget->GetCharmer() && IsFriendlyTo(pTarget->GetCharmer()));
}
...
bool Unit::SelectHostileTarget()
{
    ...
    if (oldTarget && m_chargeTarget && oldTarget->isAlive())
    {
        if (movespline->Finalized())
            m_chargeTarget = false;
        else
            target = oldTarget;
    }
	...
	if (target)
	{
			...
			!HasAuraType(SPELL_AURA_MOD_FEAR) && !HasAuraType(SPELL_AURA_MOD_CONFUSE))
		{
            ...
            if (!GetMotionMaster()->operator->()->IsReachable() && ((Creature*)this)->GetCombatReachTimerExpired() && (target->GetEntry() != 12422))
			{
				// remove all taunts
	...
	{
		for(auto& guid : m_attackers)
		{
			if(auto* unit = GetMap()->GetUnit(guid))
			{
				if (unit->IsInMap(this) && unit->isTargetableForAttack() && unit->isInAccessablePlaceFor((Creature*)this))
					return false;
			}
		}
	}
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellEntry const* spellProto, SpellEffectIndex effect_index, int32 const* effBasePoints)
{
	...
	if (spellProto->HasAttribute(SPELL_ATTR_LEVEL_DAMAGE_CALCULATION) && spellProto->spellLevel &&
		...
		value = int32(value*0.25f*exp(getLevel()*(60 - spellProto->spellLevel) / 1000.0f));
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
	for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
	{
		...
		if (i->stack == 0 && WorldTimer::getMSTimeDiff(i->hitTime, WorldTimer::getMSTime()) > 31 * IN_MILLISECONDS)
		{
...
{
    if (caster->GetTypeId() == TYPEID_UNIT && (!caster->GetOwner() || caster->GetOwner()->GetTypeId() == TYPEID_UNIT))
		return;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit * pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const * procSpell, uint32 damage)
{
	...
	for (ProcTriggeredList::const_iterator itr = procTriggered.begin(); itr != procTriggered.end(); ++itr)
	{
		...
		{
                // Nature's Grace
            if ((triggeredByHolder->GetSpellProto()->Id == 16886) ||
                // Hackfix for flurry (shaman, warrior)
                (triggeredByHolder->GetSpellProto()->SpellIconID == 108 && triggeredByHolder->GetSpellProto()->Effect[0] == 6 && 
                (m_extraAttacks || (procFlag & MELEE_BASED_TRIGGER_MASK && procExtra & PROC_EX_CRITICAL_HIT))))
            {
	...
	if (!removedSpells.empty())
	{
        ...
        {
            // Hack for clearcast (druid)
            if ((*i) == 16870 && !procSpell->manaCost)
                continue;
...
void Unit::SetFeared(bool apply, ObjectGuid casterGuid, uint32 spellID, uint32 time)
{
	if (apply)
	{
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            MonsterMoveWithSpeed(GetPositionX(), GetPositionY(), GetPositionZ(), GetSpeed(MOVE_RUN));
            StopMoving(true);
        }
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        if (apply)
            ...
            ((Player*)this)->TryResetClientControl();
}
...
void Unit::SetFeignDeath(bool apply, ObjectGuid casterGuid, uint32 spellID)
{
	if (apply)
	{
		...
		*/
		...
		for (auto& player : GetMap()->GetPlayers())
		{
			auto* plr = player.getSource();
			if (plr && plr->GetGUID() != GetGUID())
			{
				for (uint32 i = 0; i < 3; ++i)
				{
					auto spell = plr->GetCurrentSpell(static_cast<CurrentSpellTypes>(i));
					if (spell)
					{
						auto* target = spell->m_targets.getUnitTarget();
		...
		if (!hrm.isEmpty())
		{
			...
			while (ref != NULL)
			{
				...
				for (auto& target : creature->GetSingleCastSpellTargets())
				{
					Player* plr = sObjectMgr.GetPlayer(target.second);
					if (plr)
						plr->CastStop();
				}
...
void Unit::UpdateModelData()
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
        defaultScale = ((Creature*)this)->GetCreatureInfo()->scale;

	if (CreatureModelInfo const* modelInfo = sObjectMgr.GetCreatureModelInfo(GetDisplayId()))
	{
		// we expect values in database to be relative to scale = 1.0
        SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, GetObjectScale() / defaultScale * modelInfo->bounding_radius);

		// never actually update combat_reach for player, it's always the same. Below player case is for initialization
		if (GetTypeId() == TYPEID_PLAYER)
			SetFloatValue(UNIT_FIELD_COMBATREACH, 1.5f);
		else
            SetFloatValue(UNIT_FIELD_COMBATREACH, GetObjectScale() / defaultScale * modelInfo->combat_reach);
	}
}
...
void Unit::MonsterMoveWithSpeed(float x, float y, float z, float speed, bool generatePath, bool forceDestination, uint32 maxTravelTime)
{
	Movement::MoveSplineInit init(*this);
	init.MoveTo(x, y, z, generatePath, forceDestination);
	init.SetVelocity(speed);
	init.Launch(maxTravelTime);
}
...
class StopAttackGUIDUnary
{
private:
	const ObjectGuid& mGuid;
public:
	StopAttackGUIDUnary(const ObjectGuid& guid) : mGuid(guid) {

	}

	void operator () (Unit* unit) const {
		unit->StopAttackObject(mGuid);
	}
};

void Unit::StopAttackObject(const ObjectGuid& guid) {
	if (Unit* victim = getVictim())
	{
		if (victim->GetObjectGuid() == guid)
		{
			AttackStop();
			if (IsNonMeleeSpellCasted(false))
				InterruptNonMeleeSpells(false);
	...
	for (AttackerSet::const_iterator itr = attackers.begin(); itr != attackers.end();)
	{
        if ((*itr) == guid)
        {
            if (auto* unit = GetMap()->GetUnit(*itr))
		    {
			    unit->AttackStop();
			    itr = attackers.begin();
		    }
	...
	getHostileRefManager().deleteReferencesForGUID(guid);
	debug_log("HOSTILE_MANAGER: %u", getHostileRefManager().getSize());

	CallForAllControlledUnits(StopAttackGUIDUnary(guid), CONTROLLED_PET | CONTROLLED_GUARDIANS | CONTROLLED_CHARM);
}
...
void Unit::StopAttackFaction(uint32 faction_id)
{
	...
	for (AttackerSet::const_iterator itr = attackers.begin(); itr != attackers.end();)
	{
		if (auto* unit = GetMap()->GetUnit(*itr))
		{
			if (unit->getFactionTemplateEntry()->faction == faction_id)
			{
				unit->AttackStop();
				itr = attackers.begin();
			}
...
void Unit::UpdateSplineMovement(uint32 t_diff)
{
	...
	{
		m_movesplineTimer.Reset(POSITION_UPDATE_DELAY / GetSpeedRate(MOVE_RUN));
		Movement::Location loc = movespline->ComputePosition();
...
uint32 Unit::GetWorldState(uint32 id)
{
    return sWorld.GetWorldState(id);
}