...
namespace Movement
{
    ...
    class MANGOS_DLL_SPEC MoveSplineInit
    {
        public:
             ...
             */
            int32 Launch(int32 maxTravelTime = 0, bool hideMovement = false);


			/* Stop any creature movement */
            void Stop(bool hideMovement = false);
             ...
             */
			void MoveTo(const Vector3& destination, bool generatePath = false, bool forceDestination = false, float maxPathRange = 0);
			void MoveTo(float x, float y, float z, bool generatePath = false, bool forceDestination = false, float maxPathRange = 0);
	...
	{
		if (unit.hasUnitState(UNIT_STAT_CAN_NOT_TURN))
			return;
