...
{
	pl->m_Events.KillAllEvents(true);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    if(xy_cell != cur_cell)
    {
		DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "Creature (GUIDLow: %u) X: %f Y: %f (%s) in grid[%u,%u]cell[%u,%u] instead grid[%u,%u]cell[%u,%u]. Charmer %s",
            c->GetGUIDLow(),
            c->GetPositionX(),c->GetPositionY(),(moved ? "final" : "original"),
            cur_cell.GridX(), cur_cell.GridY(), cur_cell.CellX(), cur_cell.CellY(),
            xy_cell.GridX(),  xy_cell.GridY(),  xy_cell.CellX(),  xy_cell.CellY(),
			c->GetCharmer() ? c->GetCharmer()->GetName() : "");
...
bool DungeonMap::Add(Player *player)
{
    ...
    else
    {
        ...
        if (pGroup)
        {
            ...
            InstanceGroupBind *groupBind = pGroup->GetBoundInstance(GetId(), player);
            if (playerBind)
            {
        ...
        switch(step.script->command)
        {
			...
			case SCRIPT_COMMAND_MOVEMENT:                   // 20
			{
				...
				case IDLE_MOTION_TYPE:
					pMover->GetMotionMaster()->MoveRandomAroundPoint(pSource->GetPositionX(), pSource->GetPositionY(), pSource->GetPositionZ(), float(step.script->movement.searchRadius));
					break;
				case RANDOM_MOTION_TYPE:
					float respX, respY, respZ, respO, searchRadius;
					((Creature*)pSource)->GetRespawnCoord(respX, respY, respZ, &respO, &searchRadius);
					searchRadius = step.script->movement.searchRadius ? step.script->movement.searchRadius : searchRadius;
					((Creature*)pSource)->GetMotionMaster()->MoveRandomAroundPoint(respX, respY, respZ, searchRadius);
					break;
            ...
            case SCRIPT_COMMAND_SET_RUN:
            {
				...
				pOwner->SetWalk(!step.script->run.run, true);
...
bool Map::GetHitPosition(float srcX, float srcY, float srcZ, float& destX, float& destY, float& destZ, float modifyDist) const
{
    ...
    if (m_TerrainData)
    {
        for (; lastvalid < 10; lastvalid++)
        {
            float pos = 0.1f * (lastvalid + 1);
            float checkX = destX * pos + srcX * (1.f - pos);
            float checkY = destY * pos + srcY * (1.f - pos);
            float checkZ = destZ * pos + srcZ * (1.f - pos);
            if (m_TerrainData->GetMapZDiff(checkX, checkY, checkZ) > zdifflimit)
                break;
        }

        float pos = 0.1f * lastvalid;
        destX = destX * pos + srcX * (1.f - pos);
        destY = destY * pos + srcY * (1.f - pos);
        destZ = destZ * pos + srcZ * (1.f - pos);
    }
    else
        lastvalid = 10;

    return result0 || result1 || lastvalid != 10;
}
