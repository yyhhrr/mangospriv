...
void DungeonPersistentState::DeleteRespawnTimes()
{
	...
	WorldDatabase.PExecute("DELETE FROM gameobject_instance WHERE instanceID = '%u'", GetInstanceId());
    CharacterDatabase.CommitTransaction();
...
void MapPersistentStateManager::DeleteInstanceFromDB(uint32 instanceid)
{
    if (instanceid)
    {
		...
		CharacterDatabase.PExecute("DELETE FROM gameobject_instance WHERE instanceID = '%u'", instanceid);
        CharacterDatabase.CommitTransaction();
    }
}
...
void MapPersistentStateManager::CleanupInstances()
{
	...
	CharacterDatabase.PExecute("DELETE FROM gameobject_instance WHERE instanceID <> 0 AND instanceID NOT IN (SELECT id FROM instance)");
...
void MapPersistentStateManager::PackInstances()
{
    ...
    for (std::set<uint32>::iterator i = InstanceSet.begin(); i != InstanceSet.end(); ++i)
    {
        if (*i != InstanceNumber)
        {
			...
			CharacterDatabase.PExecute("UPDATE gameobject_instance SET instanceID = '%u' WHERE instanceID = '%u'", InstanceNumber, *i);
