...
Unit(), m_aggroRadius(1.f), i_AI(NULL),
lootForPickPocketed(false), lootForBody(false), lootGroupCheck(false), lootForSkin(false), m_groupLootTimer(0), m_groupLootId(0),
...
m_creatureInfo(NULL), m_formation(NULL), m_combatReachableTimer(5000)
{
    m_regenTimer = 200;
...
bool Creature::InitEntry(uint32 Entry, Team team, CreatureData const* data /*=NULL*/, GameEventCreatureData const* eventData /*=NULL*/ )
{
    ...
    m_resilience = sObjectMgr.GetCreatureTemplateResilience(Entry);

    return true;
}

void Creature::SetBaseResistance(UnitMods type, int32 baseValue)
{
    MANGOS_ASSERT(type >= UNIT_MOD_RESISTANCE_HOLY && type <= UNIT_MOD_RESISTANCE_ARCANE);

    if (baseValue || getLevel() < 40)
    {
        SetModifierValue(type, BASE_VALUE, float(baseValue));
        return;
    }
    ...
    switch (GetCreatureInfo()->type)
    {
        ...
        case CREATURE_TYPE_DEMON:
            switch (type)
            {
                case UNIT_MOD_RESISTANCE_HOLY: rate -= 0.5f; break;
                case UNIT_MOD_RESISTANCE_FIRE:
                case UNIT_MOD_RESISTANCE_SHADOW: rate += 0.1f; break;
                case UNIT_MOD_RESISTANCE_NATURE: rate -= 0.25f; break;
                default: rate -= 0.05f; break;
            }
        case CREATURE_TYPE_UNDEAD:
            switch (type)
            {
                case UNIT_MOD_RESISTANCE_HOLY: rate -= 0.5f; break;
                case UNIT_MOD_RESISTANCE_SHADOW: rate += 0.2f; break;
                case UNIT_MOD_RESISTANCE_NATURE: rate -= 0.25f; break;
                case UNIT_MOD_RESISTANCE_FROST: break;
                default: rate -= 0.1f; break;
            }
            break;
    ...
    switch (GetCreatureInfo()->unit_class)
    {
        case CLASS_MAGE:
    ...
    switch (GetCreatureInfo()->rank)
    {
        ...
        case CREATURE_ELITE_RARE:
            if (type == UNIT_MOD_RESISTANCE_ARCANE)
                if (GetCreatureInfo()->type != CREATURE_TYPE_ELEMENTAL)
                    rate -= 0.2f;
                else
                    rate += 0.2f;
            if (type == UNIT_MOD_RESISTANCE_SHADOW && GetCreatureInfo()->type == CREATURE_TYPE_BEAST)
                rate -= 0.2f;
            level -= 17;
            if (level > 43)
                level = 43;
        ...
        case CREATURE_ELITE_WORLDBOSS:
            if (type != UNIT_MOD_RESISTANCE_NATURE && type != UNIT_MOD_RESISTANCE_HOLY)
                basePoints += 55.f;
            break;
    }

    int rollrange = GetCreatureInfo()->maxlevel - GetCreatureInfo()->minlevel;

    float result = basePoints + (level - 23) / 2.f * rate;

    if (rollrange)
        result += urand(0, rollrange);
    
    if (result < 0.f)
        result = 0.f;
   
    if (GetCreatureInfo()->rank != CREATURE_ELITE_WORLDBOSS)
        m_baseResistance[type - UNIT_MOD_RESISTANCE_HOLY] = true;
    SetModifierValue(type, BASE_VALUE, result); 
}
...
bool Creature::UpdateEntry(uint32 Entry, Team team, const CreatureData *data /*=NULL*/, GameEventCreatureData const* eventData /*=NULL*/, bool preserveHPAndPower /*=true*/)
{
    ...
    SetBaseResistance(UNIT_MOD_RESISTANCE_HOLY, (GetCreatureInfo()->resistance1));
    SetBaseResistance(UNIT_MOD_RESISTANCE_FIRE, (GetCreatureInfo()->resistance2));
    SetBaseResistance(UNIT_MOD_RESISTANCE_NATURE, (GetCreatureInfo()->resistance3));
    SetBaseResistance(UNIT_MOD_RESISTANCE_FROST, (GetCreatureInfo()->resistance4));
    SetBaseResistance(UNIT_MOD_RESISTANCE_SHADOW, (GetCreatureInfo()->resistance5));
    SetBaseResistance(UNIT_MOD_RESISTANCE_ARCANE, (GetCreatureInfo()->resistance6));
...
void Creature::Update(uint32 update_diff, uint32 diff)
{
    switch( m_deathState )
    {
        ...
        case DEAD:
        {
			if(AI()) {
				m_AI_locked = true;
				AI()->UpdateAIDead(diff);
				m_AI_locked = false;
			}
            if (m_respawnTime <= time(NULL) && !IsTemporarySummon() && (!m_isSpawningLinked || GetMap()->GetCreatureLinkingHolder()->CanSpawn(this)))
            {
        ...
        case ALIVE:
        {
            ...
            if (m_decreaseCombatReachTimer)
            {
                float x, y, z;
                if (!this->GetMotionMaster()->GetDestination(x, y, z))
                {
                    if (m_combatReachableTimer <= diff)
                        m_combatReachableTimer = uint32(0);
                    else
                        m_combatReachableTimer -= diff;
                }
            }
            else
                m_combatReachableTimer = std::min(m_combatReachableTimer + diff, uint32(20000));

            m_decreaseCombatReachTimer = false;

            if (this->hasUnitState(UNIT_STAT_FLEEING | UNIT_STAT_FLEEING_MOVE) && !IsPet() && update_diff >= m_regenTimer)
                CallForHelp(5.0f);
...
void Creature::ResetCombatReachTimer()
{
    m_combatReachableTimer = 20000;
}
...
void Creature::StopGroupLoot()
{
    ...
    // Update Lootable-Flag for Clients
    ToggleFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE);
...
bool Creature::AIM_Initialize()
{
	...
	if (i_AI == oldAI)
	{
		sLog.outString("Das sollte nicht passieren!");
		char c = *((char*)0);
	}
...
void Creature::PrepareBodyLootState()
{
    ...
    if (!isAlive())
    {
        ...
        if (!lootForBody)
        {
            Group* grp = GetGroupLootRecipient();
            if (recipient != NULL)
            { 
                lootForBody = true;

                if (grp)
                    grp->UpdateLooterGuid(this, true);
                ...
                if (grp)
                {
                    if (grp->GetLootMethod() == LootMethod::MASTER_LOOT)
                    {
                        grp->MasterLoot(this, &loot);
                        lootGroupCheck = true;
                    }
                    else
                    {
                        grp->MarkUnderThreshold(&loot);
                        loot.GroupLooterGuid = grp->GetLooterGuid();
                    }
                    grp->UpdateLooterGuid(this);
                }
...
bool Creature::HasLootFor(Player* plr)
{
    if (!lootForBody)
        return false;
    ...
    if (recipient == plr || plr->GetGroup() == grp)
    {
        PermissionTypes permission;

        if (grp)
        {
            if (grp->GetLootMethod() == FREE_FOR_ALL || grp->GetLootMethod() == MASTER_LOOT || plr->GetObjectGuid() == loot.GroupLooterGuid)
        ...
        WorldPacket check(SMSG_LOOT_RESPONSE, (50));
        check << LootView(loot, plr, permission);
        uint32 gold; 
        uint8 itemCnt;
        check >> gold;
        check >> itemCnt;

        return ((gold && permission != GROUP_PERMISSION) || itemCnt);
    }
...
void Creature::SetLootRecipient(Unit *unit)
{
    ...
    if (!player || (unit->GetTypeId() == TYPEID_UNIT && ((Creature*)unit)->IsPet()))    // normal creature, no player involved
        return;
...
float Creature::GetAttackDistance(Unit const* pl) const
{
    ...
    return (RetDistance*aggroRate*m_aggroRadius);
}
...
{
	if (GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_FLAG_NO_CALL_ASSIST)
		return;
...
bool Creature::IsOutOfThreatArea(Unit* pVictim) const
{
	...
	auto dist = ThreatRadius > AttackDist ? ThreatRadius : AttackDist;

	bool isInDist = IsWithinDist3d(mLastAttackPosition.x, mLastAttackPosition.y, mLastAttackPosition.z, dist) ||
					IsWithinDist3d(m_combatStartX, m_combatStartY, m_combatStartZ, dist);
...
bool Creature::MeetsSelectAttackingRequirement(Unit* pTarget, SpellEntry const* pSpellInfo, uint32 selectFlags) const
{
	...
	if(selectFlags & SELECT_FLAG_NOT_TANK && this->getVictim() == pTarget && pTarget->getClass() == CLASS_WARRIOR)
		return false;
    ...
    if (pSpellInfo)
    {
        switch (pSpellInfo->rangeIndex)
        {
            ...
            //case SPELL_RANGE_IDX_ANYWHERE: return true; not known for [Zero]
...
{
    return (!i_motionMaster.empty() && i_motionMaster.GetCurrentMovementGeneratorType() == HOME_MOTION_TYPE) || isEvading();
}
...
{
    if (lootForBody && loot.empty() && !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
    {
...
void Creature::SendAreaSpiritHealerQueryOpcode(Player *pl)
{
    ...
    if (std::shared_ptr<Spell> pcurSpell = GetCurrentSpell(CURRENT_CHANNELED_SPELL))
...
bool Creature::GetCombatReachTimerExpired()
{
    m_decreaseCombatReachTimer = true;
    return m_combatReachableTimer <= 0;
}