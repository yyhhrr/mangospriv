...
void CreatureLinkingHolder::SetFollowing(Creature* pWho, Creature* pWhom)
{
	...
	float pSpeedM = pWhom->GetSpeed(MOVE_WALK);
	float pSpeedS = pWho->GetSpeed(MOVE_WALK);
	...
	if (dist > 4)
	{
		float pSpeed = (pSpeedM / 2) + (pSpeedS / 4);
		pWho->SetSpeedRate(MOVE_WALK, pSpeed, true);
		pWho->GetMotionMaster()->MoveFollow(pWhom, PET_FOLLOW_DIST, angle);
	}
	else
	{
		float pSpeed = (pSpeedM / 4) + (pSpeedS / 6);
		pWho->SetSpeedRate(MOVE_WALK, pSpeed, true);
		pWho->GetMotionMaster()->MoveFollow(pWhom, PET_FOLLOW_DIST, angle);
	}
}
