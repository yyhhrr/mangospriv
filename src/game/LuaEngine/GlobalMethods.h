...
namespace LuaGlobalFunctions
{
    ...
    int GetPlayersInWorld(lua_State* L)
    {
		...
		for(auto& session : eWorld->GetAllSessions())
        {
...
#if (defined(TBC) || defined(CLASSIC))
				if (!pGameObj->Create(db_lowGUID, gInfo->id, map, x, y, z, o, 0, 0, 0, 0, 100, GO_STATE_READY))
#else
...
#if (defined(TBC) || defined(CLASSIC))
				if (!pGameObj->Create(map->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), entry, map, x, y, z, o, 0, 0, 0, 0, 100, GO_STATE_READY))
#else
...
#ifndef TRINITY
        if (!eObjectMgr->IsVendorItemValid(false, "npc_vendor", entry, item, maxcount, incrtime, nullptr))
            return 0;
#ifndef CLASSIC
