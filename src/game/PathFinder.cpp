...
bool PathFinder::calculate(float destX, float destY, float destZ, bool forceDest)
{
    ...
    if (m_type == PATHFIND_NOPATH && updateFilter())
        BuildPolyPath(start, dest);
...
void PathFinder::BuildPolyPath(const Vector3& startPos, const Vector3& endPos)
{
    ...
    if (m_polyLength)
    {
        for (pathStartIndex = 0; pathStartIndex < m_polyLength; ++pathStartIndex)
        {
			...
			{
				DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u, startPos: %s, endPos: %s, mapid: %u", m_polyLength, pathStartIndex, startPos.toString().c_str(), endPos.toString().c_str(), m_sourceUnit->GetMapId());
				break;
			}
...
NavTerrain PathFinder::getNavTerrain(float x, float y, float z)
{
	...
	GridMapLiquidStatus liquidStatus = m_sourceUnit->GetTerrain()->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &data);
