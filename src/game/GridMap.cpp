...
float TerrainInfo::GetMapZDiff(float x, float y, float z, bool checkVMap, float maxSearchDist) const
{
    const float off_x[] = { -1.f, 1.f };
    const float off_y[] = { -1.f, 1.f };
    if (!checkVMap)
    {
        if (GridMap* gmap = const_cast<TerrainInfo*>(this)->GetGrid(x, y))
        {
            float base_z = gmap->getHeight(x, y);
            float new_z = gmap->getHeight(x + off_x[0], y + off_y[0]);
            float diff = (base_z - new_z) * (base_z - new_z);
            new_z = gmap->getHeight(x + off_x[0], y + off_y[1]);
            diff += (base_z - new_z) * (base_z - new_z);
            new_z = gmap->getHeight(x + off_x[1], y + off_y[0]);
            diff += (base_z - new_z) * (base_z - new_z);
            new_z = gmap->getHeight(x + off_x[1], y + off_y[1]);
            diff += (base_z - new_z) * (base_z - new_z);
            return diff;
        }
    ...
    {
        float base_z = GetHeightStatic(x, y, z, true, maxSearchDist);
        float new_z = GetHeightStatic(x + off_x[0], y + off_y[0], z, true, maxSearchDist);
        float diff = (base_z - new_z) * (base_z - new_z);
        new_z = GetHeightStatic(x + off_x[1], y + off_y[0], z, true, maxSearchDist);
        diff += (base_z - new_z) * (base_z - new_z);
        new_z = GetHeightStatic(x + off_x[0], y + off_y[1], z, true, maxSearchDist);
        diff += (base_z - new_z) * (base_z - new_z);
        new_z = GetHeightStatic(x + off_x[1], y + off_y[1], z, true, maxSearchDist);
        diff += (base_z - new_z) * (base_z - new_z);
        return diff;
    }
    return VMAP_INVALID_HEIGHT_VALUE;
...
uint32 GetWmoAreaId(const TerrainInfo* info, float x, float y, float z)
{
	uint32 mogpFlags;
	int32 adtId, rootId, groupId;
	WMOAreaTableEntry const* wmoEntry = 0;
	...
	if (info->GetAreaInfo(x, y, z, mogpFlags, adtId, rootId, groupId))
	{
		wmoEntry = GetWMOAreaTableEntryByTripple(rootId, adtId, groupId);
		if (wmoEntry)
		{
...
{
	uint32 area = 0;
	if (area = GetWmoAreaId(this, x, y, z))
		return area;
