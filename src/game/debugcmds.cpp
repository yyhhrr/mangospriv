...
bool ChatHandler::HandleDebugBuffMeCommand(char*)
{
    Player *pPlayer = m_session->GetPlayer();
	pPlayer->CastSpell(pPlayer, static_cast<uint32>(10157), true);
	pPlayer->CastSpell(pPlayer, static_cast<uint32>(10938), true);
	pPlayer->CastSpell(pPlayer, static_cast<uint32>(27841), true);
	pPlayer->CastSpell(pPlayer, static_cast<uint32>(9885), true);
...
#include "Lua/Wrapper/PlayerWrapper.h"
#include <luabind\luabind.hpp>
bool ChatHandler::HandleLuaDostringCommand(char* args)
{
	try
	{
		Lua::Script script;
		script.LoadWrapper(Lua::PlayerWrapper::Wrap);

		auto unit = getSelectedUnit();

		unit = unit ? unit : m_session->GetPlayer();

		luabind::globals(script.mState)["player"] = luabind::object(script.mState, m_session->GetPlayer());
		luabind::globals(script.mState)["unit"] = luabind::object(script.mState, unit);

		script.LoadString(args);
		script.PCall(0, 0);
		//script.Test();
...
bool ChatHandler::HandleShowSpellModsCommand(char* args)
{
	...
	auto spell = sSpellStore.LookupEntry(11661);
	auto dmg = plr->SpellDamageBonusDone(plr, spell, 508, DamageEffectType::SPELL_DIRECT_DAMAGE);

	SendSysMessage(std::to_string(dmg).c_str());
