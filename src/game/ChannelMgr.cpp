...
ChannelMgr::ChannelMgr() {
	auto chnl = GetJoinChannel("world", 0);
	chnl->AddFlag(Channel::ChannelFlags::CHANNEL_FLAG_GENERAL);
	chnl->RemoveFlag(Channel::ChannelFlags::CHANNEL_FLAG_CUSTOM);
	chnl->enableLog();
}
...
void ChannelMgr::fillChannelStringForCommand(std::stringstream& strm) {
	bool first = true;
	for(auto& channel : channels) {
		...
		channel.second->writeToStreamForClient(strm);
	}
}