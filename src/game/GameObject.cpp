...
void GameObject::Update(uint32 update_diff, uint32 /*p_time*/)
{
	...
	* 181366 - Vier Reiter Truhe.
	* 179564 - Tribut Gordok.
	*
	* etc.
	*/
	if (goInfo->type == GAMEOBJECT_TYPE_CHEST && goInfo->id != 179703 && goInfo->id != 181366 && goInfo->id != 179564)
	{
		// Despawn nach 3 Minuten, wenn bereits mindestens einmal gelootet wurde.
		if (GetDecayCount() > 0) {
			if (m_despawnTime > 1800)
			{
				//error_log("%s ist despawnt.", goInfo->name);
				StopDecayTime();
				...
				m_despawnTime++;
				//error_log("Loot Zeit: %u von %s.", m_despawnTime, goInfo->name);
			}
		}
	}
    ...
    switch (m_lootState)
    {
				...
				if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
				{
					...
					else
					{
						...
						if (goInfo->faction != 0)
						{
							...
							if (owner)
							{
                                MaNGOS::AnyUnfriendlyTargetableUnitToOwnerInObjectRangeCheck u_check(this, owner, radius);
                                MaNGOS::UnitSearcher<MaNGOS::AnyUnfriendlyTargetableUnitToOwnerInObjectRangeCheck> checker(ok, u_check);
					...
					{
                        if (owner)
                            owner->CastSpell(ok, goInfo->trap.spellId, true);
                        else
                            ok->CastSpell(ok, goInfo->trap.spellId, true);
        ...
        case GO_JUST_DEACTIVATED:
		{
            ...
            if (GetOwnerGuid() || !m_spawnedByDefault)
            {
...
bool GameObject::LoadFromDB(uint32 guid, Map *map)
{
	...
	auto instanceId = map->GetInstanceId();
	if(instanceId)
	{
		CharacterDatabase.BeginTransaction();
		auto* result = CharacterDatabase.PQuery("SELECT state FROM gameobject_instance WHERE instanceID = %u AND guid = %u", instanceId, guid);
		CharacterDatabase.CommitTransaction();
		if(result)
		{
			SetGoState(static_cast<GOState>(result->Fetch()->GetUInt8()), false);
			if (m_cooldownTime == 0)
			{
...
{
	/*if (!target) && !target->GetEntry() == 176511 || !target->GetEntry() == 175124)
		return;
    ...
    {
		if(target->GetEntry() == 800004)
			return;

		Unit* casterUnit = SummonCreature(800004, GetPositionX(), GetPositionY(), GetPositionZ()+GetObjectBoundingRadius(), GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 4000);
...
void GameObject::Use(Unit* user)
{
    ...
    switch(GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_SUMMONING_RITUAL:              //18
        {
			...
			if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1)
			{
				for (int i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
				{
					GuidsSet::const_iterator itr = m_UniqueUsers.begin();
					std::advance(itr, rand() % m_UniqueUsers.size());
					if (Unit* target = ObjectAccessor::GetUnit(*this, *itr))
        ...
        case GAMEOBJECT_TYPE_MEETINGSTONE:                  //23
        {
			...
			if (!targetPlayer || targetPlayer == player || !targetPlayer->IsInSameGroupWith(player)/* || !targetPlayer->IsInSameRaidWith(player)*/)
                return;

			/*if (!player->IsInSameGroupWith(targetPlayer) || !player->IsInSameRaidWith(targetPlayer))
				return;*/
    ...
    auto spell = std::make_shared<Spell>(spellCaster, spellInfo, triggered, GetObjectGuid());
...
void GameObject::SetGoState(GOState state, bool save /* = true */)
{
	...
	if (save)
	{
		auto instanceId = GetMap()->GetInstanceId();
		if (instanceId)
		{
			std::stringstream ss;
			ss << "REPLACE INTO gameobject_instance VALUES ("
				<< instanceId << ", "
				<< GetGUIDLow() << ", "
				<< GetGoState() << ");";
			CharacterDatabase.BeginTransaction();
