...
void WorldSession::HandleGroupInviteOpcode( WorldPacket & recv_data )
{
    ...
    {
        // send "player not found" error, because 1.12 clients don't have ERR_NOT_IN_INSTANCE 
        SendPartyResult(PARTY_OP_INVITE, membername, ERR_BAD_PLAYER_NAME_S);
        return;
    }
    // just ignore us
...
void WorldSession::HandleRequestPartyMemberStatsOpcode( WorldPacket &recv_data )
{
    ...
    for(uint8 i = 0; i < MAX_AURAS; ++i)
    {
        ...
        {
            auramask |= (uint32(1) << i);					//TODO: V610 Undefined behavior. Check the shift operator '<<. The right operand ('i' = [0..47]) is greater than or equal to the length in bits of the promoted left operand.
            data << uint16(aura);
        }
    }
    ...
    if(pet)
    {
        ...
        for(uint8 i = 0; i < MAX_AURAS; ++i)
        {
            ...
            {
                petauramask |= (uint32(1) << i);			//TODO: V610 Undefined behavior. Check the shift operator '<<. The right operand ('i' = [0..47]) is greater than or equal to the length in bits of the promoted left operand.
                data << uint16(petaura);
            }
        }
