...
void BattleGround::ScheduleRevive(uint64 guid)
{
    m_DelayedRevive[guid] = uint32(500);
}

void BattleGround::ProcessResurrect(uint32 diff)
{
    for (std::map<uint64, uint32>::iterator i = m_DelayedRevive.begin(); i != m_DelayedRevive.end();)
    {
        if (i->second <= diff)
        {
            ...
            i = m_DelayedRevive.erase(i);
        }
        else
        {
            i->second = i->second - diff;
            i++;
        }
    }
...
void BattleGround::EndBattleGround(Team winner)
{
    ...
    for(BattleGroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (GetStartTime() >= 600000 || winner == ALLIANCE || winner == HORDE)
        {
            if (team == winner)
            {
...
void BattleGround::RewardQuestComplete(Player *plr)
{
    ...
    switch(GetTypeID())
    {
        ...
        case BATTLEGROUND_WS:
            ...
            break;
        case BATTLEGROUND_AB:
            //quest = SPELL_AB_QUEST_REWARD; //TODO: No such spell exists. Fix this with some code
            return;
        default:
            return;
    }
...
{
    if (GetTypeID() == BATTLEGROUND_AB && creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE))
        creature->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_UNTRACKABLE);;
