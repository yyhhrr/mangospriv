...
bool ChatHandler::HandleChannelListCommand(char* args) {
	...
	char teamChar = tolower(args[0]);
	ChannelMgr* mgr = nullptr;
	std::stringstream strm;

	if(teamChar == 'h') {
		mgr = channelMgr(Team::HORDE);
		strm << "Listing channels for team 'horde':" << std::endl;
	} else if(teamChar == 'a') {
		mgr = channelMgr(Team::ALLIANCE);
		strm << "Listing channels for team 'alliance':" << std::endl;
	} else {
		return false;
	}

	if(mgr == nullptr) {
		PSendSysMessage("|cffff0000No channels available|r");
		return true;
	}

	mgr->fillChannelStringForCommand(strm);
	PSendSysMessage(strm.str().c_str());
...
bool ChatHandler::HandleHonorShow(char* /*args*/)
{
    ...
    PSendSysMessage(LANG_HONOR_LIFE, target->GetStoredHonor(), honorable_kills, dishonorable_kills, highest_rank, hrank_name);
