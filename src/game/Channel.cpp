...
: m_announce(true), m_moderate(false), m_name(name), m_flags(0), m_channelId(channel_id), m_logMessages(false)
{
    ...
    if(ch)                                                  // it's built-in channel
    {
        ...
        m_announce = true;                                 // no join/leave announces
	...
	std::transform(m_name.begin(), m_name.end(), std::back_inserter(nameLower), tolower);
	if(nameLower.find("general") != std::string::npos || nameLower.find("allgemein") != std::string::npos) {
		m_flags = CHANNEL_FLAG_GENERAL;
	}

	if(nameLower.find("trade") != std::string::npos || nameLower.find("handel") != std::string::npos) {
		m_flags = CHANNEL_FLAG_TRADE;
	}

	if(nameLower.find("localdefense - ") != std::string::npos || nameLower.find("lokaleverteidigung - ") != std::string::npos) {
		m_flags = CHANNEL_FLAG_GENERAL;
	}
}
...
void Channel::Join(ObjectGuid p, const char *pass)
{
	...
	if(plr->GetSession()->GetSecurity() < SEC_GAMEMASTER) {
    ...
    if(m_announce && (plr->GetSession()->GetSecurity() < SEC_GAMEMASTER || !sWorld.getConfig(CONFIG_BOOL_SILENTLY_GM_JOIN_TO_CHANNEL) ))
    {
        MakeJoined(&data, p);
        SendToAll(&data);
    }
    ...
    if(!IsConstant() && !m_ownerGuid)
    {
		...
		if(HasFlag(CHANNEL_FLAG_CUSTOM) == true)
			m_players[p].SetModerator(true);
    }

	if(HasFlag(CHANNEL_FLAG_CUSTOM) == false)
		m_players[p].SetModerator(false);
}
...
void Channel::KickOrBan(ObjectGuid good, const char *badname, bool ban)
{
	...
	if(HasFlag(ChannelFlags::CHANNEL_FLAG_CUSTOM) == false) {
...
void Channel::Password(ObjectGuid p, const char *pass)
{
	...
	if(HasFlag(ChannelFlags::CHANNEL_FLAG_CUSTOM) == false) {
		if(sec < SEC_GAMEMASTER) {
			sLog.outChar("CHANNEL_SECURITY: Player %s (GUID: %016llX) tried to set password to non-custom channel %s!",
				plr->GetName(), plr->GetGUID(), m_name.c_str());
...
void Channel::List(Player* player)
{
    ...
    {
        WorldPacket data(SMSG_CHANNEL_LIST, (GetName().size()+1)+1+4+m_players.size()*(8+1));
...
void Channel::Say(ObjectGuid p, const char *what, uint32 lang)
{
    ...
    else
    {
		...
		if(m_logMessages && mLogFile.is_open()) {
			mLogMutex.lock();
			time_t t = time(nullptr);

			tm* TM = localtime(&t);
			mLogFile << "[" << std::setw(2) << std::setfill('0') << TM->tm_hour
				<< ":" << std::setw(2) << std::setfill('0') << TM->tm_min << "] ["
				<< (plr ? plr->GetName() : "null") << "] [" << m_name << "]: "
				<< what << std::endl;
...
void Channel::enableLog(bool enabled) {
	m_logMessages = enabled;
	if(enabled) {
		std::stringstream logFileName;
		logFileName << m_name << ".txt";
		mLogFile.open(logFileName.str().c_str(), std::ios::out);
	} else {
		mLogFile.close();
	}
}
...
void Channel::SetOwner(ObjectGuid guid, bool exclaim)
{
    ...
    if (m_ownerGuid)
    {
		...
		if(HasFlag(CHANNEL_FLAG_CUSTOM) == true && exclaim) {
...
void Channel::writeToStreamForClient(std::stringstream& strm) {
	strm << "|cff00ff00" << m_name << "|r ";
	if(m_password.empty() == false) {
		strm << "- |cffff0000Password: " << m_password << "|r ";
	}

	strm << "- ";
	if(HasFlag(CHANNEL_FLAG_CUSTOM)) {
		strm << "Custom";
	} else if(HasFlag(CHANNEL_FLAG_CITY)) {
		strm << "City";
	...
	strm << " - ";
	if(m_ownerGuid != 0) {
		Player* owner = sObjectMgr.GetPlayer(m_ownerGuid);
		if(owner != nullptr) {
			strm << "Owner: " << (owner->GetSession()->GetPlayerName() != nullptr) ? owner->GetSession()->GetPlayerName() : "null";
		} else {
			strm << "Owner: none";
		}
	} else {
	...
	for(auto& player : m_players) {
		if(player.second.IsModerator()) {
			if(numMods != 0)
				strm << ", ";

			Player* mod = sObjectMgr.GetPlayer(player.second.player);
			if(mod != nullptr) {
				strm << ((mod->GetSession()->GetPlayerName() != nullptr) ? mod->GetSession()->GetPlayerName() : "null");
			}
