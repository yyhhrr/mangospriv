...
private:
    ...
    public:
		...
		void AddFlag(uint8 flag) { m_flags |= flag; }
		void RemoveFlag(uint8 flag) { m_flags &= ~flag; }

		void writeToStreamForClient(std::stringstream& strm);
		...
		void enableLog(bool enabled = true);
