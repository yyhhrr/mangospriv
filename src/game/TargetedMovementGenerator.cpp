...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T& owner, bool updateDestination)
{
    ...
    // offset next path calculation to prevent npcs "shaking"
    if (i_path->getPathType() & PATHFIND_INCOMPLETE)
        i_recheckDistance.Reset(irand(1000, 2500));
    ...
    if (m_followSpeedUp)
    {
        if (i_offset && i_target->IsWithinDistInMap(&owner, 10.f))
            m_followSpeedUp = 0.f;
        else if (!((D*)this)->EnableWalking())
        {
            uint32 moveFlags = (owner.m_movementInfo.GetMovementFlags() & ~MOVEFLAG_WALK_MODE) | MOVEFLAG_SPLINE_ENABLED | MOVEFLAG_FORWARD;
            init.SetVelocity(m_followSpeedUp * owner.GetSpeed(MOVE_RUN));
        }
    }

    int32 time = init.Launch();
    if (forceDest && !((D*)this)->EnableWalking() && !m_followSpeedUp && !((Pet*)&owner)->isInCombat() && time > 3000)
    {
        m_followSpeedUp = std::min(0.5f + ((float)time / 3000.f), 2.5f);
        uint32 moveFlags = (owner.m_movementInfo.GetMovementFlags() & ~MOVEFLAG_WALK_MODE) | MOVEFLAG_SPLINE_ENABLED | MOVEFLAG_FORWARD;
        init.SetVelocity(m_followSpeedUp * owner.GetSpeed(MOVE_RUN));
        init.Launch();
    }
...
{
    return !i_target->IsWithinDist3d(x, y, z, this->GetDynamicTargetDistance(owner, true)) || !i_target->IsWithinLOS(x, y, z);
