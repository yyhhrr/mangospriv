...
void WorldSession::HandleReclaimCorpseOpcode(WorldPacket &recv_data)
{
    ...
    // !Cheating - Wiederbelebung und sowas im Bg
...
void WorldSession::HandleResurrectResponseOpcode(WorldPacket & recv_data)
{
	...
	// Exploit Fix
    if (Player* pRezzer = sObjectMgr.GetPlayer(guid))
    {
        if (sMapStore.LookupEntry(pRezzer->GetMapId())->IsDungeon() && !pRezzer->IsInSameRaidWith(GetPlayer()) && !(pRezzer->GetMapId() != GetPlayer()->m_resurrectMap))
        {
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
	...
	auto plr = GetPlayer();
	MapEntry const* mEntry = sMapStore.LookupEntry(at->target_mapId);

	if (!plr || !mEntry)
		return;

	DungeonPersistentState* state = plr->GetBoundInstanceSaveForSelfOrGroup(at->target_mapId);

	//Check if the player has already joined 5 IDs in the last hour and if so, deny entry
	if (!state && mEntry->IsDungeon())
	{
		if (plr->m_instanceResetTimer.size() >= 5 && GetSecurity() <= SEC_PLAYER)
		{
            if (GetSessionDbcLocale() == LOCALE_deDE)
                SendAreaTriggerMessage("Ihr habt in letzter Zeit zu viele Instanzen betreten");
            else
			    SendAreaTriggerMessage("You have entered too many instances recently");
			return;
		}
		auto now = time(nullptr) + 3600;
		CharacterDatabase.PExecute("INSERT INTO character_instance_reset_time VALUES (%u, %I64u)", plr->GetAccountId(), now);
		plr->m_instanceResetTimer.push_back(now);
	}
