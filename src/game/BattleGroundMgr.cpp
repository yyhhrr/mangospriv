...
void BattleGroundMgr::BuildPvpLogDataPacket(WorldPacket *data, BattleGround *bg)
{
    ...
    *data << (uint32)(bg->GetPlayerScoresSize());
	...
	uint32 n = 0;
    for(BattleGround::BattleGroundScoreMap::const_iterator itr = bg->GetPlayerScoresBegin(); itr != bg->GetPlayerScoresEnd() && n < 40; ++itr, ++n)
    {
        *data << ObjectGuid(itr->first);

        Player *plr = sObjectMgr.GetPlayer(itr->first);

        *data << uint32(plr ? plr->GetHonorRankInfo().rank : 0);
...
{
    if (BattleGroundMap* BgMap = bg->GetBgMap())
    {
        if (Creature* SpiritHealer = BgMap->GetCreature(guid))
        {
            SpiritHealer->SendAreaSpiritHealerQueryOpcode(player);
            return;
        }
    }
