...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void Process(Loot& loot, Player* owner) const;                     // Rolls an item from the group (if any) and adds the item to the loot
...
LootSlotType LootItem::GetSlotTypeForSharedLoot(PermissionTypes permission, Player* viewer, bool condition_ok /*= false*/) const
{
    ...
    if (rollWinner)
    {
        if (rollWinner == viewer->GetGUIDLow())
...
bool Loot::hasLootItem(uint32 itemid) {
	for(auto& item : m_questItems) {
...
void Loot::removeLootItem(uint32 itemid) {
	std::remove_if(m_questItems.begin(), m_questItems.end(), [&itemid](LootItemList::value_type& itm) { return itm.itemid == itemid; });
	std::remove_if(items.begin(), items.end(), [&itemid](LootItemList::value_type& itm) { return itm.itemid == itemid; });
}
...
bool Loot::FillLoot(uint32 loot_id, LootStore const& store, Player* loot_owner, bool personal, bool noEmptyError)
{
	...
	sScriptMgr.OnUpdateLoot(*this, loot_owner, loot_id);
...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
    ...
    Player *PriorityLooter = nullptr;
    if (l.GroupLooterGuid)
        PriorityLooter = sObjectMgr.GetPlayer(l.GroupLooterGuid);
    ...
    if (q_itr != lootPlayerQuestItems.end())
    {
        ...
        for (QuestItemList::const_iterator qi = q_list->begin() ; qi != q_list->end(); ++qi)
        {
            ...
            if (lv.permission == GROUP_PERMISSION && item.AllowedForPlayer(PriorityLooter))
                continue;
...
void LootTemplate::LootGroup::Process(Loot& loot, Player* owner) const
{
    LootStoreItem const * item = nullptr;
	int i = 0;
...
void LootTemplate::Process(Loot& loot, LootStore const& store, bool rate, Player* owner, uint8 groupId) const
{
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        Groups[groupId-1].Process(loot, owner);
        return;
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin() ; i != Entries.end() ; ++i )
    {
        ...
        if (i->mincountOrRef < 0)                           // References processing
        {
            ...
            for (uint32 loop=0; loop < i->maxcount; ++loop )// Ref multiplicator
                Referenced->Process(loot, store, rate, owner, i->group);
        }
