...
void BattleGroundWS::CheckPlayerSize()
{
	//weltklasse
	Map* pMap = GetBgMap();
	...
	uint32 m_pAlliancePlayers = GetInvitedCount(ALLIANCE);
	uint32 m_pHordePlayers = GetInvitedCount(HORDE);

	if (PlayerList.getSize() > 20)
	{
		SetMaxPlayers(20);
		SetMaxPlayersPerTeam(10);

		if (m_pAlliancePlayers > 10)
		{
			Map::PlayerList::const_iterator itr = PlayerList.getFirst();
			Player* pPlayer = itr->getSource();

			if (pPlayer->GetTeam() == ALLIANCE)
			{
				RemovePlayerAtLeave(pPlayer->GetObjectGuid(), true, true);

				std::ofstream logFile("bg-removed-players.txt", std::ios::app);
				if (logFile)
					logFile << pPlayer->GetName() << "\n";
			}
		}
		else if (m_pHordePlayers > 10)
		{
			Map::PlayerList::const_iterator itr = PlayerList.getFirst();
			Player* pPlayer = itr->getSource();
...
void BattleGroundWS::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
	{
		...
		{
			BattleGroundWS::CheckPlayerSize();
...
void BattleGroundWS::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
    ...
    if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetEntry() == WS_FLAG_ALLIANCE)
    {
        if (Source->GetTeam() == ALLIANCE)
        {
    ...
    if (GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetEntry() == WS_FLAG_HORDE)
    {
