...
struct SpellModifier
{
    ...
    uint32 spellId;
    std::shared_ptr<Spell> lastAffected;                              // mark last charge user, used for cleanup delayed remove spellmods at spell success or restore charges at cast fail (Is one pointer only need for cases mixed castes?)
};
...
struct LookingForGroupSlot
{
    ...
    bool canAutoJoin() const { return entry; }
...
class MANGOS_DLL_SPEC Player : public Unit
{
    ...
    public:
		...
		uint32 GetRaidProgress(uint32 raid) const;
		void SetRaidProgress(uint32 raid, uint32 progress) const;
		void UpdateRaidProgress(uint32 raid, uint32 progress) const;
        ...
        bool TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options = 0, bool anticheat = false);
        ...
        void LoadCorpse();
        void LoadPet();
		void summonGuardian(uint32 entry, int32 duration, bool replaceOld);
        ...
        bool IsAffectedBySpellmod(SpellEntry const *spellInfo, SpellModifier *mod, const std::shared_ptr<Spell> spell = NULL);
        template <class T> T ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, const std::shared_ptr<Spell> spell = NULL);
        SpellModifier* GetSpellMod(SpellModOp op, uint32 spellId) const;
		SpellModList* GetSpellModifier();
        void RemoveSpellMods(const std::shared_ptr<Spell> spell);
		void ResetSpellModsDueToCanceledSpell (const std::shared_ptr<Spell> spell);
        ...
        void AddSpellAndCategoryCooldowns(SpellEntry const* spellInfo, uint32 itemId, std::shared_ptr<Spell> spell = NULL, bool infinityCooldown = false );
        void AddSpellCooldown(uint32 spell_id, uint32 itemid, time_t end_time);
        void SendCooldownEvent(SpellEntry const *spellInfo, uint32 itemId = 0, std::shared_ptr<Spell> spell = NULL);
		...
		bool IsPvpAccount() const
		{
			return mIsPvpAccount;
		}
		void IsPvpAccount(bool set)
		{
			mIsPvpAccount = set;
		}
        ...
        void RewardReputation(uint32 faction_id, int32 ammount);
		...
		void AttemptJoin(Player* _player, uint32 update_diff, uint32 p_time);
        ...
        void RemoveFromMeetingStoneQueue(uint32 arg);
		void SendMeetingStoneLeave(WorldPacket & recv_data);
	...
	protected:
		bool mIsPvpAccount = false;

		time_t lastTimeLooted = 0;
    ...
    private:
		...
		std::vector<time_t> m_instanceResetTimer;
...
template <class T> T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, const std::shared_ptr<Spell> spell)
{
    ...
    // If non-instant spell -> instant spell dont apply flat modifiers
    if (op == SPELLMOD_CASTING_TIME && totalpct <= -100)
        totalflat = 0;
