...
Player::Player (WorldSession *session): Unit(), m_mover(this), m_camera(this), m_reputationMgr(this), mAI(nullptr)
{
    m_transport = 0;
...
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT account FROM characters WHERE guid = %I64u", GetGUID());
	uint32 accountId = 0;
	if (result)
...
uint32 Player::GetRaidProgress(uint32 raid) const
{
	QueryResult* result = CharacterDatabase.PQuery("SELECT bosses FROM character_raid WHERE guid = %I64u AND raid = %u", GetGUID(), raid);
...
void Player::SetRaidProgress(uint32 raid, uint32 progress) const
{
	CharacterDatabase.PExecute("REPLACE INTO character_raid VALUES (%I64u, %u, %u)", GetGUID(), raid, progress);
}

void Player::UpdateRaidProgress(uint32 raid, uint32 progress) const
{
	CharacterDatabase.PExecute("INSERT INTO character_bosskill VALUES ( %I64u, %u, %u, CURRENT_TIMESTAMP )", GetGUID(), raid, progress);

	auto bosses = GetRaidProgress(raid);
	bosses |= progress;
	SetRaidProgress(raid, bosses);
}
...
bool Player::Create( uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId */)
{
    ...
    if(oEntry && sConfig.GetBoolDefault("StartPlayerGear", true))
    {
...
{
    if(!isAlive() || isGameMaster() || isTotalImmune())
        return 0;
...
void Player::Update( uint32 update_diff, uint32 p_time )
{
	...
	auto removeTime = [](time_t& t)
	{
		return t <= time(nullptr);
	};

	for(auto& t : m_instanceResetTimer)
	{
		if(removeTime(t))
			CharacterDatabase.PExecute("DELETE FROM character_instance_reset_time WHERE account = %u AND time = %I64u", GetAccountId(), t);
	}

	m_instanceResetTimer.erase(
		std::remove_if(std::begin(m_instanceResetTimer), std::end(m_instanceResetTimer), removeTime),
		std::end(m_instanceResetTimer));
    ...
    if (hasUnitState(UNIT_STAT_MELEE_ATTACKING) && !hasUnitState(UNIT_STAT_CAN_NOT_REACT) && !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED))
    {
    ...
    if (m_hintTimer)
    {
        if (m_hintTimer <= update_diff)
        {
            if (getLevel() < 60)
                ChatHandler(this).SendSysMessage(sObjectMgr.GetRandomHintText());
            m_hintTimer = 0;
        }
        else
            m_hintTimer -= update_diff;
    }
}
...
void Player::SetDeathState(DeathState s)
{
    ...
    if(s == JUST_DIED && cur)
    {
		...
		if(getClass() == CLASS_WARRIOR) {
			auto auras = GetAurasByType(SPELL_AURA_MOD_SHAPESHIFT);
			for(auto& aura : auras) {
				if(aura->GetCaster() == this) {
					...
					default:
						RemoveAura(aura, AURA_REMOVE_BY_DEATH);
						break;
					}
				}
			}
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options, bool anticheat)
{
	if (anticheat)
	{
    ...
    if ((GetMapId() == mapid) && (!m_transport))
    {
        ...
        m_teleport_dest = WorldLocation(mapid, x, y, z+1.0f, orientation);
                ...
                pSkill->id==SKILL_LOCKPICKING && _spell_idx->second->max_value==0)
            {
                switch(GetSkillRangeType(pSkill, _spell_idx->second->racemask != 0))
                {
                    ...
                    case SKILL_RANGE_LEVEL:
                        if ((pSkill->id == 160 || pSkill->id == 172) && getClass() == CLASS_SHAMAN)
...
uint32 Player::resetTalentsCost() const
{
    ...
    if(m_resetTalentsCost < 1*SILVER)
        return 1*SILVER;
    // then 5 gold
    else if(m_resetTalentsCost < 5*SILVER)
        return 5*SILVER;
    // After that it increases in increments of 5 gold
    else if(m_resetTalentsCost < 10*SILVER)
        return 10*SILVER;
    else
    {
...
PlayerAI* getAi(Player* plr, uint32 spellId)
{
	switch(plr->getClass())
	{
	case CLASS_WARRIOR:
	...
	case CLASS_DRUID:
	default:
		return new PlayerAI(plr, spellId);
	}
}
...
void Player::SetCharmed(bool apply, uint64 casterGUID, uint32 spellID)
{
    if (apply)
        ...
        TryResetClientControl();
    if (apply)
	{
		delete mAI;
		mAI = getAi(this, spellID);
		
        return;

        if (Unit* pCaster = ObjectAccessor::GetUnit(*this,casterGUID))
        {
            if (!pCaster->CanHaveThreatList()) // Wenn der Caster keine ThreatList hat, mssen wir dem Spieler sein Target in die Liste schreiben
            {
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
    ...
    auto* head = GetItemByPos(INVENTORY_SLOT_BAG_0, SLOT_HEAD);
    auto* legs = GetItemByPos(INVENTORY_SLOT_BAG_0, SLOT_LEGS);
    auto* mainhand = GetItemByPos(INVENTORY_SLOT_BAG_0, SLOT_MAIN_HAND);
    
    // correct reapply all effects
    ApplyEnchantment(head, false);
    ApplyEnchantment(head, true);
    ApplyEnchantment(legs, false);
    ApplyEnchantment(legs, true);
    ApplyEnchantment(mainhand, false);
    ApplyEnchantment(mainhand, true);
...
void Player::RepopAtGraveyard()
{
    ...
    // Such zones are considered unreachable as a ghost and the player must be automatically revived - dotox
    if (!isAlive() && GetPositionZ() < -500.0f)
    {
...
static double crit_ratio[MAX_CLASSES][60] = {
    ...
    {0.002174, 0.002070, 0.002070, 0.001976, 0.001976, 0.001890, 0.001890, 0.001812, 0.001812, 0.001739, 0.001739, 0.001672, 0.001553, 0.001553, 0.001449, 0.001449, 0.001403, 0.001318, 0.001318, 0.001242, 0.001208, 0.001208, 0.001144, 0.001115, 0.001087, 0.001060, 0.001035, 0.001011, 0.000988, 0.000945, 0.000925, 0.000925, 0.000887, 0.000870, 0.000836, 0.000820, 0.000820, 0.000791, 0.000776, 0.000750, 0.000737, 0.000737, 0.000713, 0.000701, 0.000679, 0.000669, 0.000659, 0.000639, 0.000630, 0.000612, 0.000604, 0.000596, 0.000580, 0.000572, 0.000557, 0.000550, 0.000544, 0.000530, 0.000524, 0.000506}, //Paladin
    {0.002727, 0.002721, 0.002598, 0.002417, 0.002317, 0.002224, 0.002138, 0.002058, 0.001938, 0.001871, 0.001735, 0.001557, 0.001434, 0.001328, 0.001217, 0.001154, 0.001081, 0.001004, 0.000947, 0.000885, 0.000849, 0.000797, 0.000759, 0.000716, 0.000684, 0.000654, 0.000621, 0.000596, 0.000567, 0.000541, 0.000524, 0.000501, 0.000479, 0.000462, 0.000443, 0.000428, 0.000411, 0.000395, 0.000380, 0.000368, 0.000357, 0.000344, 0.000331, 0.000320, 0.000308, 0.000300, 0.000289, 0.000278, 0.000269, 0.000260, 0.000253, 0.000245, 0.000237, 0.000228, 0.000221, 0.000215, 0.000208, 0.000201, 0.000194, 0.000188}, //Hunter
    {0.004476, 0.004290, 0.004118, 0.003813, 0.003677, 0.003550, 0.003321, 0.003217, 0.003120, 0.002941, 0.002640, 0.002394, 0.002145, 0.001980, 0.001775, 0.001660, 0.001560, 0.001450, 0.001355, 0.001271, 0.001197, 0.001144, 0.001084, 0.001040, 0.000980, 0.000936, 0.000903, 0.000865, 0.000830, 0.000792, 0.000768, 0.000741, 0.000715, 0.000691, 0.000664, 0.000643, 0.000628, 0.000609, 0.000592, 0.000572, 0.000556, 0.000542, 0.000528, 0.000512, 0.000497, 0.000486, 0.000474, 0.000464, 0.000454, 0.000440, 0.000431, 0.000422, 0.000412, 0.000404, 0.000394, 0.000386, 0.000378, 0.000370, 0.000364, 0.000345}, //Rogue
    {0.000909, 0.000909, 0.000909, 0.000865, 0.000865, 0.000865, 0.000865, 0.000826, 0.000826, 0.000826, 0.000826, 0.000790, 0.000790, 0.000790, 0.000790, 0.000757, 0.000757, 0.000757, 0.000727, 0.000727, 0.000727, 0.000727, 0.000699, 0.000699, 0.000699, 0.000673, 0.000673, 0.000673, 0.000649, 0.000649, 0.000649, 0.000627, 0.000627, 0.000627, 0.000606, 0.000606, 0.000606, 0.000586, 0.000586, 0.000586, 0.000568, 0.000568, 0.000551, 0.000551, 0.000551, 0.000534, 0.000534, 0.000519, 0.000519, 0.000519, 0.000505, 0.000505, 0.000491, 0.000491, 0.000478, 0.000478, 0.000466, 0.000466, 0.000454, 0.000454}, //Priest
    {0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000}, //None
    {0.001663, 0.001663, 0.001583, 0.001583, 0.001511, 0.001511, 0.001511, 0.001446, 0.001446, 0.001385, 0.001385, 0.001330, 0.001330, 0.001279, 0.001231, 0.001188, 0.001188, 0.001147, 0.001147, 0.001073, 0.001073, 0.001039, 0.001039, 0.001008, 0.000978, 0.000950, 0.000950, 0.000924, 0.000924, 0.000875, 0.000875, 0.000853, 0.000831, 0.000831, 0.000792, 0.000773, 0.000773, 0.000756, 0.000756, 0.000723, 0.000707, 0.000707, 0.000693, 0.000679, 0.000665, 0.000652, 0.000639, 0.000627, 0.000627, 0.000605, 0.000594, 0.000583, 0.000583, 0.000573, 0.000554, 0.000545, 0.000536, 0.000536, 0.000528, 0.000508}, //Shaman
    {0.000771, 0.000771, 0.000771, 0.000735, 0.000735, 0.000735, 0.000735, 0.000735, 0.000735, 0.000701, 0.000701, 0.000701, 0.000701, 0.000701, 0.000671, 0.000671, 0.000671, 0.000671, 0.000671, 0.000643, 0.000643, 0.000643, 0.000643, 0.000617, 0.000617, 0.000617, 0.000617, 0.000617, 0.000593, 0.000593, 0.000593, 0.000593, 0.000571, 0.000571, 0.000571, 0.000551, 0.000551, 0.000551, 0.000551, 0.000532, 0.000532, 0.000532, 0.000532, 0.000514, 0.000514, 0.000514, 0.000498, 0.000498, 0.000498, 0.000482, 0.000482, 0.000482, 0.000467, 0.000467, 0.000467, 0.000454, 0.000454, 0.000454, 0.000441, 0.000441}, //Mage
    {0.001500, 0.001500, 0.001429, 0.001429, 0.001429, 0.001364, 0.001364, 0.001364, 0.001304, 0.001304, 0.001250, 0.001250, 0.001250, 0.001200, 0.001154, 0.001111, 0.001111, 0.001111, 0.001071, 0.001034, 0.001000, 0.001000, 0.000968, 0.000968, 0.000909, 0.000909, 0.000909, 0.000882, 0.000882, 0.000833, 0.000833, 0.000811, 0.000811, 0.000789, 0.000769, 0.000750, 0.000732, 0.000732, 0.000714, 0.000698, 0.000682, 0.000682, 0.000667, 0.000667, 0.000638, 0.000625, 0.000625, 0.000612, 0.000600, 0.000588, 0.000577, 0.000577, 0.000566, 0.000556, 0.000545, 0.000536, 0.000526, 0.000517, 0.000517, 0.000500}, //Warlock
    {0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000, 0.010000}, //None
    {0.002020, 0.002020, 0.001923, 0.001923, 0.001836, 0.001836, 0.001756, 0.001756, 0.001683, 0.001553, 0.001496, 0.001496, 0.001443, 0.001443, 0.001346, 0.001346, 0.001303, 0.001262, 0.001262, 0.001122, 0.001122, 0.001092, 0.001063, 0.001063, 0.001010, 0.001010, 0.000985, 0.000962, 0.000962, 0.000878, 0.000859, 0.000859, 0.000841, 0.000824, 0.000808, 0.000792, 0.000777, 0.000777, 0.000762, 0.000709, 0.000696, 0.000696, 0.000685, 0.000673, 0.000651, 0.000641, 0.000641, 0.000631, 0.000621, 0.000585, 0.000577, 0.000569, 0.000561, 0.000561, 0.000546, 0.000539, 0.000531, 0.000525, 0.000518, 0.000500} //Druid
};
...
float Player::GetDodgeFromAgility()
{
    ...
    float dodge = dodge_base[pclass-1] + GetStat(STAT_AGILITY) * crit_ratio[pclass-1][level-1] * crit_to_dodge[pclass-1];
    return dodge*100.0f;
}
...
float Player::OCTRegenMPPerSpirit()
{
    ...
    switch (Class)
    {
        ...
        case CLASS_SHAMAN:  addvalue = (Spirit/5 + 15);   break;
        case CLASS_PRIEST:  addvalue = (Spirit/4 + 13); break;
        case CLASS_MAGE:    addvalue = (Spirit/4 + 13); break;
        case CLASS_WARLOCK: addvalue = (Spirit/4 +  8);   break;
    }
...
bool Player::UpdateCraftSkill(uint32 spellid)
{
    ...
    for(SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
    {
        if (_spell_idx->second->skillId)
        {
			...
			//Account Premium:
			if(GetSession()->IsPremium())
			craft_skill_gain *= sWorld.getConfig(CONFIG_UINT32_SKILL_GAIN_CRAFTING_PREMIUM);
...
bool Player::UpdateGatherSkill(uint32 SkillId, uint32 SkillValue, uint32 RedLevel, uint32 Multiplicator )
{
	...
	//Account Premium:
	if(GetSession()->IsPremium())
	gathering_skill_gain *= sWorld.getConfig(CONFIG_UINT32_SKILL_GAIN_GATHERING_PREMIUM);
...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
    ...
    int32 skilldif = 6 * plevel - (defence ? GetBaseDefenseSkillValue() : GetBaseWeaponSkillValue(attType));
...
//For Script-Usage
void Player::RewardReputation(uint32 faction_id, int32 ammount)
{
    ...
    GetReputationMgr().ModifyReputation(pFaction, ammount);
}
...
void Player::UpdateHonor()
{
	...
	/* Zukunfts Dotox
	float RankPoints = GetRankPoints() * 2;
	HonorRankInfo prk = MaNGOS::Honor::CalculateHonorRank(RankPoints);*/
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    {
        case AREATEAM_ALLY:
			pvpInfo.inHostileArea = GetTeam() != ALLIANCE && (sWorld.IsPvPRealm() || zone->flags & AREA_FLAG_CAPITAL || IsPvpAccount());
            break;
        case AREATEAM_HORDE:
			pvpInfo.inHostileArea = GetTeam() != HORDE && (sWorld.IsPvPRealm() || zone->flags & AREA_FLAG_CAPITAL || IsPvpAccount());
            break;
        case AREATEAM_NONE:
            // overwrite for battlegrounds, maybe batter some zone flags but current known not 100% fit to this
			pvpInfo.inHostileArea = sWorld.IsPvPRealm() || InBattleGround() || IsPvpAccount();
            break;
        default:                                            // 6 in fact
	...
	// Quelle: youtu.be/iUFpZeNGPSs?t=32m
	if (!IsTaxiFlying())
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    // reset reckoning charges
    if (getClass() == CLASS_PALADIN)
        m_extraAttacks = 0;
    if (duel->opponent->getClass() == CLASS_PALADIN)
        duel->opponent->m_extraAttacks = 0;
...
void Player::_ApplyItemBonuses(ItemPrototype const *proto,uint8 slot,bool apply)
{
	...
	*/
	...
	{
        if (proto->Damage[i].DamageType == SPELL_SCHOOL_NORMAL || attType == RANGED_ATTACK)
		{
			if (proto->Damage[i].DamageMax > 0 && proto->Damage[i].DamageMin > 0)
			{
				SetBaseWeaponDamage(attType, MINDAMAGE, (apply ? proto->Damage[i].DamageMin : BASE_MINDAMAGE));
				SetBaseWeaponDamage(attType, MAXDAMAGE, (apply ? proto->Damage[i].DamageMax : BASE_MAXDAMAGE));
			}
		}
		else
		{
			auto school = static_cast<SpellSchools>(proto->Damage[i].DamageType);
			SetWeaponElementalDamage(school, attType, MINDAMAGE, (apply ? proto->Damage[i].DamageMin : 0));
			SetWeaponElementalDamage(school, attType, MAXDAMAGE, (apply ? proto->Damage[i].DamageMax : 0));
		}
	}
...
void Player::ApplyEquipSpell(SpellEntry const* spellInfo, Item* item, bool apply, bool form_change)
{
    ...
    else
    {
        ...
        if (item)
            ...
            RemoveAurasDueToItemSpell(nullptr, spellInfo->Id);           // un-apply spell (item set case)
    }
}
...
void Player::CastItemCombatSpell(Unit* Target, WeaponAttackType attType)
{
    ...
    if (std::shared_ptr<Spell> spell = GetCurrentSpell(CurrentSpellTypes(CURRENT_MELEE_SPELL)))
...
void Player::CastItemUseSpell(Item *item,SpellCastTargets const& targets)
{
    ...
    for(int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
		...
		auto spell = std::make_shared<Spell>(this, spellInfo, (count > 0));
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
	...
	if(loot_type != LootType::LOOT_PICKPOCKETING)
    ...
    switch(guid.GetHigh())
    {
        ...
        case HIGHGUID_UNIT:
        {
            ...
            else
            {
                ...
                // usually our loot will be generated upon creature death
                if (!creature->lootForBody)
                {
                    ...
                    if (group)
                    {
                        ...
                        loot->GroupLooterGuid = group->GetLooterGuid();
                    }
                ...
                if (!creature->lootGroupCheck && group)
                {
                    creature->lootGroupCheck = true;

                    ObjectGuid looterGuid = group->GetLooterGuid();
                    if (loot->GroupLooterGuid)
                        group->SetLooterGuid(loot->GroupLooterGuid);
                    ...
                    group->SetLooterGuid(looterGuid);
                }
                ...
                else
                {
                    if(Group* group = creature->GetGroupLootRecipient())
                    {
                        if (group == GetGroup())
                        {
                            ...
                            else if (!creature->loot.GroupLooterGuid || creature->loot.GroupLooterGuid == GetObjectGuid())
                                permission = ALL_PERMISSION;
                            else
                                permission = GROUP_PERMISSION;
                        }
...
void Player::SendInitWorldStates(uint32 zoneid)
{
	...
	uint32 count = 0;
	WorldPacket data(SMSG_INIT_WORLD_STATES, 4 + 4 + 12);
	data << mapid;
	data << zoneid;
...
Item* Player::StoreNewItem( ItemPosCountVec const& dest, uint32 item, bool update,int32 randomPropertyId )
{
    ...
    if( pItem )
    {
        ...
        if (pItem && pItem->GetProto()->Quality >= ITEM_QUALITY_EPIC)
        {
            CharacterDatabase.BeginTransaction();
            _SaveInventory();
			//CharacterDatabase.PExecute("INSERT INTO character_receivedItem VALUES ( %I64u, %u, %u, CURRENT_TIMESTAMP )", GetGUID(), item, count);
            CharacterDatabase.CommitTransaction();
        }
...
void Player::DestroyItem( uint8 bag, uint8 slot, bool update )
{
    ...
    if( pItem )
    {
		...
		//CharacterDatabase.PExecute("INSERT INTO character_destroyedItem VALUES ( %I64u, %u, %u, %u, CURRENT_TIMESTAMP )", GetGUID(), pItem->GetEntry(), pItem->GetCount(), 3);
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, Object* questGiver, bool announce)
{
	...
	//TODO: Nur dafür da um Exploiter ausfindig zu machen - danach wieder löschen!
	/*if(quest_id == 7830)
	{
		std::ofstream logFile("exploitlog.txt", std::ios::app);
		if(logFile)
		{
			typedef std::chrono::system_clock Clock;
			auto now = Clock::now();
			std::time_t now_c = Clock::to_time_t(now);
			tm* time = std::localtime(&now_c);

			logFile << "[" << std::setfill('0') << std::setw(2) << time->tm_mday << "-"
				<< std::setfill('0') << std::setw(2) << time->tm_mon + 1 << "-" << time->tm_year + 1900 << " " 
				<< std::setfill('0') << std::setw(2) << time->tm_hour + 1 << ":"
				<< std::setfill('0') << std::setw(2) << time->tm_min << "]: " << GetName() << "\n";
		}
	}
	*/
	
	/*
	if (quest_id == 20031 || quest_id == 20032 || quest_id == 20035 || quest_id == 20036)
		WorldDatabase.PExecute("UPDATE world_events SET progress =progress+0.015 WHERE id = '1';");

	if (quest_id == 20033 || quest_id == 20034)
		WorldDatabase.PExecute("UPDATE world_events SET progress =progress+0.04 WHERE id = '1';");

	if (quest_id == 20037 || quest_id == 20038)
		WorldDatabase.PExecute("UPDATE world_events SET progress =progress+0.01 WHERE id = '1';");
	*/
	...
	//Premium System:
	if (GetSession()->IsPremium() && GetSession()->GetPlayer()->getXPRate() == 0)
	{
		XP = 0;
	}
	else if (GetSession()->IsPremium() && GetSession()->GetPlayer()->getXPRate() > 0)
		XP = XP;
...
bool Player::HasQuestForItem( uint32 itemid ) const
{
    for( int i = 0; i < MAX_QUEST_LOG_SIZE; ++i )
    {
		...
		if (itemid == 21110 && questid == 8620) //Hackfix: Some questitems (like 21112) require looted items in order to be created. should check for that
			return true;
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder)
{
	...
	//"health, power1, power2, power3, power4, power5, exploredZones, equipmentCache, ammoId, actionBars, honor_current_ranking, isPvpFlagged  FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
	...
	if (m_bgData.bgInstanceID)                              //saved in BattleGround
	{
		...
		else
		{
			// leave bg
			if (player_at_bg)
				currentBg->RemovePlayerAtLeave(GetObjectGuid(), true, true);
	...
	bool isPvpFlagged = fields[56].GetBool();
	if (isPvpFlagged)
		UpdatePvP(true, true);
	...
	auto* res = LoginDatabase.PQuery("SELECT pvp FROM account WHERE id = %u", GetAccountId());
	if (!res)
		return true;

	auto rawString = res->Fetch()->GetString();
	if (rawString)
	{
		std::string pvp(rawString);
		if (pvp == "PVP")
			IsPvpAccount(true);
	}
	...
	auto resetTimes = CharacterDatabase.PQuery("SELECT time FROM character_instance_reset_time WHERE account = %u", GetAccountId());

	if (!resetTimes)
		return true;

	auto times = resetTimes->Fetch();
	do
	{
		auto time = times->GetUInt64();
		m_instanceResetTimer.push_back(time);
	} while (resetTimes->NextRow());
...
{
	// 0 = Windows; 1 = MacOS
	QueryResult* result = LoginDatabase.PQuery("SELECT os FROM account WHERE id = %u", GetAccountId());
	uint32 os = 0;

	if (result)
	{
		std::string pOsString = result->Fetch()[0].GetString();
...
void Player::SaveToDB()
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "REPLACE INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, exploredZones, equipmentCache, ammoId, actionBars, honor_current_ranking, isPvpFlagged) "
        "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
        "?, ?, ?, ?, ?, "
        "?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, "
	...
	uberInsert.addInt32(MaNGOS::Honor::CalculateHonorRank(m_stored_honor).rank);

	uberInsert.addBool(IsPvP());
...
void Player::_SaveAuras()
{
    ...
    for(SpellAuraHolderMap::const_iterator itr = auraHolders.begin(); itr != auraHolders.end(); ++itr)
    {
        ...
        // do not saves single target holders
...
void Player::UpdatePvPFlag(time_t currTime)
{
    ...
    if(pvpInfo.endTimer == 0)
        return;
	if (currTime > (pvpInfo.endTimer + 300))
		UpdatePvP(false);
}
...
void Player::Whisper(const std::string& text, uint32 language, ObjectGuid receiver)
{
	...
	/*//Mein Charakter kann nur von Freunden angeschrieben werden - keine Nervensägen mehr.
	std::string playername = rPlayer->GetName();
	if (playername == "Phuram")
	{
		Player *pPlayer = GetCharmerOrOwnerPlayerOrPlayerItself();
		if (pPlayer == nullptr)
			return;

		if (!rPlayer->GetSocial()->HasFriend(pPlayer->GetObjectGuid()) && !rPlayer->IsInSameGroupWith(pPlayer) || !rPlayer->IsInSameRaidWith(pPlayer))
		{
			ChatHandler(this).SendSysMessage("Phuram kann nur von Spielern aus seiner Freundesliste angeschrieben werden!");
			return;
		}
	}*/
...
bool Player::IsAffectedBySpellmod(SpellEntry const *spellInfo, SpellModifier *mod, const std::shared_ptr<Spell> spell)
{
    if (!mod || !spellInfo)
        return false;
...
SpellModList* Player::GetSpellModifier()
{
	return m_spellMods;
}

void Player::RemoveSpellMods(const std::shared_ptr<Spell> spell)
{
    ...
    for(int i = 0; i < MAX_SPELLMOD; ++i)
    {
        for (SpellModList::const_iterator itr = m_spellMods[i].begin(); itr != m_spellMods[i].end();)
        {
            ...
            if (mod->charges == -1 && (mod->lastAffected == spell || mod->lastAffected==NULL))
            {
...
void Player::ResetSpellModsDueToCanceledSpell (const std::shared_ptr<Spell> spell)
{
    for(int i = 0; i < MAX_SPELLMOD; ++i )
    {
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
	...
	if (m_session->GetPlayer()->getInstantFlight() >= 1 && GetSession()->IsPremium())
    {
        TaxiNodesEntry const* lastnode = sTaxiNodesStore.LookupEntry(nodes[nodes.size()-1]);
        m_taxi.ClearTaxiDestinations();
		CastSpell(m_session->GetPlayer(), 2479, true);
    ...
    else
    {
        ...
        if (std::shared_ptr<Spell> spell = GetCurrentSpell(CURRENT_GENERIC_SPELL))
            if (spell->m_spellInfo->Id != spellid)
                InterruptSpell(CURRENT_GENERIC_SPELL,false);

        InterruptSpell(CURRENT_AUTOREPEAT_SPELL,false);

        if (std::shared_ptr<Spell> spell = GetCurrentSpell(CURRENT_CHANNELED_SPELL))
            if (spell->m_spellInfo->Id != spellid)
...
/*
 * User can specify his XP Rate for kills, quests or exploration
 * command.Rate
*/
...
void Player::setInstantFlight(uint32 pInstantFlight)
{
	if (GetSession()->IsPremium())
	{
		if (pInstantFlight > 1)
	...
	pFlight = pInstantFlight;
}

uint32 Player::getInstantFlight()
{
    return pFlight;
}
...
void Player::UpdateHomebindTime(uint32 time)
{
    ...
    else if (m_HomebindTimer > 0)
    {
        ...
        {
            // trigger Hearthstone cooldown
            SpellEntry const *spellInfo = sSpellStore.LookupEntry(8690);
            if (!spellInfo)
            {
                auto spell = std::make_shared<Spell>(this, spellInfo, true);
...
void Player::UpdatePvP(bool state, bool ovrride)
{
    if(!state || ovrride)
    {
        SetPvP(state || IsPvpAccount());
        pvpInfo.endTimer = 0;
    }
    else
    {
        // player has pvp toggled off
        if (!IsPvP())
        {
...
void Player::AddSpellAndCategoryCooldowns(SpellEntry const* spellInfo, uint32 itemId, std::shared_ptr<Spell> spell, bool infinityCooldown)
{
...
void Player::SendCooldownEvent(SpellEntry const *spellInfo, uint32 itemId, std::shared_ptr<Spell> spell)
{
...
void Player::SendUpdateToOutOfRangeGroupMembers()
{
	...
	Player* pPlayer = this;
	if (!pPlayer || pPlayer == nullptr || !pPlayer->IsInWorld())
		return;

	group->UpdatePlayerOutOfRange(pPlayer);

    m_groupUpdateMask = GROUP_UPDATE_FLAG_NONE;
    m_auraUpdateMask = 0;

	Pet *pPet = GetPet();
	if (!pPet || pPet == nullptr)
		return;

	if (pPet)
		pPet->ResetAuraUpdateMask();
}
...
void Player::ApplyEquipCooldown( Item * pItem )
{
    ...
    // reset reckoning charges
    if (pItem->GetProto()->Class == ITEM_CLASS_WEAPON && getClass() == CLASS_PALADIN)
        m_extraAttacks = 0;
...
void Player::TryResetClientControl()
{
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_CONFUSED) || HasAuraType(SPELL_AURA_MOD_FEAR) || GetCharmer()
        || HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FLEEING) || HasAuraType(SPELL_AURA_MOD_CONFUSE))
        return;
...
void Player::summonGuardian(uint32 entry, int32 duration, bool replaceOld) {
	CreatureInfo const* cInfo = sCreatureStorage.LookupEntry<CreatureInfo>(entry);
    if (!cInfo)
    {
        sLog.outErrorDb("Player::summonGuardian: creature entry %u not found.", entry);
        return;
    }

    if(FindGuardianWithEntry(entry)) {
		if(replaceOld) {
			RemoveGuardian(FindGuardianWithEntry(entry));
		} else {
			return;
		}
	}
    ...
    CreatureCreatePos pos(this, GetOrientation());

    Map *map = GetMap();
    uint32 pet_number = sObjectMgr.GeneratePetNumber();
    if (!spawnCreature->Create(map->GenerateLocalLowGuid(HIGHGUID_PET), pos, cInfo, pet_number))
    {
        sLog.outError("Player::summonGuardian: can't create creature entry %u for spell %u.", entry);
    ...
    spawnCreature->SetOwnerGuid(GetObjectGuid());
    spawnCreature->setPowerType(POWER_MANA);
    spawnCreature->SetUInt32Value(UNIT_NPC_FLAGS, spawnCreature->GetCreatureInfo()->npcflag);
    spawnCreature->setFaction(getFaction());
    spawnCreature->SetUInt32Value(UNIT_FIELD_PET_NAME_TIMESTAMP, 0);
    spawnCreature->SetCreatorGuid(GetObjectGuid());

    spawnCreature->InitStatsForLevel(level, this);
