...
void Object::BuildMovementUpdate(ByteBuffer * data, uint8 updateFlags) const
{
    ...
    *data << uint8(updateFlags);                            // update flags
    ...
    if (updateFlags & UPDATEFLAG_LIVING)
    {
		...
		float x, y, z;
		if ((m_objectTypeId == TYPEID_UNIT || m_objectTypeId == TYPEID_PLAYER) && ((Unit*)this)->GetMotionMaster()->GetDestination(x, y, z))
			moveFlags |= MOVEFLAG_WALK_MODE | MOVEFLAG_FORWARD | MOVEFLAG_ASCENDING;
    ...
    if (updateFlags & UPDATEFLAG_LIVING)                    // 0x20
    {
        ...
        *data << float(unit->GetSpeed(MOVE_TURN_RATE));
        ...
        if (m_objectTypeId == TYPEID_UNIT || m_objectTypeId == TYPEID_PLAYER)
        {
        ...
        for(uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                ...
                else if (index == UNIT_DYNAMIC_FLAGS && GetTypeId() == TYPEID_UNIT)
                {
                    ...
                    else if (((Creature*)this)->HasLootFor(target))
                        *data << (m_uint32Values[ index ] & ~UNIT_DYNFLAG_TAPPED);
                    else
                        *data << (m_uint32Values[index] & ~UNIT_DYNFLAG_TAPPED & ~UNIT_DYNFLAG_LOOTABLE);
                }
                else
                {
                    if (GetTypeId() == TYPEID_UNIT && index >= UNIT_FIELD_RESISTANCES_01 && index <= UNIT_FIELD_RESISTANCES_06
                        && (((Creature*)this)->m_baseResistance)[index - UNIT_FIELD_RESISTANCES_01])
                        *data << uint32(0);
                    else
...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj) const
{
    ...
    if (obj->GetObjectBoundingRadius() >= 5.f)
        return(IsWithinLOS(ox, oy, oz) || IsWithinLOS(ox, oy, oz + obj->GetObjectBoundingRadius()));
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz) const
{
	float x, y, z;
    GetPosition(x, y, z);
    if (GetObjectBoundingRadius() >= 5.f && GetMap()->IsInLineOfSight(x, y, z + GetObjectBoundingRadius() + 2.f, ox, oy, oz + 2.f))
        return true;
    ...
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            if (!((Creature const*)this)->CanFly())
            {
                ...
                // Between 0-5 meters under map ground -> probably an object clipping in ground
                float map_z = GetTerrain()->GetHeightStatic(x, y, z, false);
                if (z - map_z > -5.f && z - map_z < 0.f && GetTerrain()->GetMapZDiff(x, y, z, false) < 4.f)
                    z = map_z;
            }
            else
            {
        ...
        case TYPEID_PLAYER:
        {
            // for server controlled moves player work same as creature (but it can always swim)
            {
                ...
                // Between 0-10 meters under map ground -> probably an object clipping in ground
                float map_z = GetTerrain()->GetHeightStatic(x, y, z, false);
                if (z - map_z > -10.f && z - map_z < 0.f && GetTerrain()->GetMapZDiff(x, y, z, false) < 5.f)
                    z = map_z;
            }
            break;
        }
...
void WorldObject::MonsterTextEmote(const char* text, Unit* target, bool IsBossEmote)
{
	...
	//BuildMonsterChat(&data, GetObjectGuid(), CHAT_MSG_MONSTER_EMOTE, text, LANG_UNIVERSAL,
...
void WorldObject::MonsterTextEmote(int32 textId, Unit* target, bool IsBossEmote)
{
	...
	//MaNGOS::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_EMOTE, textId, LANG_UNIVERSAL, target);
    MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder> say_do(say_build);
    MaNGOS::CameraDistWorker<MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder> > say_worker(this,range,say_do);
    Cell::VisitWorldObjects(this, say_worker, range);
}
...
void WorldObject::BuildMonsterChat(WorldPacket *data, ObjectGuid senderGuid, uint8 msgtype, char const* text, uint32 language, char const* name, ObjectGuid targetGuid, char const* targetName)
{
	...
	*data << uint32(language);
	if (msgtype != CHAT_MSG_MONSTER_EMOTE && msgtype != CHAT_MSG_RAID_BOSS_EMOTE && msgtype != CHAT_MSG_RAID_BOSS_WHISPER)
...
GameObject* WorldObject::SummonGameObject(uint32 id, float x, float y, float z, float angle, uint32 despwtime)
{
	...
	if (!pGameObj->Create(map->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), id, map, x, y, z, angle, 0, 0, 0, 0, 100, GO_STATE_READY))
	{
		delete pGameObj;
		return NULL;
	}
