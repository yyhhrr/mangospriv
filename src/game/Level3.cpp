...
bool ChatHandler::HandleReloadLuaEngineCommand(char* /*args*/)
{
	sEluna->ReloadEluna();
	return true;
}
...
void ChatHandler::ShowItemListHelper( uint32 itemId, int loc_idx, Player* target /*=NULL*/, uint32 amount )
{
    ...
    if (m_session)
        PSendSysMessage(LANG_ITEM_LIST_CHAT, itemId, itemId, name.c_str(), usableStr, amount);
    else
        PSendSysMessage(LANG_ITEM_LIST_CONSOLE, itemId, name.c_str(), usableStr, amount);
}
...
bool ChatHandler::HandleLookupItemCommand(char* args)
{
    ...
    for (uint32 id = 0; id < sItemStorage.MaxEntry; id++)
    {
            ...
            if (il)
            {
                if ((int32)il->Name.size() > loc_idx && !il->Name[loc_idx].empty())
                {
                    ...
                    if (Utf8FitTo(name, wnamepart))
                    {
						...
						Player* target = getSelectedPlayer();
						if (target)
							amount = target->GetItemCount(pProto->ItemId, true);

                        ShowItemListHelper(pProto->ItemId, loc_idx, pl, amount);
                        ++counter;
                        continue;
                    }
                }
        ...
        if (Utf8FitTo(name, wnamepart))
		{
            ...
            ShowItemListHelper(pProto->ItemId, -1, pl, amount);
            ++counter;
        }
    }
...
bool ChatHandler::HandleNpcInfoCommand(char* /*args*/)
{
	...
	std::ofstream logFile("npc-info.txt", std::ios::app);
	if (logFile)
		logFile << target->GetGuidStr().c_str() << "\n";
...
bool ChatHandler::HandleGMFlyCommand(char* args)
{
	...
	*/
	/*
	auto group = m_session->GetPlayer()->GetGroup();
	if (!group)
		return true;

	WorldPacket data;
	ChatHandler::FillMessageData(&data, m_session, 1, LANG_ADDON, "KLHTM\tversionquery");
	group->BroadcastPacket(&data, false, group->GetMemberGroup(m_session->GetPlayer()->GetObjectGuid()));
	*/
	...
	obj->SetGoState(static_cast<GOState>((obj->GetGoState() + 1) % 3));*/
...
bool ChatHandler::HandleSummonAllCommand(char* args)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        if (pl->GetSession()->GetSecurity() == SEC_PLAYER)
			pl->TeleportTo(plr->GetMapId(), plr->GetPositionX(), plr->GetPositionY(), plr->GetPositionZ(), plr->GetOrientation());
	}

	return true;
}

bool ChatHandler::HandleSummonRaidCommand(char* args)
{
	Player* plr = getSelectedPlayer() ? getSelectedPlayer() : m_session->GetPlayer();
	if(!plr)
		return false;
	...
	{
		PSendSysMessage("You must be in a (raid) group!");
		return false;
	}

	auto* memberRef = grp->GetFirstMember();
	do
	{
		if(auto* pl = memberRef->getSource())
		{
			if(pl->isInCombat() == false && pl->isAlive())
				pl->TeleportTo(plr->GetMapId(), plr->GetPositionX(), plr->GetPositionY(), plr->GetPositionZ(), plr->GetOrientation());
		}
...
bool ChatHandler::HandleSummonAndRessRaidCommand(char* args)
{
	Player* plr = getSelectedPlayer() ? getSelectedPlayer() : m_session->GetPlayer();
	if(!plr)
		return false;
	...
	do
	{
		if(auto* pl = memberRef->getSource())
		{
			if(pl->isInCombat() == false)
			{
				pl->TeleportTo(plr->GetMapId(), plr->GetPositionX(), plr->GetPositionY(), plr->GetPositionZ(), plr->GetOrientation());
				pl->ResurrectPlayer(1.f);
			}
		}
