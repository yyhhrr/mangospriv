...
}*/
{
    ...
    StringCchPrintf( szPath, MAX_PATH, "%s\\Dumps\\", szPath);
    ...
    StringCchPrintf( szLogName, MAX_PATH, "%s%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld.log", 
               szPath, szVersion, 
               stLocalTime.wYear, stLocalTime.wMonth, stLocalTime.wDay, 
               stLocalTime.wHour, stLocalTime.wMinute, stLocalTime.wSecond, 
               GetCurrentProcessId(), GetCurrentThreadId());

	StringCchPrintf(szAssertName, MAX_PATH, "%s%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld-assert.log",
               szPath, szVersion, 
               stLocalTime.wYear, stLocalTime.wMonth, stLocalTime.wDay, 
               stLocalTime.wHour, stLocalTime.wMinute, stLocalTime.wSecond, 
               GetCurrentProcessId(), GetCurrentThreadId());
	...
	auto logPath = sConfig.GetStringDefault("LogsDir","");
	if(!logPath.empty())
		logPath += "\\";
	std::tr2::sys::path serverLog (logPath + "Server.log");
	std::tr2::sys::path assertLog (logPath + "assertion.log");
	std::tr2::sys::copy_file(serverLog, std::tr2::sys::path(szLogName));
	std::tr2::sys::copy_file(assertLog, std::tr2::sys::path(szAssertName));


	//ExceptionReportWin ew;
	//ew.recordExceptionInfo(pExceptionInfo);
	ExitProcess(0);
